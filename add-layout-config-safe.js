  ...
  try {
    ...
    if (!hasImport) {
      ...
      if (lastImportIndex >= 0) {
        ...
        lines.splice(lastImportIndex + 1, 0, 'import { usePageLayout } from "@/hooks/use-page-layout";');
      } else {
        // Find "use client" and add after it
        const useClientIndex = lines.findIndex(line => line.includes('use client'));
        if (useClientIndex >= 0) {
          lines.splice(useClientIndex + 2, 0, 'import { usePageLayout } from "@/hooks/use-page-layout";', '');
        } else {
          // Add at top after any comments
          let insertIndex = 0;
          while (insertIndex < lines.length && (lines[insertIndex].trim().startsWith('//') || lines[insertIndex].trim().startsWith('/*') || lines[insertIndex].trim() === '')) {
            insertIndex++;
          }
          lines.splice(insertIndex, 0, 'import { usePageLayout } from "@/hooks/use-page-layout";', '');
        }
      }
      modified = true;
    }
    ...
    if (!content.includes('usePageLayout')) {
      throw new Error('Validation failed: usePageLayout not found in modified content');
    }

    if (!content.includes('export default function')) {
      throw new Error('Validation failed: export default function not found in modified content');
    }
