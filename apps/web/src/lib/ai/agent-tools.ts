			...
			tables: [
				{
					name: "customers",
					description:
						"Customer records with contact info, revenue, and status",
				},
				{
					name: "team_members",
					description:
						"Employee/staff records with roles, contact info, and assignments",
				},
				{
					name: "vendors",
				...
				},
				{
					name: "jobs",
					description:
						"Work orders and service jobs with status, scheduling, and costs",
				},
				{
					name: "appointments",
...
export const queryDatabaseTool = tool({
	...
	inputSchema: z.object({
		...
		select: z
			...
			.optional()
			.default("*")
			.describe("Columns to select (comma-separated or * for all)"),
		filters: z
			.array(
				z.object({
					column: z.string(),
...
export const getCompanyOverviewTool = tool({
	...
	execute: async (_params, { companyId }: { companyId: string }) => {
		...
		const [
			...
			supabase
				.from("invoices")
				.select("id, status, total_amount, balance_amount", { count: "exact" })
				.eq("company_id", companyId),
			supabase
				.from("estimates")
				.select("id, status, total_amount", { count: "exact" })
		...
		const activeCustomers =
			customersResult.data?.filter((c) => c.status === "active").length || 0;
		const activeTeam =
			teamResult.data?.filter((t) => t.status === "active").length || 0;
		const openJobs =
			jobsResult.data?.filter((j) =>
				["pending", "scheduled", "in_progress"].includes(j.status),
			).length || 0;
		const completedJobs =
			jobsResult.data?.filter((j) => j.status === "completed").length || 0;
		const pendingInvoices =
			invoicesResult.data?.filter((i) => ["sent", "viewed"].includes(i.status))
				.length || 0;
		...
		const pendingEstimates =
			estimatesResult.data?.filter((e) => ["sent", "viewed"].includes(e.status))
				.length || 0;
		const activeContracts =
			contractsResult.data?.filter((c) => c.status === "active").length || 0;

		// Team role breakdown
		const roleBreakdown: Record<string, number> = {};
		teamResult.data?.forEach((t) => {
			roleBreakdown[t.role || "unassigned"] =
				(roleBreakdown[t.role || "unassigned"] || 0) + 1;
		});
...
export const searchAllEntitesTool = tool({
	...
	execute: async ({ query, limit }, { companyId }: { companyId: string }) => {
		...
		const searchResults = await searchAllEntities(supabase, companyId, query, {
			limit,
		});
		...
		if (searchResults.customers.length > 0)
			results.customers = searchResults.customers;
		if (searchResults.jobs.length > 0) results.jobs = searchResults.jobs;
		if (searchResults.properties.length > 0)
			results.properties = searchResults.properties;
		if (searchResults.equipment.length > 0)
			results.equipment = searchResults.equipment;
		if (searchResults.priceBookItems.length > 0)
			results.priceBookItems = searchResults.priceBookItems;
	...
	execute: async ({ customerId }, { companyId }: { companyId: string }) => {
		...
		const [customerResult, jobsResult, invoicesResult] = await Promise.all([
			...
			supabase
				.from("invoices")
				.select(
					"id, invoice_number, status, total_amount, balance_amount, due_date",
				)
				.eq("customer_id", customerId)
				.order("created_at", { ascending: false })
				.limit(5),
...
export const searchTeamMembersTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("team_members")
			.select(
				"id, first_name, last_name, email, phone, role, department, status, hire_date, avatar_url",
			)
			.eq("company_id", companyId)
			.eq("status", "active");
...
export const sendTeamSmsTool = tool({
	...
	execute: async (
		...
		if (!member?.phone)
			return { success: false, error: "Team member phone not found" };
...
export const searchVendorsTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("vendors")
			.select(
				"id, name, contact_name, email, phone, category, status, account_number, payment_terms",
			)
			.eq("company_id", companyId);
...
export const getVendorDetailsTool = tool({
	...
	execute: async ({ vendorId }, { companyId }: { companyId: string }) => {
		...
		if (vendorResult.error)
			return { success: false, error: vendorResult.error.message };
...
export const sendVendorSmsTool = tool({
	...
	execute: async (
		...
		if (!vendor?.phone)
			return { success: false, error: "Vendor phone not found" };
...
export const searchPropertiesTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("properties")
			.select(
				"id, name, address, city, state, zip_code, property_type, customer:customers(display_name)",
			)
			.eq("company_id", companyId);

		if (query)
			queryBuilder = queryBuilder.or(
				`address.ilike.%${query}%,name.ilike.%${query}%`,
			);
	...
	execute: async ({ propertyId }, { companyId }: { companyId: string }) => {
		...
		const [propertyResult, equipmentResult, jobsResult] = await Promise.all([
			...
			supabase
				.from("equipment")
				.select(
					"id, name, model, serial_number, install_date, warranty_expiry, last_service_date",
				)
		...
		if (propertyResult.error)
			return { success: false, error: propertyResult.error.message };
...
export const searchEquipmentTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("equipment")
			.select(
				"id, name, model, serial_number, equipment_type, install_date, warranty_expiry, last_service_date, property:properties(address, customer:customers(display_name))",
			)
			.eq("company_id", companyId);

		if (query)
			queryBuilder = queryBuilder.or(
				`name.ilike.%${query}%,model.ilike.%${query}%,serial_number.ilike.%${query}%`,
			);
		if (equipmentType)
			queryBuilder = queryBuilder.eq("equipment_type", equipmentType);
...
export const getMaintenanceDueTool = tool({
	...
	inputSchema: z.object({
		daysAhead: z
			...
			.optional()
			.default(30)
			.describe("Look ahead days for upcoming maintenance"),
	}),
	execute: async ({ daysAhead }, { companyId }: { companyId: string }) => {
		...
		const { data, error } = await supabase
			.from("equipment")
			.select(
				"id, name, model, last_service_date, next_service_date, property:properties(address, customer:customers(display_name, phone, email))",
			)
			.eq("company_id", companyId)
		...
		const overdue =
			data?.filter((e) => new Date(e.next_service_date) < new Date()) || [];
		const upcoming =
			data?.filter((e) => new Date(e.next_service_date) >= new Date()) || [];
...
export const searchJobsTool = tool({
	...
	execute: async (
		...
		if (query && !status && !customerId) {
			const jobs = await searchJobsFullText(supabase, companyId, query, {
				limit,
			});
		...
		let queryBuilder = supabase
			.from("jobs")
			.select(
				"id, job_number, title, status, total_amount, scheduled_start, scheduled_end, customer:customers(display_name, phone)",
			)
			.eq("company_id", companyId)
			.is("deleted_at", null);
		...
		if (query)
			queryBuilder = queryBuilder.or(
				`title.ilike.%${query}%,job_number.ilike.%${query}%`,
			);
...
export const createAppointmentTool = tool({
	...
	inputSchema: z.object({
		...
		notes: z
			.string()
			.optional()
			.describe("Additional notes for the technician"),
	}),
	execute: async (params, { companyId }: { companyId: string }) => {
		const supabase = createServiceSupabaseClient();
...
export const searchInvoicesTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("invoices")
			.select(
				"id, invoice_number, title, status, total_amount, balance_amount, due_date, customer:customers(display_name)",
			)
			.eq("company_id", companyId);

		if (query)
			queryBuilder = queryBuilder.ilike("invoice_number", `%${query}%`);
		if (status) queryBuilder = queryBuilder.eq("status", status);
		if (customerId) queryBuilder = queryBuilder.eq("customer_id", customerId);
		...
		const { data, error } = await supabase
			...
			.insert({
				...
				due_date: params.dueDate
					? new Date(params.dueDate).toISOString()
					: null,
				notes: params.notes,
				status: "draft",
			})
	...
	execute: async ({ period }, { companyId }: { companyId: string }) => {
		...
		const [invoicesResult, paymentsResult, overdueResult] = await Promise.all([
			...
			supabase
				.from("invoices")
				.select(
					"id, invoice_number, balance_amount, due_date, customer:customers(display_name)",
				)
				.eq("company_id", companyId)
				.eq("status", "sent")
		...
		const totalInvoiced =
			invoicesResult.data?.reduce((sum, inv) => sum + inv.total_amount, 0) || 0;
		const totalCollected =
			paymentsResult.data?.reduce((sum, pay) => sum + pay.amount, 0) || 0;
		const totalOutstanding =
			invoicesResult.data?.reduce((sum, inv) => sum + inv.balance_amount, 0) ||
			0;
...
export const getDashboardMetricsTool = tool({
	...
	execute: async (_params, { companyId }: { companyId: string }) => {
		...
		const completedJobs =
			jobsResult.data?.filter((j) => j.status === "completed") || [];
		const overdueInvoices =
			invoicesResult.data?.filter(
				(i) => i.status === "sent" && i.balance_amount > 0,
			) || [];
		...
		return {
			...
			metrics: {
				...
				totalOutstanding:
					overdueInvoices.reduce((sum, i) => sum + i.balance_amount, 0) / 100,
			},
		};
	},
});
...
export const getProactiveInsightsTool = tool({
	...
	execute: async (_params, { companyId }: { companyId: string }) => {
		...
		const [overdueResult, inactiveCustomersResult, upcomingAppointmentsResult] =
			await Promise.all([
				// Overdue invoices
				supabase
					.from("invoices")
					.select(
						"id, invoice_number, balance_amount, due_date, customer:customers(display_name, phone, email)",
					)
					.eq("company_id", companyId)
					.eq("status", "sent")
					.lt("due_date", new Date().toISOString())
				...
				supabase
					.from("appointments")
					.select(
						"id, title, start_time, customer:customers(display_name, phone)",
					)
					.eq("company_id", companyId)
					.eq("status", "scheduled")
		...
		if (overdueResult.data && overdueResult.data.length > 0) {
			...
			insights.push({
				...
				recommendation:
					"Consider sending payment reminders or initiating collection calls",
				data: overdueResult.data,
			});
		}

		if (
			inactiveCustomersResult.data &&
			inactiveCustomersResult.data.length > 0
		) {
			insights.push({
		...
		if (
			...
			upcomingAppointmentsResult.data.length > 0
		) {
			insights.push({
...
export const scheduleReminderTool = tool({
	...
	inputSchema: z.object({
		...
		subject: z
			.string()
			.optional()
			.describe("Email subject (for email channel)"),
...
export const getScheduledRemindersTool = tool({
	...
	execute: async (params, { companyId }: { companyId: string }) => {
		...
		if (params.recipientType)
			queryBuilder = queryBuilder.eq("recipient_type", params.recipientType);
		if (params.recipientId)
			queryBuilder = queryBuilder.eq("recipient_id", params.recipientId);
		if (params.relatedType)
			queryBuilder = queryBuilder.eq("related_type", params.relatedType);
		if (params.relatedId)
			queryBuilder = queryBuilder.eq("related_id", params.relatedId);
	...
	execute: async (
		...
		if (jobId) {
			...
			const { data: job, error } = await supabase
				.from("jobs")
				.select(
					"*, customer:customers(display_name), line_items, labor_cost, material_cost, total_amount",
				)
				.eq("id", jobId)
				.eq("company_id", companyId)
				.single();
		...
		const { data: jobs } = await supabase
			.from("jobs")
			.select(
				"id, title, labor_cost, material_cost, total_amount, status, completed_at, customer:customers(display_name)",
			)
			.eq("company_id", companyId)
			.eq("status", "completed")
		...
		const summary = {
			...
			totalRevenue:
				(jobs?.reduce((s, j) => s + (j.total_amount || 0), 0) || 0) / 100,
			totalLaborCost:
				(jobs?.reduce((s, j) => s + (j.labor_cost || 0), 0) || 0) / 100,
			totalMaterialCost:
				(jobs?.reduce((s, j) => s + (j.material_cost || 0), 0) || 0) / 100,
			totalProfit: 0,
			averageMargin: 0,
		};
		summary.totalProfit =
			summary.totalRevenue - summary.totalLaborCost - summary.totalMaterialCost;
...
export const getRevenueBreakdownTool = tool({
	...
	execute: async (
		...
		const { data: invoices } = await supabase
			.from("invoices")
			.select(
				"id, total_amount, paid_amount, created_at, customer:customers(id, display_name), job:jobs(service_type)",
			)
			.eq("company_id", companyId)
			.eq("status", "paid")
			.gte("created_at", startDate.toISOString());
...
export const getARAgingReportTool = tool({
	...
	execute: async (_params, { companyId }: { companyId: string }) => {
		...
		const { data: invoices } = await supabase
			.from("invoices")
			.select(
				"id, invoice_number, balance_amount, due_date, created_at, customer:customers(display_name, phone, email)",
			)
			.eq("company_id", companyId)
		...
		Object.keys(aging).forEach((key) => {
			aging[key as keyof typeof aging].amount =
				aging[key as keyof typeof aging].amount / 100;
		});
...
export const getTeamPerformanceReportTool = tool({
	...
	execute: async (
		...
		if (teamMemberId)
			queryBuilder = queryBuilder.eq("assigned_to", teamMemberId);
		...
		Object.values(performance).forEach((p) => {
			p.completionRate =
				p.jobsAssigned > 0 ? (p.jobsCompleted / p.jobsAssigned) * 100 : 0;
		});

		const results = Object.values(performance).sort(
...
export const getCustomerLifetimeValueTool = tool({
	...
	execute: async (
		...
		if (customerId) {
			...
			const totalRevenue =
				(invoicesResult.data
					...
					.reduce((s, i) => s + (i.paid_amount || 0), 0) || 0) / 100;
			const jobCount = jobsResult.data?.length || 0;
			const firstInvoice = invoicesResult.data?.sort(
				(a, b) =>
					new Date(a.created_at).getTime() - new Date(b.created_at).getTime(),
			)[0];
			const customerSince = firstInvoice
				? new Date(firstInvoice.created_at)
				: new Date(customerResult.data.created_at);
			const monthsAsCustomer = Math.max(
				1,
				Math.floor(
					(Date.now() - customerSince.getTime()) / (1000 * 60 * 60 * 24 * 30),
				),
			);
		...
		const clvData = customers?.map((c) => ({
			...
			averageJobValue:
				c.job_count > 0 ? (c.total_revenue || 0) / 100 / c.job_count : 0,
		}));
...
export const searchEstimatesTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("estimates")
			.select(
				"id, estimate_number, title, status, total_amount, valid_until, customer:customers(display_name)",
			)
			.eq("company_id", companyId);

		if (query)
			queryBuilder = queryBuilder.ilike("estimate_number", `%${query}%`);
		if (status) queryBuilder = queryBuilder.eq("status", status);
		if (customerId) queryBuilder = queryBuilder.eq("customer_id", customerId);
...
export const searchContractsTool = tool({
	...
	execute: async (
		...
		let queryBuilder = supabase
			.from("contracts")
			.select(
				"id, contract_number, title, status, total_value, start_date, end_date, customer:customers(display_name)",
			)
			.eq("company_id", companyId);

		if (query)
			queryBuilder = queryBuilder.ilike("contract_number", `%${query}%`);
		if (status) queryBuilder = queryBuilder.eq("status", status);
		if (customerId) queryBuilder = queryBuilder.eq("customer_id", customerId);
...
export const storeMemoryTool = tool({
	...
	execute: async (
		{ content, memoryType, entityType, entityId, importance, tags },
...
export const searchMemoriesTool = tool({
	...
	inputSchema: z.object({
		query: z
			.string()
			.describe(
				"Natural language search query (e.g., 'customer preferences for Johnson family')",
			),
		entityType: z
			.enum([
				"customer",
...
export const recallContextTool = tool({
	...
	inputSchema: z.object({
		topic: z
			.string()
			.describe(
				"The current topic or context (e.g., 'scheduling job for Smith residence', 'invoice payment issue')",
			),
		...
		if (customerName) {
			...
			if (customer) {
				...
				const customerMemories = await searchMemories(
					companyId,
					`${customerName} customer preferences`,
					{
						entityType: "customer",
...
export const searchCommunicationsFullTextTool = tool({
	...
	inputSchema: z.object({
		query: z
			.string()
			.describe(
				"Search query (e.g., 'water heater issue', 'preferred time morning')",
			),
		channel: z
			.enum(["email", "sms", "call", "all"])
			.optional()
			.default("all")
	...
	execute: async (
		...
		dbQuery = dbQuery.or(
			`body_plain.ilike.%${query}%,call_transcript.ilike.%${query}%,subject.ilike.%${query}%`,
		);
		...
		return {
			...
			call: {
				...
				duration: data.call_duration
					? `${Math.floor(data.call_duration / 60)}m ${data.call_duration % 60}s`
					: "Unknown",
...
export const searchVoicemailTranscriptsTool = tool({
	...
	inputSchema: z.object({
		...
		urgentOnly: z
			...
			.optional()
			.default(false)
			.describe("Only show urgent voicemails"),
		unreadOnly: z
			.boolean()
			.optional()
			.default(false)
...
export const getCustomerCommunicationHistoryTool = tool({
	...
	inputSchema: z.object({
		...
		includeTranscripts: z
			...
			.optional()
			.default(false)
			.describe("Include full call transcripts"),
	}),
	execute: async (
		{ customerId, days, includeTranscripts },
		{ companyId }: { companyId: string },
	) => {
		const supabase = createServiceSupabaseClient();
		...
		const { data: voicemails, error: vmError } = await supabase
			.from("voicemails")
			.select(
				"id, from_number, duration, transcription, is_urgent, received_at",
			)
			.eq("company_id", companyId)
			.eq("customer_id", customerId)
			.is("deleted_at", null)
...
Returns a clear recommendation on whether to proceed with outdoor work.`,
	...
	execute: async ({ lat, lon, jobType }) => {
		...
		return {
			...
			reason:
				suitability.reason ||
				"Weather conditions are acceptable for outdoor work",
...
- Notifying customers of delays`,
	inputSchema: z.object({
		...
		destinationLon: z
			.number()
			.describe("Longitude of the destination/job site"),
		originLat: z
			.number()
			.optional()
	...
	execute: async ({ destinationLat, destinationLon, originLat, originLon }) => {
		...
		if (!traffic) {
			return {
				success: false,
				error:
					"Unable to fetch traffic data. Check Google Maps API configuration.",
			};
		}
	...
	execute: async ({ address, city, state, zipCode, country }) => {
		...
		if (!result) {
			return {
				success: false,
				error:
					"Unable to geocode address. The address may be invalid or geocoding service unavailable.",
			};
		}
...
- Work suitability assessment`,
	inputSchema: z.object({
		...
		shopLat: z
			.number()
			.optional()
			.describe("Shop/office latitude for traffic routing"),
		shopLon: z
			.number()
			.optional()
			.describe("Shop/office longitude for traffic routing"),
		jobType: z
			.string()
			.optional()
	...
	execute: async ({ propertyLat, propertyLon, shopLat, shopLon, jobType }) => {
		...
		if (traffic && traffic.routeAffectingIncidents > 0) {
			issues.push(
				`${traffic.routeAffectingIncidents} traffic incident(s) on route`,
			);
		}
...
- Inspection requirements`,
	inputSchema: z.object({
		workType: z
			.string()
			.describe(
				"Type of work being performed (e.g., 'water heater replacement', 'outlet installation')",
			),
		propertyType: z
		...
		city: z
			.string()
			.optional()
			.describe("City for jurisdiction-specific guidance"),
	...
	execute: async ({ workType, propertyType, city, state }) => {
		...
		if (
			...
			if (workLower.includes("water heater")) {
				notes.push(
					"Water heater permits often include gas and electrical inspections",
				);
				confidence = "high";
			}
		}
		...
		if (workLower.includes("roof") || workLower.includes("re-roof")) {
			...
			inspections.push("Final inspection");
			notes.push(
				"Requirements vary significantly by jurisdiction - some exempt repairs under certain square footage",
			);
			confidence = "low";
		}
		...
		if (
			...
			notes.push(
				"Minor repairs typically exempt, but verify with local jurisdiction",
			);
			confidence = "low";
		}
		...
		if (
			workLower.includes("like for like") ||
			workLower.includes("same location")
		) {
			notes.push(
				"Like-for-like replacements may be exempt in some jurisdictions",
			);
			confidence = "low";
		}
...
Returns a checklist of items to verify for code compliance.`,
	inputSchema: z.object({
		jobType: z
			.string()
			.describe(
				"Type of job (e.g., 'water heater installation', 'panel upgrade', 'AC replacement')",
			),
		includesSafety: z
			.boolean()
			.optional()
				...
				for (const pattern of patterns[area] || []) {
					...
					for (const match of matches) {
						insights.push({
							...
							importance:
								area === "urgent" || area === "issues" ? "high" : "medium",
						});
					}
				}
			}
			...
			waypoints.map(async (wp) => {
				...
				try {
					const response = await fetch(geocodeUrl, {
						headers: {
							"User-Agent":
								"Stratos-FSM/1.0 (https://stratos.app; support@stratos.app)",
							Accept: "application/json",
						},
					});
					if (!response.ok) {
						console.warn(
							`Geocoding failed for ${wp.address}: ${response.status}`,
						);
		...
		if (apiKey) {
			try {
				...
				if (routeData.features?.[0]) {
					...
					return {
						distance_km: (feature.properties.summary.distance / 1000).toFixed(
							1,
						),
	...
	execute: async ({ location, supplyType, radiusKm }) => {
		...
		try {
			...
			const suppliers = data.elements
				.map(
					(el: {
						tags?: { name?: string; phone?: string; website?: string };
						lat?: number;
						lon?: number;
...
export const searchInventoryTool = tool({
	...
	execute: async ({ companyId, query, category, inStockOnly }) => {
		...
		let dbQuery = supabase
			.from("inventory")
			.select(
				"id, sku, name, description, category, quantity_on_hand, quantity_reserved, reorder_point, unit_cost, unit_price, location",
			)
			.eq("company_id", companyId)
			.or(
...
export const checkPartsAvailabilityTool = tool({
	...
	execute: async ({ companyId, parts }) => {
		...
		const inventoryQuery = supabase
			.from("inventory")
			.select(
				"id, sku, name, quantity_on_hand, quantity_reserved, unit_cost, unit_price",
			)
			.eq("company_id", companyId)
			.is("deleted_at", null);
		...
		if (nameOnlyParts.length > 0) {
			...
			inventoryItems.push(
				...nameResults.filter((r): r is NonNullable<typeof r> => r !== null),
			);
		}
		...
		const results = parts.map((part) => {
			const item = inventoryItems.find(
				(inv) =>
					...
					(part.name &&
						inv.name.toLowerCase().includes(part.name.toLowerCase())),
			);
		...
		return {
			...
			totalEstimatedPrice: results.reduce(
...
export const getLowStockAlertsTool = tool({
	...
	execute: async ({ companyId, category, limit }) => {
		...
		let query = supabase
			.from("inventory")
			.select(
				"id, sku, name, category, quantity_on_hand, reorder_point, reorder_quantity, unit_cost",
			)
			.eq("company_id", companyId)
			.is("deleted_at", null)
			...
			.map((item) => ({
				...
				urgency:
					item.quantity_on_hand === 0
						? "critical"
						: item.quantity_on_hand <= (item.reorder_point || 0) / 2
							? "high"
							: "normal",
			}));
		...
		return {
			...
			items: lowStockItems,
			criticalCount: lowStockItems.filter((i) => i.urgency === "critical")
				.length,
		};
	},
});
...
export const getPropertyEquipmentTool = tool({
	...
	execute: async ({ companyId, propertyId, customerId }) => {
		...
		let query = supabase
			.from("equipment")
			.select(
				"id, name, model_number, serial_number, manufacturer, install_date, warranty_expiry, last_service_date, next_service_due, status, notes",
			)
			.eq("company_id", companyId)
			.is("deleted_at", null);
		...
		return {
			equipment: data.map((eq) => ({
				...
				warrantyStatus: eq.warranty_expiry
					? new Date(eq.warranty_expiry) > now
						? "active"
						: "expired"
					: "unknown",
				serviceDue: eq.next_service_due
					? new Date(eq.next_service_due) <= now
					: false,
			})),
			total: data.length,
		};
...
export const getEquipmentServiceHistoryTool = tool({
	...
	execute: async ({ companyId, equipmentId, limit }) => {
		...
		const { data: serviceHistory } = await supabase
			.from("job_equipment")
			.select(
				"job:jobs(id, title, status, completed_at), notes, work_performed, created_at",
			)
...
export const checkEquipmentWarrantyTool = tool({
	...
	execute: async ({ companyId, equipmentId }) => {
		...
		const { data: equipment, error } = await supabase
			.from("equipment")
			.select(
				"id, name, model_number, serial_number, warranty_expiry, warranty_type, warranty_notes",
			)
			.eq("id", equipmentId)
			.eq("company_id", companyId)
			.single();
		...
		const now = new Date();
		const warrantyEnd = equipment.warranty_expiry
			? new Date(equipment.warranty_expiry)
			: null;
		...
		if (warrantyEnd) {
			daysRemaining = Math.ceil(
				(warrantyEnd.getTime() - now.getTime()) / (24 * 60 * 60 * 1000),
			);
			status =
				daysRemaining < 0
					? "expired"
	...
	execute: async ({ companyId, skills, certifications, date }) => {
		try {
			...
			const { data: members, error } = await supabase
				...
				.select(
					"id, role, skills, certifications, service_areas, user:users(id, first_name, last_name, email)",
				)
				.eq("company_id", companyId)
				.eq("role", "technician");
			...
			if (date) {
				...
				assignments?.forEach((a) => {
					if (a.assigned_to)
						jobCounts[a.assigned_to] = (jobCounts[a.assigned_to] || 0) + 1;
				});
			...
			return {
				error:
					err instanceof Error ? err.message : "Failed to find technicians",
			};
		}
	},
});
	...
	execute: async ({ companyId, technicianId, startDate, endDate }) => {
		try {
			...
			const { data: jobs } = await supabase
				.from("jobs")
				.select(
					"id, title, status, scheduled_start, scheduled_end, customer:customers(name)",
				)
			...
			return {
				error:
					err instanceof Error
						? err.message
						: "Failed to get technician workload",
			};
		}
	},
});
...
export const searchPriceBookTool = tool({
	...
	execute: async ({ companyId, query, category }) => {
		...
		let dbQuery = supabase
			...
			.select(
				"id, name, description, category, sku, unit_price, cost, is_active",
			)
			.eq("company_id", companyId)
			.eq("is_active", true)
			.or(
...
export const calculateEstimateTool = tool({
	...
	inputSchema: z.object({
		...
		discount: z
			.object({ type: z.enum(["percentage", "fixed"]), value: z.number() })
			.optional(),
	...
	execute: async ({ companyId, items, discount, taxRate }) => {
		...
		const priceBookIds = items
			.filter((i) => i.priceBookItemId)
			.map((i) => i.priceBookItemId);
		let priceBookItems: {
			id: string;
			name: string;
			unit_price: number;
		...
		if (priceBookIds.length > 0) {
			const { data } = await supabase
				.from("price_book_items")
				.select("id, name, unit_price, cost")
				.in("id", priceBookIds);
			priceBookItems = data || [];
		}
		...
		return {
			...
			subtotal,
			discount:
				discountAmount > 0 ? { ...discount, amount: discountAmount } : null,
	...
	execute: async ({ companyId, jobType, requiredSkills, preferredDate }) => {
		try {
			...
			const { data: technicians } = await supabase
				.from("company_memberships")
				.select(
					"id, skills, preferred_job_types, user:users(first_name, last_name)",
				)
				.eq("company_id", companyId)
				.eq("role", "technician");
			...
			const scored = technicians.map((tech) => {
				...
				const techSkills = ((tech.skills as string[]) || []).map((s) =>
					s.toLowerCase(),
				);
				const matchedSkills = (requiredSkills || []).filter((s) =>
					techSkills.includes(s.toLowerCase()),
				);
				...
				const preferredTypes = (
					(tech.preferred_job_types as string[]) || []
				).map((t) => t.toLowerCase());
			...
			return {
				error:
					err instanceof Error ? err.message : "Failed to suggest technician",
			};
		}
	},
});
	...
	execute: async ({ technicianId, date }) => {
		try {
			...
			const { data: jobs } = await supabase
				.from("jobs")
				.select(
					"id, title, scheduled_start, priority, property:properties(address, city, latitude, longitude)",
				)
				.eq("assigned_to", technicianId)
				.is("deleted_at", null)
				.gte("scheduled_start", dayStart.toISOString())
			...
			const jobsWithCoords = jobs
				.filter(
					(j) =>
						(j.property as { latitude: number; longitude: number })?.latitude,
				)
				.map((j) => ({
					...j,
			...
			if (jobsWithCoords.length < 2)
				return { message: "Not enough jobs with location data", jobs };
			...
			while (remaining.length > 0) {
				...
				remaining.forEach((job, idx) => {
					const dist = Math.sqrt(
						(job.lat - current.lat) ** 2 + (job.lng - current.lng) ** 2,
					);
			...
			return {
				error:
					err instanceof Error ? err.message : "Failed to optimize job order",
			};
		}
	},
});
...
export const webSearchTool = tool({
	...
	inputSchema: z.object({
		...
		dateRestrict: z
			...
			.optional()
			.describe(
				"Limit to recent results: d7 (7 days), m1 (1 month), y1 (1 year)",
			),
	}),
	execute: async ({ query, numResults, dateRestrict }) => {
		if (!googleCustomSearchService.isConfigured()) {
			return {
				success: false,
				error:
					"Web search is not configured. Please set GOOGLE_API_KEY and GOOGLE_SEARCH_ENGINE_ID.",
...
export const webSearchNewsTool = tool({
	...
	inputSchema: z.object({
		...
		daysBack: z
			...
			.optional()
			.default(7)
			.describe("How many days back to search (1-30)"),
		numResults: z
			.number()
			.optional()
			.default(5)
...
export const webSearchTechnicalTool = tool({
	...
	inputSchema: z.object({
		query: z
			.string()
			.describe(
				"The technical search query - include technology names and specific topics",
			),
		numResults: z
			.number()
			.optional()
...
export const webSearchSiteTool = tool({
	...
	inputSchema: z.object({
		...
		site: z
			.string()
			.describe(
				"The domain to search within (e.g., 'epa.gov', 'osha.gov', 'lennox.com')",
			),
		numResults: z
			.number()
			.optional()
 ...
 */
export function getDestructiveToolMetadata(
	toolName: string,
): DestructiveToolMetadata | null {
