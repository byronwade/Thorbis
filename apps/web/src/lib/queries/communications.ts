...
export const getCommunications = cache(
	async (
		...
		if (error) {
			console.error(
				"[Communications Query] Error fetching communications:",
				error,
			);
			return [];
		}
		...
		for (const comm of communications) {
			...
			if (canView) {
				filtered.push({
					...
					mailboxOwnerId: comm.mailbox_owner_id,
					assignedTo: comm.assigned_to,
					visibilityScope: comm.visibility_scope as
						| VisibilityScope
						| null
						| undefined,
					emailCategory: comm.category as EmailCategory | undefined,
...
/**
 * Get communications using optimized RPC function
 * Uses single query with filters instead of multiple post-fetch operations
 *
 * @param companyId - Company ID
 * @param channel - Filter by channel (email, sms, call, voicemail)
...
export const getCommunicationsFilteredRpc = cache(
	async (
		...
		limit: number = 50,
		offset: number = 0,
	): Promise<{ data: Communication[]; totalCount: number }> => {
		const supabase = await createServiceSupabaseClient();

		const { data, error } = await supabase.rpc("get_communications_filtered", {
		...
		// RPC returns array with total_count in each row
		...
		const communications: Communication[] = (data || []).map(
			(row: Record<string, unknown>) => ({
				...
				mailboxOwnerId: row.mailbox_owner_id as string | null,
				assignedTo: row.assigned_to as string | null,
				visibilityScope: row.visibility_scope as VisibilityScope | null,
				emailCategory: row.category as EmailCategory | undefined,
		...
		return { data: communications, totalCount };
	},
);
...
export const getCompanyCommunications = cache(
	async (
		...
		if (error) {
			console.error(
				"[Communications Query] Error fetching company communications:",
				error,
			);
			return [];
		}
