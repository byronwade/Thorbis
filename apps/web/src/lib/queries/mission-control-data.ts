...
export const getMissionControlData = cache(
	async (companyId?: string): Promise<MissionControlData> => {
		try {
        ...
        `,
			...
			const revenueTodayCents = completedToday.reduce(
				(sum, job) =>
					sum + formatCurrencyCents(job.financial?.total_amount ?? 0),
				0,
			);
			...
			if (metrics.revenueToday === 0) {
				alerts.push({
					...
					description:
						"Keep an eye on active jobs to ensure timely completion.",
				});
			}
