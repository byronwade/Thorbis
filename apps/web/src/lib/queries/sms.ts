 ...
 */
export const getSms = cache(
	async (params: SmsQueryParams = {}): Promise<SmsResult> => {
		...
		if (conversationWith) {
			query = query.or(
				`from_phone.eq.${conversationWith},to_phone.eq.${conversationWith}`,
			);
		}
		...
		if (search) {
			query = query.or(
				`body.ilike.%${search}%,from_phone.ilike.%${search}%,to_phone.ilike.%${search}%`,
			);
		}
 ...
 */
export const getSmsConversations = cache(
	async (limit: number = 50): Promise<SmsConversation[]> => {
		const companyId = await getActiveCompanyId();
		if (!companyId) {
			return [];
		}
		...
		const conversations: SmsConversation[] = data.map(
			(row: {
				phone_number: string;
				...
				lastMessage: {
					...
					from_phone:
						row.last_message_direction === "inbound" ? row.phone_number : null,
					to_phone:
						row.last_message_direction === "outbound" ? row.phone_number : null,
 ...
 */
export const getSmsById = cache(
	async (smsId: string): Promise<SmsRecord | null> => {
		const companyId = await getActiveCompanyId();
		if (!companyId) {
			return null;
		}
 ...
 */
export const getSmsConversation = cache(
	async (phoneNumber: string, limit: number = 100): Promise<SmsRecord[]> => {
		const companyId = await getActiveCompanyId();
		if (!companyId) {
			return [];
		}
