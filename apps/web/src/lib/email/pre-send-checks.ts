...
export async function checkSuppressionList(
	...
	Map<string, { suppressed: boolean; reason?: string; suppressedAt?: string }>
> {
	const supabase = await createServiceSupabaseClient();
	const results = new Map<
		string,
		{ suppressed: boolean; reason?: string; suppressedAt?: string }
	>();
...
export async function checkDomainWarmup(domainId: string): Promise<{
	...
	const { data: domain } = await supabase
		...
		.select(
			"created_at, emails_sent_today, total_emails_sent, warmup_completed",
		)
		.eq("id", domainId)
		.single();
...
export async function verifyDomainStatus(domainId: string): Promise<{
	...
	const { data: domain } = await supabase
		.from("company_email_domains")
		.select(
			"status, is_suspended, sending_enabled, spf_verified, dkim_verified, dmarc_verified",
		)
		.eq("id", domainId)
		.single();
...
export async function runPreSendChecks(
	...
	if (!domainStatus.canSend) {
		...
		if (!domainStatus.dnsStatus.dmarc) {
			warnings.push(
				"DMARC not configured - consider adding for better deliverability",
			);
		}
	}
	...
	if (suppressedCount === recipientEmails.length) {
		errors.push(
			"All recipients are on suppression list - no emails will be sent",
		);
	}
