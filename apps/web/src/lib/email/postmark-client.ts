...
async function postmarkRequest<T>(
	...
	if (!postmarkConfig.apiKey) {
		return {
			success: false,
			error:
				"Postmark API key is not configured. Set POSTMARK_API_KEY environment variable.",
		};
	}
...
async function sendPostmarkEmail(options: {
	...
	console.log(
		`[Postmark] Sending email to ${recipient}, subject: "${subject}"`,
	);
	...
	if (result.success) {
		console.log(
			`[Postmark] Email sent successfully, MessageID: ${result.data.MessageID}`,
		);
	} else {
...
export function verifyPostmarkWebhook(options: {
	...
	if (!postmarkConfig.webhookSecret) {
		console.warn(
			"[Postmark] No webhook secret configured - accepting webhook without verification",
		);
		return true;
	}
