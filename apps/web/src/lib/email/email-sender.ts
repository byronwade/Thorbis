...
export async function sendEmail({
	...
	try {
		...
		if (companyId && activeDomainId) {
			...
			const incrementResult = await incrementEmailCounter(
				activeDomain.domainId,
			);
		...
		if (providerResult.usedFallback && providerResult.success) {
			const primaryAttempt = providerResult.attempts.find(
				(a) => a.provider === "resend",
			);
			...
			console.log(
				`[EmailSender] Used Postmark fallback after Resend failed: ${primaryAttempt?.error}`,
			);
		}
		...
		if (providerResult.success) {
			...
			console.error(
				`[EmailSender] ✗ All providers failed: ${providerResult.error}`,
			);
			...
			return {
				success: false,
				error:
					providerResult.error || "Failed to send email (all providers failed)",
			};
		}
	} catch (error) {
...
export async function handleBounceWebhook(
	...
	const { addToSuppressionList, addToGlobalBounceList } = await import(
		"./pre-send-checks"
	);
