	...
	async acquire(
		...
		return new Promise((resolve, reject) => {
			...
			const timeoutId = setTimeout(() => {
				...
				const index = bucket.queue.findIndex(
					(r) => r.requestedAt === requestedAt,
				);
				if (index !== -1) {
	...
	getStatus(
		...
		return {
			...
			utilizationPercent: Math.round(
				((config.maxTokens - bucket.tokens) / config.maxTokens) * 100,
			),
		};
	}
...
export async function acquireRateLimit(
	...
	const globalKey = `global:${endpoint}`;
	const globalResult = await globalLimiter.acquire(
		globalKey,
		config,
		maxWaitMs,
	);
	...
	if (companyId) {
		const companyLimiter = getCompanyLimiter(companyId);
		const companyResult = await companyLimiter.acquire(
			endpoint,
			config,
			maxWaitMs,
		);
		...
		return {
			...companyResult,
			waitTime: Math.max(
				globalResult.waitTime || 0,
				companyResult.waitTime || 0,
			),
		};
	}
...
export function getRateLimitStatus(
	...
	const global: Record<
		string,
		ReturnType<TokenBucketLimiter["getStatus"]>
	> = {};
	...
	let company:
		| Record<string, ReturnType<TokenBucketLimiter["getStatus"]>>
		| undefined;
