...
import { TelnyxErrorCode, TelnyxWebhookError } from "./errors";
...
const WEBHOOK_CONFIG = {
	...
	logLevel: (process.env.TELNYX_WEBHOOK_LOG_LEVEL || "info") as
		| "debug"
		| "info",
};
...
export function verifyWebhookSignature(params: {
	...
	try {
		...
		if (!publicKeyBase64) {
			if (isProduction) {
				...
				return {
					valid: false,
					error:
						"TELNYX_PUBLIC_KEY is required in production - webhook rejected",
				};
			}
		...
		telnyxLogger.debug("Webhook signature verified successfully", {
			correlationId,
		});
		return { valid: true };
	} catch (error) {
...
export async function isWebhookDuplicateAsync(
	...
	try {
		...
		if (!supabase) {
			...
			telnyxLogger.warn(
				"Database not available, using in-memory deduplication",
			);
		...
		if (error) {
			telnyxLogger.error("Database dedup check failed", {
				error: error.message,
				webhookId,
			});
...
export async function cleanupExpiredWebhookDedup(): Promise<number> {
	try {
		...
		if (error) {
			telnyxLogger.error("Webhook dedup cleanup failed", {
				error: error.message,
			});
			return 0;
		}
		...
		if (deletedCount > 0) {
			telnyxLogger.info(
				`Cleaned up ${deletedCount} expired webhook dedup entries`,
			);
		}
