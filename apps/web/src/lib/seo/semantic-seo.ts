...
function generateAIOverviewMetadata(optimization: AIOverviewOptimization) {
	return {
...
function calculateContentQualityScore(signals: ContentQualitySignals): number {
	let score = 0;
...
function formatForFeaturedSnippet(content: FeaturedSnippetContent): string {
	switch (content.type) {
		case "paragraph":
