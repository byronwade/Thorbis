	...
	const bulkActions: BulkAction[] = [
		{
			...
			onClick: (selectedIds) => {
				...
				toast.success(
					`Exported ${selectedIds.size} payment${selectedIds.size > 1 ? "s" : ""}`,
				);
			},
		},
		{
			...
			onClick: async (selectedIds) => {
				...
				for (const paymentId of selectedIds) {
					...
					if (result.success) {
						...
						errorCount++;
						console.error(
							`Failed to archive payment ${paymentId}:`,
							result.error,
						);
					}
				}
				if (successCount > 0) {
					toast.success(
						`Archived ${successCount} payment${successCount > 1 ? "s" : ""}`,
					);
				}
				if (errorCount > 0) {
					toast.error(
						`Failed to archive ${errorCount} payment${errorCount > 1 ? "s" : ""}`,
					);
				}
