	...
	const bulkActions: BulkAction[] = [
		{
			...
			onClick: (selectedIds) => {
				// Navigate to new appointment with maintenance plans pre-selected
				window.location.href = `/dashboard/work/appointments/new?plans=${Array.from(selectedIds).join(",")}`;
			},
		},
		{
			label: "Export",
			...
			onClick: (selectedIds) => {
				// Export selected plans as CSV
				const selectedPlans = maintenancePlans.filter((p) =>
				...
				a.href = url;
				a.download = `maintenance-plans-export-${new Date().toISOString().split("T")[0]}.csv`;
				a.click();
				URL.revokeObjectURL(url);
				toast.success(
					`Exported ${selectedIds.size} plan${selectedIds.size > 1 ? "s" : ""}`,
				);
			},
		},
		{
		...
		},
		{
			...
			onClick: (selectedIds) => {
				toast.info(
					`Cancel functionality for ${selectedIds.size} plan${selectedIds.size > 1 ? "s" : ""} coming soon`,
				);
			},
			variant: "destructive",
		},
