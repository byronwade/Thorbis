	...
	const bulkActions: BulkAction[] = [
		{
			...
			onClick: (selectedIds) => {
				// Navigate to new purchase order with materials pre-selected
				window.location.href = `/dashboard/work/purchase-orders/new?materials=${Array.from(selectedIds).join(",")}`;
			},
		},
		{
			label: "Export",
			...
			onClick: (selectedIds) => {
				...
				toast.success(
					`Exported ${selectedIds.size} material${selectedIds.size > 1 ? "s" : ""}`,
				);
			},
		},
		{
			...
			onClick: (selectedIds) => {
				toast.info(
					`Archive functionality for ${selectedIds.size} material${selectedIds.size > 1 ? "s" : ""} coming soon`,
				);
			},
		},
	];
