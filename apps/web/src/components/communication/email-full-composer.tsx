...
import { deleteDraftAction, saveDraftAction } from "@/actions/email-actions";
import {
	type Recipient,
...
const CustomerContextCard = dynamic(
	() =>
		import("@/components/communication/customer-context-card").then(
			(mod) => mod.CustomerContextCard,
		),
	{
		loading: () => (
						...
						{isPendingSend ? (
							...
							<div className="flex items-center">
								<Button
									...
									onClick={handleSend}
									disabled={
										isSending || !body.trim() || recipients.length === 0
									}
								>
									{isSending ? (
