...
export function EmailReplyComposer({
	...
	useEffect(() => {
		...
		const addRecipient = (
			...
			if (
				email &&
				email !== "Unknown" &&
				!recipientMap.has(email.toLowerCase())
			) {
	...
	const getSchedulePreset = (
		preset: "tomorrow_morning" | "tomorrow_afternoon" | "monday_morning",
	) => {
		const now = new Date();
		let date: Date;
	...
	const handleScheduleSend = async (scheduledFor: Date) => {
		...
		const firstCustomerRecipient = recipients.find(
			(r) => r.type === "customer",
		);
	...
	const smartReplies = useMemo(() => {
		...
		const emailContent =
			`${selectedEmail.subject || ""} ${selectedEmail.body || ""}`.toLowerCase();
		const suggestions: string[] = [];

		// Questions - suggest answers
		if (
			emailContent.includes("?") ||
			emailContent.includes("when") ||
			emailContent.includes("can you")
		) {
			if (
				emailContent.includes("schedule") ||
				emailContent.includes("appointment") ||
				emailContent.includes("when")
			) {
				suggestions.push(
			...
			if (
				emailContent.includes("price") ||
				emailContent.includes("cost") ||
				emailContent.includes("quote")
			) {
				suggestions.push(
					"I'll prepare a quote for you and send it over shortly.",
				);
				suggestions.push(
					"The pricing depends on a few factors. Could you provide more details?",
				);
			}
		...
		// Thank you messages
		if (
			emailContent.includes("thank") ||
			emailContent.includes("thanks") ||
			emailContent.includes("appreciate")
		) {
			suggestions.push(
		...
		if (
			emailContent.includes("confirm") ||
			emailContent.includes("let me know")
		) {
		...
		// Urgent / ASAP
		if (
			emailContent.includes("urgent") ||
			emailContent.includes("asap") ||
			emailContent.includes("emergency")
		) {
				...
				{isExpanded && (
					<div className="border-t border-border/50 px-3 py-2 space-y-2 bg-muted/30">
						<div className="flex items-start gap-2">
							<span className="text-xs font-medium text-muted-foreground w-12 pt-2">
								To:
							</span>
							<RecipientAutocomplete
								value={recipients}
										...
										<div className="space-y-2">
											...
											<div className="space-y-1">
												<Button
													...
													onClick={() =>
														handleScheduleSend(
															getSchedulePreset("tomorrow_morning"),
														)
													}
												>
												...
												<Button
													...
													onClick={() =>
														handleScheduleSend(
															getSchedulePreset("tomorrow_afternoon"),
														)
													}
												>
													<Clock className="h-3.5 w-3.5 mr-2" />
												...
												<Button
													...
													onClick={() =>
														handleScheduleSend(
															getSchedulePreset("monday_morning"),
														)
													}
												>
