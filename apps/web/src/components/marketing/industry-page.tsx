...
> = {
	...
	flow: {
		...
		badgeStyle:
			"bg-gradient-to-r from-sky-500/20 to-blue-500/20 text-sky-600 dark:text-sky-400",
	...
	aquatic: {
		...
		badgeStyle:
			"bg-gradient-to-r from-cyan-500/20 to-teal-500/20 text-cyan-600 dark:text-cyan-400",
		statsLayout: "horizontal",
	...
	technical: {
		...
		badgeStyle:
			"bg-gradient-to-r from-slate-500/20 to-gray-500/20 text-slate-600 dark:text-slate-400",
		statsLayout: "grid",
	...
	elevated: {
		...
		cardStyle:
			"border-stone-500/20 hover:border-stone-500/40 clip-path-[polygon(0_0,100%_5%,100%_100%,0_95%)]",
		badgeStyle:
			"bg-gradient-to-r from-stone-500/20 to-amber-500/20 text-stone-600 dark:text-stone-400",
		statsLayout: "stacked",
	...
	secure: {
		...
		cardStyle:
			"border-zinc-500/30 hover:border-zinc-400/50 bg-zinc-900/5 dark:bg-zinc-100/5",
		badgeStyle:
			"bg-gradient-to-r from-zinc-600/30 to-slate-600/30 text-zinc-200 dark:text-zinc-300",
		statsLayout: "stacked",
	...
	industrial: {
		...
		badgeStyle:
			"bg-gradient-to-r from-neutral-500/20 to-stone-500/20 text-neutral-600 dark:text-neutral-400",
		statsLayout: "grid",
...
function GridStats({
	...
	stats: MarketingIndustryContent["stats"];
	accentColor: string;
}) {
	return (
		...
		<div className="space-y-4">
			{stats.map((stat, index) => (
				...
				>
					...
					<p className={cn("text-4xl font-bold", accentColor)}>{stat.value}</p>
		...
		<div className="grid grid-cols-2 gap-6">
			{stats.map((stat) => (
				<div
					...
					key={stat.label}
				>
					<p className={cn("text-3xl font-bold", accentColor)}>{stat.value}</p>
...
export function IndustryPage({ industry }: IndustryPageProps) {
	...
	const renderStats = () => {
		switch (config.statsLayout) {
			...
			default:
				return (
					<GridStats stats={industry.stats} accentColor={config.accentColor} />
				);
		}
	};
		...
		<div className="space-y-16">
			...
			>
				...
				<div className="relative grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,480px)]">
					<div className="space-y-6 px-6 py-12 sm:px-10 lg:px-16">
						<Badge
							className={cn("px-4 py-1.5 font-semibold", config.badgeStyle)}
						>
			...
			<section className="space-y-8">
				<div className="text-center">
					<Badge className={cn("mb-4", config.badgeStyle)}>Capabilities</Badge>
					<h2 className="text-3xl font-bold tracking-tight">
						Tailored to Your Industry
					</h2>
					<p className="text-muted-foreground mx-auto mt-2 max-w-2xl">
						Purpose-built features that solve {industry.name.toLowerCase()}{" "}
						challenges
					</p>
				</div>
				<div
						...
						return (
							<Card
								className={cn(
									...
									config.cardStyle,
									industry.designVariant === "stacked" &&
										index === 0 &&
										"md:col-span-2",
								)}
								key={prop.title}
							...
							{industry.playbook.map((item, index) => (
								<div className="space-y-2" key={item.title}>
									<div className="flex items-start gap-3">
										...
										>
											<span
												className={cn("text-sm font-bold", config.accentColor)}
											>
												{index + 1}
											</span>
										</div>
			...
			{industry.testimonial ? (
				...
				>
					<div
						className={cn(
							"absolute right-8 top-8 text-8xl opacity-10",
							config.accentColor,
						)}
					>
						&ldquo;
			...
			<section className="space-y-6">
				<div className="text-center">
					<Badge className={cn("mb-4", config.badgeStyle)}>FAQ</Badge>
					<h2 className="text-3xl font-bold tracking-tight">
			...
			>
				...
				<p className="text-muted-foreground mx-auto mb-6 max-w-2xl text-lg">
					$200/month base with unlimited users. No per-technician fees. No
					long-term contracts.
				</p>
