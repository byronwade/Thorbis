...
export function ActivityLogContent() {
	...
	const [undoDialog, setUndoDialog] = useState<{
		...
		preview: Awaited<ReturnType<typeof previewUndoAction>>["preview"];
	} | null>(null);
		...
		startTransition(async () => {
			const [logResult, statsResult] = await Promise.all([
				getActivityLogAction({
					...
					offset: page * 50,
					severityFilter:
						severityFilters.length > 0 ? severityFilters : undefined,
	...
	const confirmUndo = async () => {
		...
		const result = await undoActionAction(
			undoDialog.entry.id,
				...
				<div className="divide-y overflow-auto h-full">
					{isPending && entries.length === 0 ? (
						...
						<div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
							...
							<p className="text-xs">
								AI actions will appear here when performed
							</p>
						</div>
					) : (
						entries.map((entry) => {
							const severity =
								SEVERITY_CONFIG[entry.severity as SeverityFilter] ||
								SEVERITY_CONFIG.low;
								...
								>
									...
									>
										<ActionIcon
											className={cn(
												"size-4",
												`text-${severity.color.replace("bg-", "")}`,
											)}
										/>
									</div>
			...
			>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							This will restore the entity to its previous state. This action is
							logged.
						</DialogDescription>
					</DialogHeader>
							...
							<div className="p-3 bg-muted rounded-lg space-y-2 text-sm">
								<div className="flex justify-between">
									...
									<span className="font-medium">
										{undoDialog.preview.entityType}
									</span>
								</div>
								<div className="flex justify-between">
									<span className="text-muted-foreground">Operation</span>
									<span className="font-medium">
										{undoDialog.preview.operation}
									</span>
								</div>
								{undoDialog.preview.changedFields.length > 0 && (
									<div className="flex justify-between">
										...
										<span className="font-medium">
											{undoDialog.preview.changedFields.join(", ")}
										</span>
									</div>
								)}
							</div>
					...
					<DialogFooter>
						...
						<Button
							onClick={confirmUndo}
							disabled={isUndoing || !undoDialog?.preview?.canRevert}
							variant="destructive"
						>
