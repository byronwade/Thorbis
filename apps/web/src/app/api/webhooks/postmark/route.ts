...
} from "@/lib/email/postmark-client";
...
async function handleDelivery(payload: PostmarkWebhookPayload) {
	console.log(`[Postmark Webhook] Delivery confirmed for ${payload.Recipient}`);
...
async function handleSpamComplaint(payload: PostmarkWebhookPayload) {
	console.log(`[Postmark Webhook] Spam complaint from ${payload.Recipient}`);
