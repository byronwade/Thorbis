	...
	try {
		...
		if (!timestampResult.valid) {
			...
			return NextResponse.json(
				createWebhookResponse(
					false,
					timestampResult.error || "Invalid timestamp",
				),
				{ status: 400 },
			);
		}
		...
		return NextResponse.json(createWebhookResponse(false, "Internal error"), {
			status: 500,
		});
	}
}
...
async function handleCallEvent(
	...
	const supabase = await createServiceSupabaseClient();
	if (!supabase) {
		telnyxLogger.error("Failed to create Supabase client for call event", {
			correlationId,
			eventType,
		});
	...
	switch (eventType) {
		case "call.initiated": {
			await handleCallInitiated(
				payload as CallInitiatedPayload,
				supabase,
				correlationId,
			);
			break;
...
async function handleCallInitiated(
	...
	if (customerLookup.hasDuplicates) {
		providerMetadata.duplicate_customers = {
			...
			message:
				"Multiple customers found with this phone number. Consider merging these contacts.",
		};
	}
	if (Object.keys(providerMetadata).length > 0) {
		communicationPayload.provider_metadata =
			providerMetadata as CommunicationInsert["provider_metadata"];
	}
...
async function handleMessageEvent(
	...
	const supabase = await createServiceSupabaseClient();
	if (!supabase) {
		telnyxLogger.error("Failed to create Supabase client for message event", {
			correlationId,
			eventType,
		});
		throw new Error("Database unavailable");
	...
	switch (eventType) {
		...
		default:
			telnyxLogger.debug("Unhandled message event", {
				correlationId,
				eventType,
			});
	}
