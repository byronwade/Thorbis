...
async function handleCallInitiated(payload: any) {
	...
	if (error) {
		console.error(
			"Telnyx webhook: Failed to create call log for call.initiated:",
			error.message,
		);
	}
}
...
async function handleCallAnswered(payload: any) {
	...
	if (error) {
		console.error(
			"Telnyx webhook: Failed to update call log for call.answered:",
			error.message,
		);
	}
...
async function handleCallHangup(payload: any) {
	...
	if (error) {
		console.error(
			"Telnyx webhook: Failed to update call log for call.hangup:",
			error.message,
		);
	}
...
async function handleRecordingSaved(payload: any) {
	...
	if (error) {
		console.error(
			"Telnyx webhook: Failed to update call log with recording:",
			error.message,
		);
	}
...
async function handleMachineDetection(payload: any) {
	...
	if (error) {
		console.error(
			"Telnyx webhook: Failed to update call log with machine detection:",
			error.message,
		);
	}
	...
	if (result === "machine") {
		console.info(
			`Telnyx webhook: Machine detected for call ${call_control_id} - would route to voicemail`,
		);
		// TODO: Implement voicemail routing via Telnyx call control API
	}
}
