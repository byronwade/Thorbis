...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (authError || !user) {
			console.error(
				"[Gmail OAuth] User not authenticated:",
				authError?.message,
			);
		...
		if (membershipError || !membership) {
			...
			return redirectWithError(
				"You must be a company admin or owner to connect Gmail",
			);
		}
...
async function exchangeCodeForTokens(
	...
	try {
		...
		if (!clientId || !clientSecret) {
			console.error(
				"[Gmail OAuth] Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET",
			);
			return null;
		}
		...
		if (!response.ok) {
			...
			console.error(
				`[Gmail OAuth] Token exchange failed: ${response.status}`,
				errorText,
			);
			return null;
		}
...
async function getGoogleUserInfo(
	...
	try {
		...
		if (!response.ok) {
			const errorText = await response.text();
			console.error(
				`[Gmail OAuth] Failed to get user info: ${response.status}`,
				errorText,
			);
			return null;
		}
