...
async function calculateSeasonalFactors(
	...
	for (let month = 0; month < 12; month++) {
		...
		const revenueFactor =
			avgRevenuePerMonth > 0 ? data.revenue / avgRevenuePerMonth : 1;

		await supabase.from("analytics_seasonal_factors").upsert(
			{
	...
	for (let day = 0; day < 7; day++) {
		...
		const demandFactor =
			avgJobsPerDay > 0 ? (data.jobs / ((totalJobs / 7) * 52)) * 52 : 1;
		const revenueFactor =
			avgRevenuePerDay > 0 ? data.revenue / (totalRevenue / 7) : 1;
	...
	// Find peak periods
	const peakMonth = Object.entries(byMonth).sort(
		(a, b) => b[1].jobs - a[1].jobs,
	)[0];
	const slowMonth = Object.entries(byMonth).sort(
		(a, b) => a[1].jobs - b[1].jobs,
	)[0];
