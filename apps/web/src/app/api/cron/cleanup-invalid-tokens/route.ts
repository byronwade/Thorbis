...
export async function GET(request: NextRequest) {
	...
	try {
		...
		invalidCutoff.setDate(
			invalidCutoff.getDate() - INVALID_TOKEN_RETENTION_DAYS,
		);
		...
		if (invalidError) {
			...
			console.error(
				"[Token Cleanup] Error deleting invalid tokens:",
				invalidError,
			);
		} else {
			deletedTokens += invalidTokens?.length || 0;
			console.log(
				`[Token Cleanup] Deleted ${invalidTokens?.length || 0} invalid tokens`,
			);
		}
		...
		unsyncedCutoff.setDate(
			unsyncedCutoff.getDate() - UNSYNCED_TOKEN_RETENTION_DAYS,
		);
		...
		if (unsyncedError) {
			...
			console.error(
				"[Token Cleanup] Error deleting unsynced tokens:",
				unsyncedError,
			);
		} else {
			deletedTokens += unsyncedTokens?.length || 0;
			console.log(
				`[Token Cleanup] Deleted ${unsyncedTokens?.length || 0} never-synced tokens`,
			);
		}
		...
		if (orphanedError) {
			...
			console.error(
				"[Token Cleanup] Error deleting orphaned tokens:",
				orphanedError,
			);
		} else {
			deletedTokens += orphanedTokens?.length || 0;
			console.log(
				`[Token Cleanup] Deleted ${orphanedTokens?.length || 0} orphaned tokens`,
			);
		}
		...
		if (inactiveError) {
			...
			console.error(
				"[Token Cleanup] Error deleting inactive accounts:",
				inactiveError,
			);
		} else {
			deletedAccounts += inactiveAccounts?.length || 0;
			console.log(
				`[Token Cleanup] Deleted ${inactiveAccounts?.length || 0} inactive accounts`,
			);
		}
		...
		const { data: orphanedAccounts, error: orphanedAccountsError } =
			await supabase
				.from("user_email_accounts")
				.delete()
		...
		if (orphanedAccountsError) {
			...
			console.error(
				"[Token Cleanup] Error deleting orphaned accounts:",
				orphanedAccountsError,
			);
		} else {
			deletedAccounts += orphanedAccounts?.length || 0;
			console.log(
				`[Token Cleanup] Deleted ${orphanedAccounts?.length || 0} orphaned accounts`,
			);
		}
