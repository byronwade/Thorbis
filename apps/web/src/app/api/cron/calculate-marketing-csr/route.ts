...
export async function GET(request: Request) {
	...
	try {
		...
		for (const company of companies) {
			try {
				const campaignsProcessed = await calculateMarketingROI(
					supabase,
					company.id,
				);
		...
		const totalCampaigns = results.reduce(
			(sum, r) => sum + r.campaignsProcessed,
			0,
		);
...
async function calculateMarketingROI(
	...
	for (const campaign of campaigns) {
		try {
			...
			const totalLeads =
				calls?.filter(
					(c) => c.call_outcome !== "spam" && c.call_outcome !== "wrong_number",
				)?.length ?? 0;
			...
			if (jobIds.length > 0) {
				...
				totalRevenue =
					jobs?.reduce((sum, j) => sum + (j.total_amount ?? 0), 0) ?? 0;
				totalProfit =
					jobs?.reduce((sum, j) => sum + (j.profit_actual ?? 0), 0) ?? 0;
			}
			...
			await supabase.from("analytics_marketing_attribution").upsert(
				{
...
async function calculateChannelAttribution(
	...
	for (const [channel, data] of Object.entries(channelData)) {
		const callAnswerRate =
			data.calls > 0 ? (data.answered / data.calls) * 100 : 0;
...
async function calculateCSRMetrics(
	...
	for (const csr of csrUsers) {
		try {
			...
			const callAnswerRate =
				totalCalls > 0 ? (totalCallsAnswered / totalCalls) * 100 : 0;
			...
			const bookingRate =
				opportunities > 0 ? (jobsBooked / opportunities) * 100 : 0;

			// Calculate revenue attribution
			const bookedRevenue =
				calls?.reduce((sum, c) => sum + (c.attributed_revenue ?? 0), 0) ?? 0;
			const avgBookedTicket = jobsBooked > 0 ? bookedRevenue / jobsBooked : 0;
			const revenuePerCall =
				totalCallsAnswered > 0 ? bookedRevenue / totalCallsAnswered : 0;

			// Calculate timing metrics
			const avgTalkTime =
				calls && calls.length > 0
					? calls.reduce((sum, c) => sum + (c.talk_time_seconds ?? 0), 0) /
						calls.length
					: 0;
			const avgRingTime =
				calls && calls.length > 0
					? calls.reduce((sum, c) => sum + (c.ring_time_seconds ?? 0), 0) /
						calls.length
					: 0;
			const avgHoldTime =
				calls && calls.length > 0
					? calls.reduce((sum, c) => sum + (c.hold_time_seconds ?? 0), 0) /
						calls.length
					: 0;
			const totalTalkMinutes =
			...
			const avgQualityScore =
				callsWithScore.length > 0
					? callsWithScore.reduce(
							(sum, c) => sum + (c.call_quality_score ?? 0),
							0,
						) / callsWithScore.length
					: 0;
			const positiveCallsCount =
				calls?.filter((c) => c.customer_sentiment === "positive")?.length ?? 0;
			const negativeCallsCount =
				calls?.filter((c) => c.customer_sentiment === "negative")?.length ?? 0;
			...
			const performanceScore =
				bookingScore + revenueScore + volumeScore + qualityScore;
			...
			// Upsert CSR metrics
			await supabase.from("analytics_csr_metrics").upsert(
				{
					company_id: companyId,
					user_id: csr.id,
					date: todayStr,
	...
	if (csrMetrics.length > 1) {
		...
		const bookingRanks = [...csrMetrics].sort(
			(a, b) => b.bookingRate - a.bookingRate,
		);
		const revenueRanks = [...csrMetrics].sort((a, b) => b.revenue - a.revenue);
		const volumeRanks = [...csrMetrics].sort(
			(a, b) => b.callVolume - a.callVolume,
		);

		for (const metric of csrMetrics) {
			const bookingRank =
				bookingRanks.findIndex((m) => m.userId === metric.userId) + 1;
			const revenueRank =
				revenueRanks.findIndex((m) => m.userId === metric.userId) + 1;
			const volumeRank =
				volumeRanks.findIndex((m) => m.userId === metric.userId) + 1;
