...
async function calculateCompanySummary(
	...
	// Revenue metrics
	const revenueTotal = last30Snapshots.reduce(
		(sum, s) => sum + (s.total_revenue ?? 0),
		0,
	);
	const revenue7day = last7Snapshots.reduce(
		(sum, s) => sum + (s.total_revenue ?? 0),
		0,
	);
	const avgJobValue =
		last30Snapshots.reduce((sum, s) => sum + (s.average_ticket ?? 0), 0) /
		Math.max(last30Snapshots.length, 1);

	// Jobs metrics
	const jobsCompleted = last30Snapshots.reduce(
		(sum, s) => sum + (s.jobs_completed ?? 0),
		0,
	);
	const jobsCreated = last30Snapshots.reduce(
		(sum, s) => sum + (s.jobs_created ?? 0),
		0,
	);
	const jobsCancelled = last30Snapshots.reduce(
		(sum, s) => sum + (s.jobs_cancelled ?? 0),
		0,
	);
	const firstTimeFixes = last30Snapshots.reduce(
		(sum, s) => sum + (s.first_time_fix_count ?? 0),
		0,
	);
	const callbacks = last30Snapshots.reduce(
		(sum, s) => sum + (s.callback_count ?? 0),
		0,
	);
	const firstTimeFixRate =
		jobsCompleted > 0 ? (firstTimeFixes / jobsCompleted) * 100 : 0;
	const callbackRate =
		jobsCompleted > 0 ? (callbacks / jobsCompleted) * 100 : 0;

	// Estimate metrics
	const estimatesCreated = last30Snapshots.reduce(
		(sum, s) => sum + (s.estimates_created ?? 0),
		0,
	);
	const estimatesApproved = last30Snapshots.reduce(
		(sum, s) => sum + (s.estimates_approved ?? 0),
		0,
	);
	const estimatesDeclined = last30Snapshots.reduce(
		(sum, s) => sum + (s.estimates_declined ?? 0),
		0,
	);
	...
	// Customer metrics
	const newCustomers = last30Snapshots.reduce(
		(sum, s) => sum + (s.new_customers ?? 0),
		0,
	);
	const totalCustomers = last30Snapshots[0]?.total_active_customers ?? 0;

	// Communications
	const emailsSent = last30Snapshots.reduce(
		(sum, s) => sum + (s.emails_sent ?? 0),
		0,
	);
	const emailsReceived = last30Snapshots.reduce(
		(sum, s) => sum + (s.emails_received ?? 0),
		0,
	);
	const smsSent = last30Snapshots.reduce(
		(sum, s) => sum + (s.sms_sent ?? 0),
		0,
	);
	const smsReceived = last30Snapshots.reduce(
		(sum, s) => sum + (s.sms_received ?? 0),
		0,
	);
	const callsMade = last30Snapshots.reduce(
		(sum, s) => sum + (s.calls_made ?? 0),
		0,
	);
	const callsReceived = last30Snapshots.reduce(
		(sum, s) => sum + (s.calls_received ?? 0),
		0,
	);
	...
	// Response time
	const responseTimes = last30Snapshots
		.filter((s) => s.average_response_time_minutes)
		.map((s) => s.average_response_time_minutes!);
	...
	// Contracts
	const contractsCreated = last30Snapshots.reduce(
		(sum, s) => sum + (s.contracts_created ?? 0),
		0,
	);
	const contractsRenewed = last30Snapshots.reduce(
		(sum, s) => sum + (s.contracts_renewed ?? 0),
		0,
	);
	const contractsCancelled = last30Snapshots.reduce(
		(sum, s) => sum + (s.contracts_cancelled ?? 0),
		0,
	);
	const activeContracts = last30Snapshots[0]?.active_contracts ?? 0;
	const contractRevenue = last30Snapshots.reduce(
		(sum, s) => sum + (s.contract_revenue ?? 0),
		0,
	);
	const contractRenewalRate =
		contractsCreated + contractsRenewed > 0
			? (contractsRenewed / (contractsCreated + contractsRenewed)) * 100
			: 0;

	// Technician metrics
	const activeTechnicians = last30Snapshots[0]?.active_technicians ?? 0;
	const utilizationRates = last30Snapshots
		.filter((s) => s.utilization_rate)
		.map((s) => s.utilization_rate!);
	const utilizationRate =
		utilizationRates.length > 0
			? utilizationRates.reduce((a, b) => a + b, 0) / utilizationRates.length
			: 0;
	const jobsPerTech =
		activeTechnicians > 0 ? jobsCompleted / activeTechnicians : 0;
	const revenuePerTech =
		activeTechnicians > 0 ? revenueTotal / activeTechnicians : 0;
	...
	const pipelineValue = (openEstimates ?? []).reduce(
		(sum, e) => sum + (e.total_amount ?? 0),
		0,
	);
	const weightedPipeline = (openEstimates ?? []).reduce(
		(sum, e) =>
			sum + (e.total_amount ?? 0) * ((e.conversion_probability ?? 50) / 100),
		0,
	);
	...
	for (const job of jobsWithEstimates ?? []) {
		if (job.estimated_amount && job.estimated_amount > 0) {
			const accuracy =
				100 -
				Math.abs(
					(((job.total_amount ?? 0) - job.estimated_amount) /
	...
	const avgQuoteAccuracy =
		quoteAccuracyCount > 0 ? quoteAccuracySum / quoteAccuracyCount : 0;

	// Capacity utilization from daily snapshots
	const capacityValues = last30Snapshots
		.filter((s) => s.capacity_utilization)
		.map((s) => s.capacity_utilization);
	...
	const totalBillableHours = last30Snapshots.reduce(
		(sum, s) => sum + (s.billable_hours ?? 0),
		0,
	);
	const totalNonBillableHours = last30Snapshots.reduce(
		(sum, s) => sum + (s.non_billable_hours ?? 0),
		0,
	);
	const billableHoursRatio =
		totalBillableHours + totalNonBillableHours > 0
			? (totalBillableHours / (totalBillableHours + totalNonBillableHours)) *
				100
			: 0;

	// Cash flow forecasting
	const avgDailyRevenue = revenueTotal / 30;
	const avgDailyPayments =
		last30Snapshots.reduce((sum, s) => sum + (s.payments_value ?? 0), 0) / 30;
	const cashFlow30DayForecast =
		avgDailyPayments * 30 - avgDailyRevenue * 0.6 * 30; // Assuming 60% costs
	...
	// Collection rate
	const totalInvoiced = last30Snapshots.reduce(
		(sum, s) => sum + (s.invoices_value ?? 0),
		0,
	);
	const totalCollected = last30Snapshots.reduce(
		(sum, s) => sum + (s.payments_value ?? 0),
		0,
	);
	...
	const avgCustomerLifetimeValue =
		customerRevenue && customerRevenue.length > 0
			? customerRevenue.reduce((sum, c) => sum + (c.total_revenue ?? 0), 0) /
	...
	const avgJobProfitability =
		profitableJobs && profitableJobs.length > 0
			? profitableJobs.reduce(
					(sum, j) => sum + (j.gross_margin_percent ?? 0),
					0,
				) / profitableJobs.length
			: 0;
	...
	const serviceAgreementRevenue = (serviceAgreements ?? []).reduce(
		(sum, sa) => sum + (sa.monthly_value ?? 0) * 12,
		0,
	);
	const recurringRevenuePercent =
		revenueTotal > 0
			? (serviceAgreementRevenue / 12 / (revenueTotal / 30)) * 100
			: 0;

	// Preventive vs reactive ratio
	const preventiveJobs = last30Snapshots.reduce(
		(sum, s) => sum + (s.preventive_maintenance_count ?? 0),
		0,
	);
	const reactiveJobs = jobsCompleted - preventiveJobs;
	const preventiveVsReactiveRatio =
		reactiveJobs > 0 ? preventiveJobs / reactiveJobs : 0;
	...
	// Growth calculations
	const lastMonthRevenue = recentSnapshots
		.slice(30, 60)
		.reduce((sum, s) => sum + (s.total_revenue ?? 0), 0);
	const revenueGrowthMom =
		lastMonthRevenue > 0
			? ((revenueTotal - lastMonthRevenue) / lastMonthRevenue) * 100
			: 0;

	// Customer retention (simplified)
	const churnedCustomers = last30Snapshots.reduce(
		(sum, s) => sum + (s.churned_customers ?? 0),
		0,
	);
	const retentionRate =
		totalCustomers > 0
	...
	for (const s of last30Snapshots) {
		if (s.most_common_job_type)
			jobTypes[s.most_common_job_type] =
				(jobTypes[s.most_common_job_type] ?? 0) +
				(s.most_common_job_type_count ?? 0);
		if (s.most_common_lead_source)
			leadSources[s.most_common_lead_source] =
				(leadSources[s.most_common_lead_source] ?? 0) +
				(s.most_common_lead_source_count ?? 0);
		if (s.most_common_payment_method)
			paymentMethods[s.most_common_payment_method] =
				(paymentMethods[s.most_common_payment_method] ?? 0) +
				(s.most_common_payment_method_count ?? 0);
	}

	const topJobType = Object.entries(jobTypes).sort((a, b) => b[1] - a[1])[0];
	const topLeadSource = Object.entries(leadSources).sort(
		(a, b) => b[1] - a[1],
	)[0];
	const topPaymentMethod = Object.entries(paymentMethods).sort(
		(a, b) => b[1] - a[1],
	)[0];

	// Upsert company summary
	await supabase.from("analytics_company_summary").upsert(
		{
