 ...
 */
export async function getSmsAction(input: GetSmsInput): Promise<GetSmsResult> {
	try {
		const parseResult = getSmsSchema.safeParse(input);

		if (!parseResult.success) {
			throw new Error(
				`Invalid input parameters: ${parseResult.error.errors.map((e) => `${e.path.join(".")}: ${e.message}`).join(", ")}`,
			);
		}
 ...
 */
export async function markSmsConversationAsReadAction(
	phoneNumber: string,
): Promise<{
	success: boolean;
	error?: string;
...
export async function getSmsFolderCountsAction(): Promise<{
	...
	try {
		...
		const [inboxResult, sentResult, archiveResult, trashResult] =
			await Promise.all([
				// Inbox
				baseQuery
 ...
 */
export async function uploadSmsAttachments(files: File[]): Promise<{
	success: boolean;
	urls?: string[];
	error?: string;
}> {
