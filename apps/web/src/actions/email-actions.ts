...
const getEmailsSchema = z
	.object({
		...
		sortBy: z
			.enum(["created_at", "sent_at", "subject"])
			.optional()
			.default("created_at"),
...
export async function getEmailsAction(
	...
	try {
		...
		if (!parseResult.success) {
			...
			throw new Error(
				`Invalid input parameters: ${parseResult.error.issues.map((e) => `${e.path.map(String).join(".")}: ${e.message}`).join(", ")}`,
			);
		}
...
export async function markEmailAsReadAction(
	...
	try {
		...
		if (error instanceof z.ZodError) {
			return {
				success: false,
				error: `Invalid input: ${error.issues.map((e: { message: string }) => e.message).join(", ")}`,
			};
		}
		const errorMessage =
	...
	try {
		...
		if (providedContent) {
			...
			if (email.body_html || email.body) {
				...
				const webhookContent = metadata.webhook_content as
				...
				if (!html && !text) {
					const fullContent = metadata.full_content as
						| Record<string, unknown>
						| undefined;
					if (fullContent) {
						...
						for (const field of htmlFields) {
							if (
								fullContent[field] &&
								typeof fullContent[field] === "string"
							) {
				...
				if (!html && !text) {
					const webhookData = metadata.data as
						| Record<string, unknown>
						| undefined;
					if (webhookData) {
				...
				if (html || text) {
					...
					if ((html || text) && supabase) {
						...
						if (updateError) {
							console.warn(
								"⚠️  Failed to update email content in database:",
								updateError.message,
							);
						}
					}
		...
		if (supabase) {
			...
			if (updateError) {
				console.warn(
					"⚠️  Failed to update email content in database (this is OK, content still returned):",
					updateError.message,
				);
			}
		}
...
export async function toggleSpamEmailAction(emailId: string): Promise<{
	...
	try {
		...
		const isCurrentlySpam =
			currentTags.includes("spam") || email?.category === "spam";
...
export async function saveDraftAction(input: SaveDraftInput): Promise<{
	...
	try {
		...
		const fromAddress =
			companySettings?.email_from_address || "noreply@example.com";
...
export async function getDraftAction(draftId: string): Promise<{
	...
	try {
		...
		const { data: draft, error: fetchError } = await supabase
			.from("communications")
			.select(
				"id, to_address, cc_address, bcc_address, subject, body, body_html, customer_id, attachments, updated_at",
			)
			.eq("id", draftId)
			.eq("company_id", companyId)
			.eq("status", "draft")
		...
		return {
			...
			draft: {
				...
				attachments: draft.attachments as Array<{
					filename: string;
					content: string;
					contentType?: string;
				}> | null,
...
export async function retryFailedEmailAction(emailId: string): Promise<{
	...
	try {
		...
		const attachments = metadata?.scheduled_attachments as
			| Array<{
					filename: string;
					content: string;
					contentType?: string;
