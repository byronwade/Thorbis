...
/**
 * Map stock adjustment reason to movement type
 */
function mapReasonToMovementType(
	reason: z.infer<typeof stockAdjustmentSchema>["reason"],
): string {
...
async function reserveStock(formData: FormData): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		await supabase.from("stock_movements").insert({
			...
			quantity_change: 0, // Reservation doesn't change on-hand, just reserved
...
async function releaseReservedStock(
	...
	return withErrorHandling(async () => {
		...
		// Update stock_reservations status to released
		// Find active reservations for this inventory and update them
		await supabase
			...
			.eq("status", "active")
			.lte("quantity_reserved", quantity);
...
async function useReservedStock(
	...
	return withErrorHandling(async () => {
		...
		// Update stock_reservations status to fulfilled
		if (jobId) {
			await supabase
				.from("stock_reservations")
				.update({
