...
export async function POST(request: NextRequest) {
	try {
		...
		if (!data) {
			return NextResponse.json({ error: "Invalid webhook structure: missing data" }, { status: 400 });
		}
		...
		if (!eventType) {
			return NextResponse.json({ error: "Invalid webhook structure: missing event_type" }, { status: 400 });
		}

		if (!payload) {
			return NextResponse.json({ error: "Invalid webhook structure: missing payload" }, { status: 400 });
		}
