...
import { logGmailConnected, logAuditEvent } from "@/lib/email/audit-logger";
...
export async function GET(request: NextRequest) {
	...
	try {
		...
		if (authError || !user) {
			console.error("[Gmail User OAuth] User not authenticated:", authError?.message);
		...
		if (state.userId !== user.id) {
			console.error("[Gmail User OAuth] User ID mismatch - potential CSRF attack");
		...
		if (memberError || !teamMember) {
			console.error("[Gmail User OAuth] Team member not found or doesn't belong to user");
		...
		const hasReadonly = scopes.includes("https://www.googleapis.com/auth/gmail.readonly");
		...
		if (accountError || !emailAccount) {
			console.error("[Gmail User OAuth] Failed to create email account:", accountError?.message);
		...
		if (!storeResult.success) {
			console.error("[Gmail User OAuth] Failed to store tokens:", storeResult.error);
...
async function exchangeCodeForTokens(code: string): Promise<GoogleTokenResponse | null> {
	try {
		...
		if (!clientId || !clientSecret) {
			console.error("[Gmail User OAuth] Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET");
			return null;
		}
		...
		if (!response.ok) {
			...
			console.error(`[Gmail User OAuth] Token exchange failed: ${response.status}`, errorText);
			return null;
		}
...
async function getGoogleUserInfo(accessToken: string): Promise<GoogleUserInfo | null> {
	try {
		...
		if (!response.ok) {
			const errorText = await response.text();
			console.error(`[Gmail User OAuth] Failed to get user info: ${response.status}`, errorText);
			return null;
		}
