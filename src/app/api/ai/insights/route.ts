...
export async function POST(req: Request) {
	try {
		...
		if (overdueCount && overdueCount > 0) {
			const totalOverdue = overdueInvoices?.reduce((sum, inv) => sum + (inv.total || 0), 0) || 0;
			insights.push({
				id: "overdue-invoices",
				type: "urgent",
				category: "financial",
		...
		const thisWeekTotal = thisWeekRevenue?.reduce((sum, inv) => sum + (inv.total || 0), 0) || 0;
		const lastWeekTotal = lastWeekRevenue?.reduce((sum, inv) => sum + (inv.total || 0), 0) || 0;

		const trend: "up" | "down" | "stable" =
			thisWeekTotal > lastWeekTotal ? "up" : thisWeekTotal < lastWeekTotal ? "down" : "stable";
