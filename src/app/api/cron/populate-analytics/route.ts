...
export async function GET(request: Request) {
	...
	try {
		...
		for (const company of companies) {
			try {
				await populateDailySnapshot(supabase, company.id, snapshotDate, dayStart, dayEnd);
...
async function populateDailySnapshot(
	...
	const [
		...
		supabase
			.from("jobs")
			.select("id, status, total_revenue, actual_start_time, actual_end_time, job_type, is_callback")
		...
		supabase
			.from("invoice_items")
			.select("id, quantity, unit_price, item_type, invoice:invoices!inner(company_id, created_at)")
	...
	const totalRevenue = completedJobs.reduce((sum, j) => sum + (j.total_revenue ?? 0), 0);
	const averageTicket = jobsCompleted > 0 ? Math.round(totalRevenue / jobsCompleted) : 0;
	...
	const estimatesApproved = estimates.filter((e) => e.conversion_status === "won").length;
	const estimatesDeclined = estimates.filter((e) => e.conversion_status === "lost").length;
	const estimateConversionRate = estimatesCreated > 0
		? (estimatesApproved / estimatesCreated) * 100
		: 0;
	const estimateTotalValue = estimates.reduce((sum, e) => sum + (e.total_amount ?? 0), 0);
	...
	// Payment metrics
	const paymentsReceived = payments.reduce((sum, p) => sum + (p.amount ?? 0), 0);
	...
	const appointmentShowRate = appointmentsScheduled > 0
		? ((appointmentsCompleted / appointmentsScheduled) * 100)
		: 0;

	// Communication metrics
	const emailsSent = communications.filter((c) => c.type === "email" && c.direction === "outbound").length;
	const emailsReceived = communications.filter((c) => c.type === "email" && c.direction === "inbound").length;
	const smsSent = communications.filter((c) => c.type === "sms" && c.direction === "outbound").length;
	const smsReceived = communications.filter((c) => c.type === "sms" && c.direction === "inbound").length;
	const callsMade = communications.filter((c) => c.type === "call" && c.direction === "outbound").length;
	const callsReceived = communications.filter((c) => c.type === "call" && c.direction === "inbound").length;
	...
	const contractsRenewed = contracts.filter((c) => c.status === "renewed").length;
	...
	const jobValues = completedJobs.map((j) => j.total_revenue ?? 0).sort((a, b) => a - b);
	const medianJobValue = jobValues.length > 0
		? jobValues[Math.floor(jobValues.length / 2)]
		: 0;

	// Calculate percentiles
	const p25 = jobValues.length > 0 ? jobValues[Math.floor(jobValues.length * 0.25)] : 0;
	const p75 = jobValues.length > 0 ? jobValues[Math.floor(jobValues.length * 0.75)] : 0;
	const p90 = jobValues.length > 0 ? jobValues[Math.floor(jobValues.length * 0.90)] : 0;
	...
	const mostCommonJobType = Object.entries(jobTypeCounts).sort((a, b) => b[1] - a[1])[0];
	...
	const mostCommonPaymentMethod = Object.entries(paymentMethodCounts).sort((a, b) => b[1] - a[1])[0];
	...
	const capacityUtilization = capacityAvailableHours > 0
	...
	const emergencyCalls = jobs.filter((j) => (j as { priority?: string }).priority === "emergency").length;
	...
	const driveTimeTotal = (appointmentsResult.data ?? [])
		.reduce((sum, a) => sum + ((a as { drive_time_minutes?: number }).drive_time_minutes ?? 0), 0);

	// Quotes/estimates metrics for daily tracking
	const quotesValue = estimates.reduce((sum, e) => sum + (e.total_amount ?? 0), 0);
		...
		.upsert(
			{
				...
				invoices_created: invoicesCreated,
				invoices_value: invoices.reduce((sum, i) => sum + (i.total_amount ?? 0), 0),
