...
function getWebhookUrl() {
	const base = process.env.NEXT_PUBLIC_SITE_URL || process.env.SITE_URL || "https://www.thorbis.com";
...
export async function GET(request: Request) {
	...
	try {
		...
		if (TELNYX_CONFIG.connectionId) {
			try {
				const connection = await telnyxClient.callControlApplications.retrieve(TELNYX_CONFIG.connectionId);
		...
		if (TELNYX_CONFIG.messagingProfileId) {
			try {
				const profile = await telnyxClient.messagingProfiles.retrieve(TELNYX_CONFIG.messagingProfileId);
