...
export async function GET(request: Request) {
	...
	try {
		...
		for (const company of companies) {
			try {
				const dispatchProcessed = await calculateDispatchEfficiency(supabase, company.id);
				const pricebookItemsProcessed = await calculatePricebookPerformance(supabase, company.id);
				const customersProcessed = await calculateCustomerHealth(supabase, company.id);
				results.push({
		...
		const totalPricebook = results.reduce((sum, r) => sum + r.pricebookItemsProcessed, 0);
		const totalCustomers = results.reduce((sum, r) => sum + r.customersProcessed, 0);
...
async function calculateDispatchEfficiency(
	...
	const scheduleFillRate = totalAvailableHours > 0 ? (totalScheduledHours / totalAvailableHours) * 100 : 0;
	const utilizationRate = totalAvailableHours > 0 ? (totalBillableHours / totalAvailableHours) * 100 : 0;

	const avgDriveTimeBetweenJobs = totalJobsDispatched > 1
		? totalDriveTimeMinutes / (totalJobsDispatched - 1)
		: 0;
	const avgDriveMilesBetweenJobs = totalJobsDispatched > 1
		? totalDriveMiles / (totalJobsDispatched - 1)
		: 0;

	const totalWorkMinutes = totalScheduledMinutes + totalDriveTimeMinutes;
	const driveTimeRatio = totalWorkMinutes > 0 ? (totalDriveTimeMinutes / totalWorkMinutes) * 100 : 0;

	const onTimeArrivalRate = totalJobsDispatched > 0 ? (onTimeArrivals / totalJobsDispatched) * 100 : 0;
	const earlyArrivalRate = totalJobsDispatched > 0 ? (earlyArrivals / totalJobsDispatched) * 100 : 0;
	const lateArrivalRate = totalJobsDispatched > 0 ? (lateArrivals / totalJobsDispatched) * 100 : 0;
	const rescheduleRate = totalJobsDispatched > 0 ? (rescheduled / totalJobsDispatched) * 100 : 0;

	const avgJobsPerTech = activeTechnicians > 0 ? totalJobsDispatched / activeTechnicians : 0;
	...
	const revenuePerMile = totalDriveMiles > 0 ? totalRevenue / totalDriveMiles : 0;
	...
	await supabase
		...
		.upsert({
			...
			avg_drive_miles_between_jobs: Math.round(avgDriveMilesBetweenJobs * 100) / 100,
...
async function calculatePricebookPerformance(
	...
	const itemStats: Record<string, {
		priceBookItem: { id: string; name: string; category: string; unit_price: number; cost: number; item_type: string; target_margin_percent: number } | null;
		timesSold: number;
	...
	for (const item of invoiceItems) {
		const pbItem = item.price_book_item as { id: string; name: string; category: string; unit_price: number; cost: number; item_type: string; target_margin_percent: number } | null;
		...
		itemStats[key].totalCost += ((item.cost_price ?? 0) * (item.quantity ?? 1)) / 100;
	}
	...
	for (const [key, stats] of Object.entries(itemStats)) {
		...
		const avgPrice = stats.prices.length > 0
			...
			: 0;
		const minPrice = stats.prices.length > 0 ? Math.min(...stats.prices) / 100 : 0;
		const maxPrice = stats.prices.length > 0 ? Math.max(...stats.prices) / 100 : 0;

		const listPrice = (pbItem?.unit_price ?? 0) / 100;
		const priceVariance = listPrice > 0 ? ((avgPrice - listPrice) / listPrice) * 100 : 0;
		const discountRate = listPrice > 0 && avgPrice < listPrice
			? ((listPrice - avgPrice) / listPrice) * 100
			: 0;

		const totalProfit = stats.totalRevenue - stats.totalCost;
		const grossMargin = stats.totalRevenue > 0 ? (totalProfit / stats.totalRevenue) * 100 : 0;
		const markup = stats.totalCost > 0 ? (totalProfit / stats.totalCost) * 100 : 0;
...
async function calculateCustomerHealth(
	...
	for (const customer of customers) {
		try {
			...
			const allJobs = jobs || [];
			const jobs12m = allJobs.filter(j =>
				new Date(j.created_at) >= oneYearAgo
			);

			// Calculate days since last service
			const daysSinceLastService = customer.last_service_date
				? Math.floor((today.getTime() - new Date(customer.last_service_date).getTime()) / (1000 * 60 * 60 * 24))
				: 365;
			...
			const interactions30d = comms?.filter(c =>
				new Date(c.created_at) >= new Date(today.getTime() - 30 * 24 * 60 * 60 * 1000)
			)?.length ?? 0;

			const daysSinceLastContact = comms && comms.length > 0
				? Math.floor((today.getTime() - new Date(comms[0].created_at).getTime()) / (1000 * 60 * 60 * 24))
				: 180;
			...
			const outstandingBalance = invoices?.reduce((sum, inv) => sum + (inv.balance_amount ?? 0), 0) / 100 ?? 0;
			...
			const totalRevenue12m = jobs12m.reduce((sum, j) => sum + (j.total_amount ?? 0), 0) / 100;
			const avgJobValue = totalJobsLifetime > 0 ? totalRevenueLifetime / totalJobsLifetime : 0;
