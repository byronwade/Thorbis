...
export async function GET(request: Request) {
	...
	try {
		...
		const results: Array<{ companyId: string; jobsProcessed: number; success: boolean; error?: string }> = [];
		...
		for (const company of companies) {
			try {
				const jobsProcessed = await calculateJobCostingForCompany(supabase, company.id);
				results.push({ companyId: company.id, jobsProcessed, success: true });
			} catch (error: unknown) {
				const message = error instanceof Error ? error.message : "Unknown error";
				results.push({ companyId: company.id, jobsProcessed: 0, success: false, error: message });
			}
		}
	...
	for (const job of jobs) {
		try {
			...
			for (const item of invoiceItems ?? []) {
				const itemCost = (item.cost_price ?? item.unit_price ?? 0) * (item.quantity ?? 1);
			...
			// Calculate totals
			const totalCostActual = laborCostActual + materialsCostActual + equipmentCostActual + driveTimeCost;
			...
			let budgetStatus: "under_budget" | "on_budget" | "over_budget" = "on_budget";
			if (costVariancePercent < -5) budgetStatus = "under_budget";
			else if (costVariancePercent > 5) budgetStatus = "over_budget";

			// Calculate gross margin
			const grossMarginPercent = revenue > 0 ? (profitActual / revenue) * 100 : 0;
...
async function updateTechnicianJobMetrics(
	...
	for (const tech of technicians) {
		...
		const totalCost = jobs.reduce((sum, j) => sum + (j?.total_cost_actual ?? 0), 0);
		const totalProfit = jobs.reduce((sum, j) => sum + (j?.profit_actual ?? 0), 0);
