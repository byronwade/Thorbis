...
export default async function TestTelnyxConfig() {
  ...
  const allChecks = [
    ...
    },
    {
      ...
      status: Array.isArray(phoneNumber?.features) && phoneNumber.features.length > 0,
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Telnyx Configuration Check</h1>

      <div className={`p-6 rounded-lg mb-8 ${allPassing ? "bg-green-100 border-2 border-green-500" : "bg-red-100 border-2 border-red-500"}`}>
      ...
      <div className="space-y-4">
        {allChecks.map((check, i) => (
          <div
            key={i}
            className={`p-4 rounded border-2 ${check.status ? "bg-green-50 border-green-300" : "bg-red-50 border-red-300"}`}
          >
            <div className="flex items-center justify-between">
              <h3 className="font-semibold">{check.name}</h3>
              <span className={`text-sm ${check.status ? "text-green-700" : "text-red-700"}`}>
                {check.status ? "PASS" : "FAIL"}
              </span>
            </div>
            <p className="text-sm mt-1 font-mono">{check.message}</p>
          </div>
        ))}
      </div>

      <div className="mt-8 p-6 bg-gray-100 rounded">
        <h2 className="font-bold text-lg mb-4">Raw Data</h2>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-300 rounded">
        ...
        <ul className="list-disc list-inside space-y-1 text-sm">
          <li>
            ...
            <a href="/test-telnyx-send" className="text-blue-600 underline">
              /test-telnyx-send
            </a>
          </li>
          <li>
            If any checks fail, review the error messages and fix the configuration
          </li>
          <li>
            Check your environment variables in <code>.env.local</code>
          </li>
          <li>
