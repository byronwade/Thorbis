/**
 * Invoices > Pending Page - Server Component
 *
	...
	return (
		<div className="space-y-6">
			<div>
				...
				<p className="text-muted-foreground">Manage unpaid invoices and follow up on overdue payments</p>
			</div>
								...
								<div className="flex items-center gap-4 rounded-lg border p-4" key={index}>
									...
									<div className="flex-1 space-y-1">
										<div className="flex items-center gap-2">
											<p className="font-medium text-sm leading-none">{invoice.customer}</p>
											<span
												className={`inline-flex items-center rounded-full px-2 py-1 font-medium text-xs ${
													invoice.status === "Pending" ? "bg-warning text-warning" : "bg-destructive text-destructive"
												}`}
											>
												{invoice.status}
											</span>
										...
										<p className="text-muted-foreground text-sm">{invoice.services}</p>
										<p className="text-muted-foreground text-sm">Due: {invoice.dueDate}</p>
									</div>
									<div className="text-right">
										<p className="font-medium text-sm">{invoice.amount}</p>
										<button className="text-primary text-xs hover:underline">Send Reminder</button>
									</div>
								</div>
							))}
						</div>
						...
						<div className="space-y-4">
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									...
									<p className="text-muted-foreground text-xs">Email overdue customers</p>
								</div>
								<div className="text-right">
									<button className="rounded bg-primary px-3 py-1 text-primary-foreground text-sm">Send All</button>
								</div>
							</div>
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									<p className="font-medium text-sm">Payment Plans</p>
									<p className="text-muted-foreground text-xs">Set up installment plans</p>
								</div>
								<div className="text-right">
									<button className="rounded border px-3 py-1 text-sm">Manage</button>
								</div>
							</div>
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									<p className="font-medium text-sm">Collection Agency</p>
									<p className="text-muted-foreground text-xs">Send to collections</p>
								</div>
								<div className="text-right">
									<button className="rounded border px-3 py-1 text-sm">Review</button>
								</div>
							</div>
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									<p className="font-medium text-sm">Payment Portal</p>
									<p className="text-muted-foreground text-xs">Customer self-service</p>
								</div>
								<div className="text-right">
