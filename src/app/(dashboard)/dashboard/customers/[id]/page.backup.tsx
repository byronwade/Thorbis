				...
				<div className="flex items-center gap-4">
					...
					<div>
						<div className="flex items-center gap-2">
							<h1 className="font-bold text-3xl tracking-tight">{customer.display_name}</h1>
							<Badge className="capitalize" variant="outline">
								{customer.type}
							</Badge>
						</div>
						{customer.company_name ? <p className="text-muted-foreground">{customer.company_name}</p> : null}
					</div>
				</div>
				<div className="flex items-center gap-2">
			...
			<div className="grid gap-6 md:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						...
						<p className="text-muted-foreground text-xs">{activeJobs} currently active</p>
					</CardContent>
				</Card>
				...
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="font-medium text-sm">Customer Since</CardTitle>
						<Calendar className="size-4 text-muted-foreground" />
					</CardHeader>
					<CardContent>
						<div className="font-bold text-2xl">{new Date(customer.created_at).getFullYear()}</div>
						<p className="text-muted-foreground text-xs">{formatDate(new Date(customer.created_at))}</p>
					</CardContent>
				</Card>
			</div>
				...
				<div className="space-y-6 lg:col-span-2">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>Complete contact and business details</CardDescription>
						</CardHeader>
						<CardContent className="space-y-6">
							...
							<div>
								...
								<div className="grid gap-4 md:grid-cols-2">
									...
									<div>
										...
										<Link className="font-medium hover:underline" href={`mailto:${customer.email}`}>
											{customer.email}
										</Link>
									</div>
									<div>
										<div className="text-muted-foreground text-sm">Phone</div>
										<Link className="font-medium hover:underline" href={`tel:${customer.phone}`}>
											{customer.phone}
										</Link>
									</div>
							...
							{customer.notes ? (
								<>
									<Separator />
									<div>
										<div className="mb-2 text-muted-foreground text-sm">Customer Notes</div>
										<div className="rounded-md bg-muted p-3 text-sm">{customer.notes || "No notes available"}</div>
									</div>
								</>
							) : null}
							...
							<div className="grid gap-4 md:grid-cols-2">
								<div>
									...
									<div className="font-medium">{formatDate(new Date(customer.created_at))}</div>
								</div>
								<div>
									<div className="text-muted-foreground text-sm">Last Updated</div>
									<div className="font-medium">{formatDate(new Date(customer.updated_at))}</div>
								</div>
							</div>
						</CardContent>
					</Card>
					...
					{customer.is_business && <CustomerContactsManager customerId={customerId} initialContacts={contacts || []} />}

					{/* Customer Addresses */}
					<CustomerAddressesManager customerId={customerId} initialAddresses={addresses || []} />
						...
						<Card>
							<CardHeader>
								<div className="flex items-center justify-between">
									...
									<Badge variant="secondary">{paymentPlans.length} active</Badge>
								</div>
								<CardDescription>Active payment plans and schedules</CardDescription>
							</CardHeader>
										...
										<div className="flex items-start justify-between">
											<div>
												...
												<p className="text-muted-foreground text-sm">
													{formatCurrency(plan.total_amount)} • {plan.number_of_payments} {plan.payment_frequency}{" "}
													payments
												</p>
												<p className="mt-1 text-xs">
													<span className="text-muted-foreground">Next Due:</span> {plan.next_payment_due_date || "N/A"}
												</p>
											</div>
					...
					<CustomerDataTables customerId={customerId} invoices={invoices || []} jobs={jobs || []} />

					{/* Properties Section */}
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							<div>
								<h2 className="font-bold text-2xl tracking-tight">Properties ({properties?.length || 0})</h2>
								<p className="text-muted-foreground text-sm">Service locations for this customer</p>
							</div>
							<Button asChild size="sm" variant="outline">
								<Link href={`/dashboard/work/properties/new?customer=${customerId}`}>
								...
								<CardContent className="space-y-4">
									...
									{property.notes ? (
										<>
											<Separator />
											<div>
												<div className="mb-2 text-muted-foreground text-sm">Property Notes</div>
												<div className="rounded-md bg-muted p-3 text-sm">{property.notes}</div>
											</div>
										</>
									) : null}
									...
									<div className="flex gap-2">
										<Button asChild size="sm" variant="outline">
											<Link href={`/dashboard/work/properties/${property.id}`}>View Details</Link>
										</Button>
										<Button asChild size="sm" variant="outline">
											<Link href={`/dashboard/work/new?property=${property.id}`}>Create Job</Link>
										</Button>
									</div>
								</CardContent>
							</Card>
					...
					<Card>
						...
						<CardContent className="space-y-4">
							...
							<div className="flex items-center justify-between">
								...
								<span className="font-medium text-sm">{formatDate(new Date(customer.created_at))}</span>
							</div>
						</CardContent>
					</Card>
				</div>
