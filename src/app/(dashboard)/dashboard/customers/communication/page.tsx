/**
 * Customers > Communication Page - Server Component
 *
...
export default function CommunicationPage() {
	return (
		<div className="space-y-6">
			<div>
				<h1 className="font-bold text-3xl tracking-tight">Communication History</h1>
				<p className="text-muted-foreground">Track all customer communications, calls, emails, and messages</p>
			</div>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				<Card>
					<CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
						<CardTitle className="font-medium text-sm">Total Communications</CardTitle>
						<svg
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
				<Card className="col-span-4">
					<CardHeader>
						...
						<CardDescription>Latest customer interactions and messages</CardDescription>
					</CardHeader>
					<CardContent>
								...
								<div className="flex items-center gap-4 rounded-lg border p-4" key={index}>
									<div className="flex size-12 items-center justify-center rounded-full bg-accent">
										<span className="font-medium text-sm">
											{comm.type === "Phone Call" ? "📞" : comm.type === "Email" ? "📧" : "💬"}
										</span>
									</div>
									<div className="flex-1 space-y-1">
										<div className="flex items-center gap-2">
											<p className="font-medium text-sm leading-none">{comm.customer}</p>
											<span
										...
										<p className="text-muted-foreground text-sm">{comm.subject}</p>
										<p className="text-muted-foreground text-sm">{comm.notes}</p>
									</div>
									<div className="text-right">
										<p className="font-medium text-sm">{comm.timestamp}</p>
										<p className="text-muted-foreground text-xs">{comm.duration}</p>
									</div>
								</div>
							))}
						</div>
						...
						<div className="space-y-4">
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									...
									<p className="text-muted-foreground text-xs">ABC Corporation</p>
								</div>
								<div className="text-right">
							...
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									...
									<p className="text-muted-foreground text-xs">Customer preference</p>
								</div>
								<div className="text-right">
									<p className="font-bold text-sm">Phone</p>
									<p className="text-muted-foreground text-xs">45% of communications</p>
								</div>
							</div>
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									<p className="font-medium text-sm">Response Rate</p>
									<p className="text-muted-foreground text-xs">Customer responses</p>
								</div>
								<div className="text-right">
							...
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									...
									<p className="text-muted-foreground text-xs">Communication quality</p>
								</div>
								<div className="text-right">
							...
							<div className="flex items-center justify-between rounded-lg bg-accent p-3">
								<div>
									...
									<p className="text-muted-foreground text-xs">Required follow-ups</p>
								</div>
								<div className="text-right">
