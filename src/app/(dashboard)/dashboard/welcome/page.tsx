...
import { Search, CheckCircle2 as CheckCircle2Icon } from "lucide-react";
import {
	AlertDialog,
	AlertDialogAction,
	AlertDialogCancel,
...
const PRICING = {
	...
	usageBased: {
		teamMembers: { price: 5, unit: "per user/month", description: "Active team members" },
		phoneNumbers: { price: 2, unit: "per number/month", description: "Business phone numbers (local or toll-free)" },
		storage: { price: 0.50, unit: "per GB/month", description: "File storage" },
		invoices: { price: 0.15, unit: "per invoice", description: "Customer invoices sent" },
		estimates: { price: 0.10, unit: "per quote", description: "Price quotes/estimates" },
		sms: { price: 0.02, unit: "per text", description: "SMS messages sent" },
		emails: { price: 0.005, unit: "per email", description: "Transactional emails" },
		phoneCalls: { price: 0.02, unit: "per minute", description: "Phone call minutes" },
		videoCalls: { price: 0.05, unit: "per minute", description: "Video call minutes" },
		payments: { price: 0.29, unit: "per payment", description: "Payments collected (+ 2.9%)" },
	},
};
...
export default function WelcomePage() {
	...
	const [teamMembers, setTeamMembers] = useState<Array<{email: string; firstName: string; lastName: string; role?: string}>>([]);
	const [companyId, setCompanyId] = useState<string | null>(null);
	const [userInfo, setUserInfo] = useState<{email: string; firstName: string; lastName: string; phone?: string} | null>(null);
	...
	const [portingValidationErrors, setPortingValidationErrors] = useState<Record<string, string>>({});
	...
	const phoneProviders = [
		...
		{ 
			...
			accountHelpUrl: "https://help.dialpad.com/en/articles/porting-your-number"
		},
		{ 
			name: "RingCentral", 
			value: "ringcentral",
			pinHelpUrl: "https://support.ringcentral.com/article/port-number.html",
			accountHelpUrl: "https://support.ringcentral.com/article/port-number.html"
		},
		{ 
			name: "Other", 
			value: "other",
	...
	const selectedProvider = phoneProviders.find(p => p.value === portingData.currentCarrier.toLowerCase());
	const isOtherCarrier = !phoneProviders.some(p => p.value === portingData.currentCarrier.toLowerCase()) && portingData.currentCarrier !== "";
	...
	const handleArchiveCompany = async () => {
		...
		try {
			...
			if (result.success) {
				toast.success("Company archived. You can restore it later from Settings > Archive.");
		...
		if (currentStep === 1) {
			...
			if (isValid) {
				...
				try {
					...
					const result = await saveOnboardingProgress(formData, companyId || undefined);
					if (result.success && result.companyId) {
						setCompanyId(result.companyId);
						// Only show success toast if this is a new company (not updating)
						if (!companyId) {
							toast.success("Company information saved! You can continue later from your company dropdown.");
						}
						...
						if (result.error?.includes("Company not found") || result.error?.includes("archived")) {
							setCompanyId(null);
							setCurrentStep(1);
			...
			try {
				const result = await saveOnboardingStepProgress(companyId, 2, {
					teamMembers: teamMembers.filter(m => m.email || m.firstName || m.lastName),
				});
				if (result.success) {
					setCurrentStep(3);
				} else {
			...
			try {
				...
				const result = await saveOnboardingStepProgress(companyId, 3, phoneData);
				if (result.success) {
					setCurrentStep(4);
				} else {
			...
			try {
				...
				const result = await saveOnboardingStepProgress(companyId, 4, notificationData);
				if (result.success) {
					setCurrentStep(5);
				} else {
	...
	const addTeamMember = () => {
		setTeamMembers([...teamMembers, { email: "", firstName: "", lastName: "", role: "" }]);
	};
	...
	const handlePayment = async () => {
		if (!companyId) {
			toast.error("Company not created. Please go back and complete previous steps.");
			return;
		}
		...
		try {
			...
			const result = await createOrganizationCheckoutSession(
				...
				phoneNumberOption === "purchase" ? selectedNumber || undefined : undefined
			);
							...
							{currentStep === 1 && (
								<div className="space-y-6">
									...
									<div className="space-y-2">
										...
										<div className="flex items-center gap-4">
											...
											<div className="flex-1">
												...
												/>
												<FormDescription>
													Upload your company logo (max 5MB). This will appear on invoices and customer communications.
												</FormDescription>
											</div>
										</div>
									</div>
									...
									<FormField
										...
										name="orgPhone"
										render={({ field }) => {
											const isAutoFilled = userInfo?.phone && field.value === userInfo.phone;
											return (
												<FormItem>
									...
									<div className="space-y-2">
										...
										<SmartAddressInput
											key={companyId || "new"} // Force re-render when companyId changes
											label=""
											required
										...
										/>
										<FormDescription>
											Start typing to search for your address, or enter manually
										</FormDescription>
							...
							{currentStep === 3 && (
								<div className="space-y-6">
									<div className="flex items-center gap-3">
										...
										<div>
											<h2 className="font-semibold text-xl">Phone Number Setup <span className="text-muted-foreground font-normal text-base">(Optional)</span></h2>
											<p className="text-muted-foreground text-sm">
												Set up a phone number for your business communications. You can configure this later in settings.
											</p>
										</div>
									</div>
									...
									{!phoneNumberOption ? (
										<>
											<Alert>
												...
												<AlertDescription>
													Choose to purchase a new number for $2/month or port your existing business number. 
													This step is optional and can be completed later in Settings → Communications.
												</AlertDescription>
											</Alert>

											<div className="space-y-4">
												<Card className="cursor-pointer transition-all hover:border-primary hover:shadow-md" onClick={() => setPhoneNumberOption("purchase")}>
													<CardContent className="flex items-center gap-4 p-6">
														<div className="flex size-12 items-center justify-center rounded-lg bg-primary/10">
															<Phone className="size-6 text-primary" />
														</div>
														<div className="flex-1">
															<CardTitle className="mb-1 text-lg">Purchase New Number</CardTitle>
															<CardDescription>
																Get a new local or toll-free number for $2/month. Includes voice and SMS capabilities.
															</CardDescription>
														</div>
												...
												<Card className="cursor-pointer transition-all hover:border-primary hover:shadow-md" onClick={() => setPhoneNumberOption("port")}>
													<CardContent className="flex items-center gap-4 p-6">
														<div className="flex size-12 items-center justify-center rounded-lg bg-primary/10">
															<Upload className="size-6 text-primary" />
														</div>
														<div className="flex-1">
															<CardTitle className="mb-1 text-lg">Port Existing Number</CardTitle>
															<CardDescription>
																Transfer your current business number. Takes 7-10 business days. No porting fees.
															</CardDescription>
														</div>
														<ArrowRight className="size-5 text-muted-foreground" />
													</CardContent>
												...
												<Button
													...
													onClick={async () => {
														...
														if (companyId) {
															try {
																await saveOnboardingStepProgress(companyId, 3, {
																	option: "skip",
																});
															} catch (err) {
																console.error("Failed to save skip choice:", err);
															}
														}
														setCurrentStep(4);
													}}
										...
										<div className="space-y-6">
											<div>
												<h3 className="font-semibold text-lg">Purchase New Number</h3>
												<p className="text-muted-foreground text-sm">Select a phone number for your business</p>
											</div>
											...
											<div className="grid gap-4 sm:grid-cols-2">
												...
												<div className="space-y-2">
													...
													<Select value={numberType} onValueChange={(v: "local" | "toll-free") => setNumberType(v)}>
														<SelectTrigger>
															<SelectValue />
														</SelectTrigger>
														<SelectContent>
													...
													try {
														...
														if (result.success && result.data) {
															...
															if (result.data.length === 0) {
																setPurchaseError("No numbers found. Try a different area code or number type.");
															}
														} else {
															setPurchaseError(result.error || "Failed to search numbers. Please try again.");
															setAvailableNumbers([]);
														}
													} catch (error) {
														setPurchaseError("Failed to search numbers. Please try again.");
														setAvailableNumbers([]);
													} finally {
											...
											{availableNumbers.length > 0 && (
												<div className="space-y-3">
													<Label className="text-base font-semibold">Available Numbers</Label>
															...
															>
																<CardContent className="flex items-center justify-between p-4">
																	<div className="flex items-center gap-3">
																		...
																		<div>
																			<div className="font-semibold text-lg">{num.phone_number}</div>
																			<div className="text-muted-foreground text-sm">
																				${num.monthly_cost || "2.00"}/month • Includes voice & SMS
																			</div>
																		</div>
																	</div>
																</CardContent>
											...
											{selectedNumber && (
												<Alert className="border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950/20">
													...
													<AlertDescription className="text-green-800 dark:text-green-200">
														<strong>Number selected:</strong> {selectedNumber} will be purchased automatically when you complete payment.
													</AlertDescription>
												</Alert>
											)}
										</div>
										...
										<div className="space-y-6">
											{portingSuccess ? (
												<div className="space-y-6 text-center">
													...
													<div>
														...
														<p className="text-muted-foreground">
															Your request to port <strong>{portingData.phoneNumber}</strong> has been successfully submitted.
														</p>
													</div>
													<Alert className="border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950/20">
														<Info className="size-4 text-blue-600 dark:text-blue-400" />
														<AlertDescription className="text-blue-800 dark:text-blue-200">
															<strong>What happens next:</strong> We'll process your request and coordinate with your current carrier. 
															Porting typically takes 7-10 business days. You'll receive email updates at {portingData.authorizedEmail}.
														</AlertDescription>
													</Alert>
												</div>
											) : (
												<>
													<div>
														<h3 className="font-semibold text-lg">Port Existing Number</h3>
														<p className="text-muted-foreground text-sm">Transfer your current business number to Thorbis</p>
													</div>

													<Alert>
														<Info className="size-4" />
														<AlertDescription>
															<strong>Important:</strong> Keep your current service active until the port is complete (7-10 business days). 
															All information must match your current carrier's records exactly, or the port may be rejected.
														</AlertDescription>
													</Alert>
													...
													{portingError && (
														<Alert variant="destructive">
															<AlertCircle className="size-4" />
															<AlertDescription>{portingError}</AlertDescription>
														</Alert>
													)}
													...
													<div className="space-y-6">
														<Card>
															<CardHeader>
																<CardTitle className="text-base">Phone Number & Carrier Information</CardTitle>
																<CardDescription>Details about the number you want to port</CardDescription>
															</CardHeader>
															<CardContent className="space-y-4">
																<div className="space-y-2">
																	<Label htmlFor="porting-phone">Phone Number to Port *</Label>
																	<Input
																		id="porting-phone"
																		placeholder="(831) 430-6011"
																		value={portingData.phoneNumber}
																		onChange={(e) => {
																			setPortingData({ ...portingData, phoneNumber: e.target.value });
																			if (portingValidationErrors.phoneNumber) {
																				setPortingValidationErrors({ ...portingValidationErrors, phoneNumber: "" });
																			}
																		}}
																		className={portingValidationErrors.phoneNumber ? "border-destructive" : ""}
																	/>
																	{portingValidationErrors.phoneNumber && (
																		<p className="text-destructive text-sm">{portingValidationErrors.phoneNumber}</p>
																	)}
																...
																<div className="space-y-2">
																	<Label htmlFor="porting-carrier">Current Carrier/Provider *</Label>
																	<Select
																		value={phoneProviders.some(p => p.value === portingData.currentCarrier.toLowerCase()) ? portingData.currentCarrier.toLowerCase() : isOtherCarrier ? "other" : ""}
																		onValueChange={(value) => {
																			if (value === "other") {
																				setPortingData({ ...portingData, currentCarrier: otherCarrierName });
																			} else {
																				setPortingData({ ...portingData, currentCarrier: value });
																				setOtherCarrierName("");
																			}
																			if (portingValidationErrors.currentCarrier) {
																				setPortingValidationErrors({ ...portingValidationErrors, currentCarrier: "" });
																			}
																		}}
																	>
																		<SelectTrigger
																			id="porting-carrier"
																			className={portingValidationErrors.currentCarrier ? "border-destructive" : ""}
																		>
																	...
																	{portingValidationErrors.currentCarrier && (
																		<p className="text-destructive text-sm">{portingValidationErrors.currentCarrier}</p>
																	)}
																	{selectedProvider && selectedProvider.value !== "other" && (
																		<div className="flex flex-wrap gap-2 text-xs">
																			{selectedProvider.pinHelpUrl && (
																				<a
																					href={selectedProvider.pinHelpUrl}
																					target="_blank"
																			...
																			{selectedProvider.accountHelpUrl && (
																				<>
																					...
																					<a
																						href={selectedProvider.accountHelpUrl}
																						target="_blank"
																						rel="noopener noreferrer"
																						className="flex items-center gap-1 text-primary hover:underline"
																					>
																	...
																	{isOtherCarrier && (
																		<Input
																			...
																			onChange={(e) => {
																				setPortingData({ ...portingData, currentCarrier: e.target.value });
																	...
																	<p className="text-muted-foreground text-xs">
																		Select your current phone service provider
																	</p>
																</div>

																<div className="grid gap-4 sm:grid-cols-2">
																	<div className="space-y-2">
																		<Label htmlFor="porting-account">Account Number *</Label>
																		<Input
																			id="porting-account"
																			placeholder="Usually 10-15 digits"
																			value={portingData.accountNumber}
																			onChange={(e) => {
																				setPortingData({ ...portingData, accountNumber: e.target.value });
																				if (portingValidationErrors.accountNumber) {
																					setPortingValidationErrors({ ...portingValidationErrors, accountNumber: "" });
																				}
																			}}
																			className={portingValidationErrors.accountNumber ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.accountNumber && (
																			<p className="text-destructive text-sm">{portingValidationErrors.accountNumber}</p>
																		)}
																		<p className="text-muted-foreground text-xs">
																	...
																	<div className="space-y-2">
																		<Label htmlFor="porting-pin">Account PIN/Password *</Label>
																		<Input
																			id="porting-pin"
																			type="password"
																			...
																			onChange={(e) => {
																				setPortingData({ ...portingData, accountPin: e.target.value });
																				if (portingValidationErrors.accountPin) {
																					setPortingValidationErrors({ ...portingValidationErrors, accountPin: "" });
																				}
																			}}
																			className={portingValidationErrors.accountPin ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.accountPin && (
																			<p className="text-destructive text-sm">{portingValidationErrors.accountPin}</p>
																		)}
																		<p className="text-muted-foreground text-xs">
																			Set when you opened the account - call carrier if unknown
																		</p>
																	</div>
																</div>
															</CardContent>
														...
														<Card>
															<CardHeader>
																...
																<CardDescription>Must match billing address on file with your carrier exactly</CardDescription>
															</CardHeader>
															<CardContent className="space-y-4">
																<Alert variant="default" className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20">
																	...
																	<AlertDescription className="text-amber-800 dark:text-amber-200 text-sm">
																		<strong>Critical:</strong> Address must match exactly as shown on your bill. 
																		Even small differences (like "St" vs "Street") will cause rejection.
																	</AlertDescription>
																</Alert>

																<div className="space-y-2">
																	<Label htmlFor="porting-address">Street Address *</Label>
																	<Input
																		id="porting-address"
																		placeholder="123 Main Street"
																		value={portingData.addressLine1}
																		onChange={(e) => {
																			setPortingData({ ...portingData, addressLine1: e.target.value });
																			if (portingValidationErrors.addressLine1) {
																				setPortingValidationErrors({ ...portingValidationErrors, addressLine1: "" });
																			}
																		}}
																		className={portingValidationErrors.addressLine1 ? "border-destructive" : ""}
																	/>
																	{portingValidationErrors.addressLine1 && (
																		<p className="text-destructive text-sm">{portingValidationErrors.addressLine1}</p>
																	)}
																	<p className="text-muted-foreground text-xs">
																...
																<div className="grid gap-4 sm:grid-cols-3">
																	<div className="space-y-2">
																		<Label htmlFor="porting-city">City *</Label>
																		<Input
																			id="porting-city"
																			placeholder="Monterey"
																			value={portingData.city}
																			onChange={(e) => {
																				setPortingData({ ...portingData, city: e.target.value });
																				if (portingValidationErrors.city) {
																					setPortingValidationErrors({ ...portingValidationErrors, city: "" });
																				}
																			}}
																			className={portingValidationErrors.city ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.city && (
																			<p className="text-destructive text-sm">{portingValidationErrors.city}</p>
																		)}
																	</div>
																	<div className="space-y-2">
																		<Label htmlFor="porting-state">State *</Label>
																		<Input
																			id="porting-state"
																			maxLength={2}
																			placeholder="CA"
																			...
																			onChange={(e) => {
																				setPortingData({ ...portingData, state: e.target.value.toUpperCase() });
																				if (portingValidationErrors.state) {
																					setPortingValidationErrors({ ...portingValidationErrors, state: "" });
																				}
																			}}
																			className={portingValidationErrors.state ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.state && (
																			<p className="text-destructive text-sm">{portingValidationErrors.state}</p>
																		)}
																	</div>
																	<div className="space-y-2">
																		<Label htmlFor="porting-zip">ZIP Code *</Label>
																		<Input
																			id="porting-zip"
																			maxLength={10}
																			placeholder="93940"
																			...
																			onChange={(e) => {
																				setPortingData({ ...portingData, zipCode: e.target.value });
																				if (portingValidationErrors.zipCode) {
																					setPortingValidationErrors({ ...portingValidationErrors, zipCode: "" });
																				}
																			}}
																			className={portingValidationErrors.zipCode ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.zipCode && (
																			<p className="text-destructive text-sm">{portingValidationErrors.zipCode}</p>
																		)}
																	</div>
																</div>
															</CardContent>
														...
														<Card>
															<CardHeader>
																<CardTitle className="text-base">Authorized Person</CardTitle>
																<CardDescription>Account holder information for authorization</CardDescription>
															</CardHeader>
															<CardContent className="space-y-4">
																<div className="grid gap-4 sm:grid-cols-2">
																	<div className="space-y-2">
																		<Label htmlFor="porting-person">Full Name of Account Holder *</Label>
																		<Input
																			id="porting-person"
																			placeholder="John Smith"
																			value={portingData.authorizedPerson}
																			onChange={(e) => {
																				setPortingData({ ...portingData, authorizedPerson: e.target.value });
																				if (portingValidationErrors.authorizedPerson) {
																					setPortingValidationErrors({ ...portingValidationErrors, authorizedPerson: "" });
																				}
																			}}
																			className={portingValidationErrors.authorizedPerson ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.authorizedPerson && (
																			<p className="text-destructive text-sm">{portingValidationErrors.authorizedPerson}</p>
																		)}
																		<p className="text-muted-foreground text-xs">
																	...
																	<div className="space-y-2">
																		<Label htmlFor="porting-email">Email Address *</Label>
																		<Input
																			id="porting-email"
																			type="email"
																			...
																			onChange={(e) => {
																				setPortingData({ ...portingData, authorizedEmail: e.target.value });
																				if (portingValidationErrors.authorizedEmail) {
																					setPortingValidationErrors({ ...portingValidationErrors, authorizedEmail: "" });
																				}
																			}}
																			className={portingValidationErrors.authorizedEmail ? "border-destructive" : ""}
																		/>
																		{portingValidationErrors.authorizedEmail && (
																			<p className="text-destructive text-sm">{portingValidationErrors.authorizedEmail}</p>
																		)}
																		<p className="text-muted-foreground text-xs">
														...
														<Alert>
															<Info className="size-4" />
															<AlertDescription>
																<strong>Timeline:</strong> Porting typically takes 7-10 business days. 
																You'll receive email updates at each stage. Keep your current service active until the port is complete.
															</AlertDescription>
														</Alert>
																	...
																	<Accordion type="single" collapsible className="w-full">
																		<AccordionItem value="faq-1">
																			...
																			<AccordionContent className="text-sm text-muted-foreground">
																				Porting typically takes 7-10 business days. The exact timeline depends on your current carrier's processing time. 
																		...
																		<AccordionItem value="faq-2">
																			...
																			<AccordionContent className="text-sm text-muted-foreground">
																				If you selected your carrier above, use the "Find Account PIN" link. Otherwise, contact your current carrier's 
																				customer service. They can help you retrieve or reset your PIN. This is required for porting authorization.
																			</AccordionContent>
																		</AccordionItem>
																		<AccordionItem value="faq-3">
																			...
																			<AccordionContent className="text-sm text-muted-foreground">
																				Carriers verify porting requests by matching information exactly as it appears on your account. 
																				Even small differences (like "St" vs "Street") can cause rejection. Check your most recent bill for the exact format.
																			</AccordionContent>
																		</AccordionItem>
																		<AccordionItem value="faq-4">
																			<AccordionTrigger className="text-sm py-2">
																				Can I cancel my current service before porting?
																			</AccordionTrigger>
																			<AccordionContent className="text-sm text-muted-foreground">
																				No. You must keep your current service active until the port is complete. Canceling before the port completes 
																				will cause the port to fail and you may lose your number.
																			</AccordionContent>
																		</AccordionItem>
																		<AccordionItem value="faq-5">
																			...
																			<AccordionContent className="text-sm text-muted-foreground">
																				If your port is rejected, we'll notify you via email with the reason. Common reasons include incorrect account information 
																				or address mismatch. You can correct the information and resubmit the request.
																			</AccordionContent>
																		</AccordionItem>
																	</Accordion>
																</div>
									...
									{!phoneNumberOption && (
										<Alert className="border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/20">
											...
											<AlertDescription className="text-amber-800 dark:text-amber-200">
												<strong className="font-semibold">Phone Number Not Configured</strong>
												<p className="mt-2 text-sm">
													You've chosen to continue without setting up a phone number. Please note that many features will be disabled or limited without phone number capabilities:
												</p>
												...
												<p className="mt-2 text-sm">
													You can add a phone number later in <strong>Settings → Communications</strong>, but we recommend setting one up now to unlock the full functionality of Thorbis.
												</p>
											</AlertDescription>
										</Alert>
									)}
											...
											<CardContent className="space-y-4">
												<div className="flex items-center justify-between">
													<div>
														...
														<p className="text-muted-foreground text-sm">Get notified when new jobs are created</p>
													</div>
												...
												<div className="flex items-center justify-between">
													<div>
														...
														<p className="text-muted-foreground text-sm">Notifications when jobs are updated</p>
													</div>
													<Switch defaultChecked />
												</div>
												<div className="flex items-center justify-between">
													<div>
														...
														<p className="text-muted-foreground text-sm">When customers make payments</p>
													</div>
													<Switch defaultChecked />
												</div>
											</CardContent>
											...
											<CardContent className="space-y-4">
												<div className="flex items-center justify-between">
													<div>
														...
														<p className="text-muted-foreground text-sm">SMS alerts for urgent jobs</p>
													</div>
													<Switch />
												</div>
												<div className="flex items-center justify-between">
													<div>
														...
														<p className="text-muted-foreground text-sm">When appointments are rescheduled</p>
													</div>
													<Switch />
												</div>
											</CardContent>
							...
							{currentStep === 5 && (
								<div className="space-y-6">
									<div className="flex items-center gap-3">
										...
										<div>
											<h2 className="font-semibold text-xl">Payment Required</h2>
											<p className="text-muted-foreground text-sm">
									...
									<Alert>
										...
										<AlertDescription>
											<strong>Payment Required:</strong> You must complete payment before
											accessing the dashboard. Subscription includes base fee + usage-based charges.
										</AlertDescription>
									</Alert>
									...
									<Card className="border-2 border-primary/20 bg-primary/5">
										<CardHeader>
											<div className="flex items-center justify-between">
												<div>
													<CardTitle className="text-2xl">Base Subscription</CardTitle>
													<CardDescription>Platform access and core features</CardDescription>
												</div>
									...
									<Card>
										<CardHeader>
											...
											<CardDescription>
												Pay only for what you use. Charges are billed monthly based on actual usage.
											</CardDescription>
										</CardHeader>
										<CardContent>
											<div className="space-y-4">
												...
												<Alert className="mt-4">
													...
													<AlertDescription className="text-sm">
														<strong>Example Monthly Bill:</strong> Base ($100) + 1 Phone Number ($2) + 3 Team Members ($15) + 50 Invoices ($7.50) = ~$124.50/month
													</AlertDescription>
												</Alert>
											</div>
										</CardContent>
										...
										{currentStep === 2 ? (
											...
											<Button
												...
												onClick={async () => {
													...
													if (phoneNumberOption === "port" && !portingSuccess && portingData.phoneNumber) {
														// Validate all fields
														const errors: Record<string, string> = {};
														
														if (!portingData.phoneNumber.trim()) {
															errors.phoneNumber = "Phone number is required";
														} else if (portingData.phoneNumber.replace(/\D/g, "").length < 10) {
															errors.phoneNumber = "Please enter a valid phone number";
														}
														
														if (!portingData.currentCarrier.trim()) {
															errors.currentCarrier = "Current carrier is required";
														}
														...
														if (!portingData.authorizedEmail.trim()) {
															errors.authorizedEmail = "Email address is required";
														} else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(portingData.authorizedEmail)) {
															errors.authorizedEmail = "Please enter a valid email address";
														}

														if (Object.keys(errors).length > 0) {
															setPortingValidationErrors(errors);
															setPortingError("Please correct the errors below");
															return;
														}
														...
														try {
															const formData = new FormData();
															Object.entries(portingData).forEach(([key, value]) => {
															...
															if (result.success) {
																...
																if (companyId) {
																	try {
																		...
																		console.error("Failed to save porting progress:", saveErr);
																	}
																}
																setPortingSuccess(true);
																toast.success("Porting request submitted successfully!");
																// Don't advance yet - let user see success message, they can click Continue again
																return;
															} else {
																setPortingError(result.error || "Failed to submit porting request. Please try again.");
																setPortingNumber(false);
																return;
															}
														} catch (error) {
															setPortingError("An unexpected error occurred. Please try again.");
															setPortingNumber(false);
															return;
														}
													}
													// If porting was successful, or for purchase/skip, proceed with normal flow
													if (portingSuccess || phoneNumberOption !== "port") {
														handleNext();
													}
												}}
			...
			<AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Are you sure you want to archive this company? The company will be hidden from your
							company list, but you can restore it later from Settings &gt; Archive. Archived companies
