...
export default function AIPage() {
	...
	const { createChat, setActiveChat, addMessage, updateChatTitle } = useChatStore();
		...
		onFinish: (message) => {
			...
			if (activeChat) {
				addMessage(activeChat.id, {
					...
					role: message.message.role === "system" ? "assistant" : message.message.role,
				...
				if (activeChat.title === "New Chat" && messages.length > 0) {
					...
					if (firstUserMessage) {
						const title =
							`${firstUserMessage.parts.find((part) => part.type === "text")?.text.slice(0, 50)}...` || "New Chat";
