...
function getTimesheetBadgeVariant(status: string): "default" | "secondary" | "destructive" {
	if (status === "approved") {
		return "default";
	}
	if (status === "rejected") {
								...
								<TableRow key={timesheet.id}>
									...
									<TableCell>
										{timesheet.overtimeHours > 0 ? (
											<span className="text-warning">{timesheet.overtimeHours}h</span>
										) : (
									...
									<TableCell>
										<Badge variant={getTimesheetBadgeVariant(timesheet.status)}>{timesheet.status}</Badge>
									</TableCell>
