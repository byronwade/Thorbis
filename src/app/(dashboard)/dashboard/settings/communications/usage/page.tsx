...
const UsageTrendsChart = dynamic(
  () => import("@/components/telnyx/usage-trends-chart").then((mod) => mod.UsageTrendsChart),
  {
...
export default async function UsageBillingPage() {
	...
	// Get call usage
	const { data: calls } = await supabase.from("communications").select("*").eq("company_id", companyId).eq("type", "phone").gte("created_at", startOfMonth).lte("created_at", endOfMonth);

	// Get SMS usage
	const { data: messages } = await supabase.from("communications").select("*").eq("company_id", companyId).eq("type", "sms").gte("created_at", startOfMonth).lte("created_at", endOfMonth);

	// Get voicemail usage
	const { data: voicemails } = await supabase.from("voicemails").select("*").eq("company_id", companyId).gte("received_at", startOfMonth).lte("received_at", endOfMonth);

	// Get phone numbers
	const { data: phoneNumbers } = await supabase.from("phone_numbers").select("*").eq("company_id", companyId).is("deleted_at", null);
	...
	const smsSent = messages?.filter((m) => m.direction === "outbound").length || 0;
	const smsReceived = messages?.filter((m) => m.direction === "inbound").length || 0;
	const voicemailCount = voicemails?.length || 0;
	const voicemailMinutes = voicemails?.reduce((sum, vm) => sum + (vm.duration_seconds ? Math.ceil(vm.duration_seconds / 60) : 0), 0) || 0;
	...
	const totalCost = callCost + smsCost + voicemailTranscriptionCost + phoneNumberCost;

	// Get company budget settings
	const { data: company } = await supabase.from("companies").select("telnyx_budget_limit, telnyx_budget_alert_threshold").eq("id", companyId).single();
		...
		<div className="space-y-6 p-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Track your Telnyx usage and costs</p>
				</div>
				<ExportUsageButton companyId={companyId} startDate={startOfMonth} endDate={endOfMonth} />
			</div>
			...
			{/* Usage Metrics Cards */}
			<UsageMetricsCards metrics={metrics} costs={costs} />
