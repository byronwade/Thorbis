...
export function AvailabilityClient({ initialSettings }: AvailabilityClientProps) {
	const { settings, isLoading, isPending, hasUnsavedChanges, updateSetting, saveSettings, reload } =
		useSettings<AvailabilitySettingsState>({
			...
			transformSave: (state) => {
				...
				formData.append("defaultAppointmentDurationMinutes", state.defaultAppointmentDurationMinutes.toString());
	...
	const updateDay = useCallback(
		(index: number, updates: Partial<DayAvailability>) => {
			const nextWeek = settings.week.map((day, idx) => (idx === index ? { ...day, ...updates } : day));
		...
		>
						...
						<div key={day.key}>
							<div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
								<div className="flex items-center gap-3">
									<Switch checked={day.enabled} onCheckedChange={(checked) => updateDay(index, { enabled: checked })} />
				...
				<CardContent className="grid gap-4 md:grid-cols-2">
					...
					<div>
						...
						<Input
							className="mt-2"
							min={0}
							onChange={(event) => updateSetting("maxBookingAdvanceDays", Number(event.target.value))}
							type="number"
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Duration and buffers used when no service-specific rule exists</CardDescription>
				</CardHeader>
				<CardContent className="grid gap-4 md:grid-cols-2">
					<div>
						...
						<Input
							className="mt-2"
							min={15}
							onChange={(event) => updateSetting("defaultAppointmentDurationMinutes", Number(event.target.value))}
							type="number"
				...
				<CardContent className="space-y-4">
					<div className="flex items-center justify-between rounded-lg border p-3">
						<div>
							...
							<p className="text-muted-foreground text-xs">Block out a window in the middle of each day.</p>
						</div>
						<Switch
						...
						<div className="flex flex-wrap gap-4">
							...
							<div>
								...
								<Input
									className="mt-2 w-32"
									min={15}
									onChange={(event) => updateSetting("lunchBreakDurationMinutes", Number(event.target.value))}
									type="number"
