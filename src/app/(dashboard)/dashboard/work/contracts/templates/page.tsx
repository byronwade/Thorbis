			...
			<div className="border-b bg-background">
				<div className="flex items-center gap-4 px-6 py-4">
					...
					<div className="flex-1">
						...
						<p className="text-muted-foreground text-sm">Create and manage reusable contract templates</p>
					</div>
				...
				<div className="mx-auto max-w-7xl space-y-6 p-6">
					...
					<div className="grid gap-4 md:grid-cols-3">
						...
						<Card>
							...
							<CardContent>
								<div className="font-bold text-2xl">{mockTemplates[0]?.name.split(" ")[0] || "None"}</div>
								<p className="text-muted-foreground text-xs">{mockTemplates[0]?.usageCount || 0} times used</p>
							</CardContent>
						</Card>
						<Card>
							<CardHeader className="pb-2">
							...
							<CardContent>
								<div className="font-bold text-2xl">{mockTemplates.reduce((sum, t) => sum + t.usageCount, 0)}</div>
								<p className="text-muted-foreground text-xs">Contracts created</p>
							</CardContent>
						</Card>
					</div>
							...
							<Card className="flex flex-col" key={template.id}>
								...
								<CardFooter className="flex gap-2">
									<Button asChild className="flex-1" size="sm" variant="outline">
										<Link href={`/dashboard/work/contracts/new?templateId=${template.id}`}>
											<Copy className="mr-2 size-4" />
											Use Template
										</Link>
									</Button>
									<Button asChild size="sm" variant="ghost">
										<Link href={`/dashboard/work/contracts/templates/${template.id}`}>Edit</Link>
									</Button>
								</CardFooter>
							</Card>
						))}
