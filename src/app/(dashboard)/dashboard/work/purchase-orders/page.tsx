...
export default async function PurchaseOrdersPage() {
  ...
  // Fetch all purchase orders including archived (filter in UI)
  ...
  // Filter to active purchase orders for stats calculations
  const activePurchaseOrders = purchaseOrders.filter(
    (po) => !(po.archived_at || po.deleted_at)
  );

  // Calculate stats from data (from active purchase orders only)
  ...
  const cancelled = activePurchaseOrders.filter(
  ...
  return (
    <>
      ...
      <WorkDataView
        ...
        table={
          <PurchaseOrdersTable itemsPerPage={50} orders={purchaseOrders} />
        }
      />
    </>
  );
