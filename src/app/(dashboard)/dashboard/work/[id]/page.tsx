...
export default async function JobDetailsPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const assignedUser = Array.isArray(job.assigned_user) ? job.assigned_user[0] : job.assigned_user;
	...
	const [
		...
		supabase
			.from("job_tasks")
			.select("*, assigned_user:users!assigned_to(id, name, avatar), completed_by_user:users!completed_by(id, name)")
			.eq("job_id", jobId)
		...
		supabase.from("attachments").select("*").eq("entity_type", "job").eq("entity_id", jobId),
	...
	const progressPayments =
		(payments || [])
			.filter((p) => p.payment_type === "progress" || p.payment_type === "milestone")
			.map((p) => ({
				id: p.id,
				amount: p.amount || 0,
	...
	const nextAppointment = (schedules || [])
		.filter((s) => new Date(s.scheduled_start) >= new Date() && s.status !== "cancelled")
	...
	const communicationFiltersWithoutProperty = [...baseCommunicationFilters, ...invoiceFilters];
	...
	if (propertyFilter && isMissingColumnError(jobCommunicationsError, "property_id")) {
		({ data: jobCommunications, error: jobCommunicationsError } = await buildJobCommunicationsQuery(
			communicationFiltersWithoutProperty
		));
	}
