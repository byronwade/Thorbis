...
export default async function EquipmentDetailsPage({ params }: { params: Promise<{ id: string }> }) {
	...
	const [
		...
		equipment.install_job_id
			? supabase
					.from("jobs")
					.select("id, job_number, title, status, completed_at, customer:customers!customer_id(id, display_name)")
      ...
      `)
			...
			.then(async (result) => {
				...
				const relatedJobIds = new Set(relatedJobs?.map((j: any) => j.job_id) || []);
				const filteredSchedules = result.data.filter((s: any) => s.job_id && relatedJobIds.has(s.job_id));
