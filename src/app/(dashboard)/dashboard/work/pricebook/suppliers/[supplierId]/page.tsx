...
export default async function SupplierDetailPage({ params }: PageProps) {
	...
	const supplier = supplierData[supplierId as keyof typeof supplierData] || supplierData.ferguson;
		...
		<div className="mx-auto max-w-7xl space-y-6 p-6">
			...
			<div className="grid gap-4 md:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<p className="font-semibold text-2xl">{supplier.itemsImported.toLocaleString()}</p>
					</CardContent>
				</Card>
				...
				<Card>
					...
					<CardContent>
						<p className="font-semibold text-2xl">{formatRelativeTime(supplier.lastSyncAt)}</p>
					</CardContent>
				</Card>
								...
								<div className="flex items-center justify-between py-2">
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="text-muted-foreground text-xs">{sync.itemsUpdated} items updated</p>
										</div>
									</div>
									<div className="text-right">
										<p className="font-medium text-sm">{formatDateTime(sync.date)}</p>
										<p className="text-muted-foreground text-xs">{formatRelativeTime(sync.date)}</p>
									</div>
								</div>
			...
			<div className="grid gap-4 md:grid-cols-2">
				<Card>
					...
					<CardContent className="space-y-3">
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">Connected Since</span>
							<span className="font-medium text-sm">{supplier.connectionDate.toLocaleDateString()}</span>
						</div>
						<Separator />
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">API Endpoint</span>
							<span className="font-medium text-sm">api.ferguson.com</span>
						</div>
						<Separator />
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">Sync Frequency</span>
				...
				<Card>
					...
					<CardContent className="space-y-3">
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">Update Pricing</span>
							<Badge className="text-xs" variant="secondary">
								Enabled
							</Badge>
						</div>
						<Separator />
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">Update Availability</span>
							<Badge className="text-xs" variant="secondary">
								Enabled
							</Badge>
						</div>
						<Separator />
						<div className="flex justify-between">
							<span className="text-muted-foreground text-sm">Import New Items</span>
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						<div>
							...
							<CardDescription>{supplier.itemsImported} items imported from this supplier</CardDescription>
						</div>
						<Button asChild size="sm" variant="outline">
							<Link href={`/dashboard/work/pricebook?supplier=${supplier.id}`}>View All Items</Link>
						</Button>
					</div>
				</CardHeader>
			</Card>
