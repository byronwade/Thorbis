...
export default function CommissionCalculator() {
	...
	const commission =
		calculationType === "flat" ? flatCommission : calculationType === "tiered" ? tieredCommission : techTotal;
			...
			<div className="space-y-4">
				<div className="flex items-center gap-3">
					...
					<div>
						...
						<p className="mt-1 text-muted-foreground">Calculate sales commissions and technician incentive pay</p>
					</div>
				</div>
			</div>
			...
			<Card className="border-primary/20 bg-gradient-to-br from-blue-500/5 to-transparent">
				...
				<CardContent className="space-y-2 text-sm">
					...
					<ul className="ml-4 list-disc space-y-1">
						...
						<li>
							<strong>Tiered:</strong> Higher rates as sales increase (motivates high performers)
						</li>
						<li>
							<strong>Technician:</strong> Base pay plus performance bonuses per job
						</li>
					</ul>
				</CardContent>
			</Card>
			...
			<div className="grid gap-8 lg:grid-cols-2">
						...
						<Card>
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-2">
									...
									<Input id="rate" onChange={(e) => setFlatRate(e.target.value)} type="number" value={flatRate} />
									<p className="text-muted-foreground text-xs">Typical range: 5-15% for sales, 2-5% for technicians</p>
								</div>
							</CardContent>
						</Card>
					)}
							...
							<CardContent className="space-y-4">
								...
								<div className="space-y-3 rounded-lg border p-3">
									...
									<div className="grid grid-cols-2 gap-2">
										<div className="space-y-1">
											...
											<Input onChange={(e) => setTier1Threshold(e.target.value)} type="number" value={tier1Threshold} />
										</div>
										<div className="space-y-1">
											<Label className="text-xs">Rate (%)</Label>
											<Input onChange={(e) => setTier1Rate(e.target.value)} type="number" value={tier1Rate} />
										</div>
									</div>
								</div>
								...
								<div className="space-y-3 rounded-lg border p-3">
									...
									<div className="grid grid-cols-2 gap-2">
										<div className="space-y-1">
											<Label className="text-xs">Up to ($)</Label>
											<Input onChange={(e) => setTier2Threshold(e.target.value)} type="number" value={tier2Threshold} />
										</div>
										<div className="space-y-1">
											<Label className="text-xs">Rate (%)</Label>
											<Input onChange={(e) => setTier2Rate(e.target.value)} type="number" value={tier2Rate} />
										</div>
									</div>
								</div>
								...
								<div className="space-y-3 rounded-lg border p-3">
									...
									<div className="grid grid-cols-2 gap-2">
										...
										<div className="space-y-1">
											<Label className="text-xs">Rate (%)</Label>
											<Input onChange={(e) => setTier3Rate(e.target.value)} type="number" value={tier3Rate} />
										</div>
									</div>
								</div>
							</CardContent>
						...
						<Card>
							...
							<CardContent className="space-y-4">
								<div className="space-y-2">
									...
									<Input id="base" onChange={(e) => setTechBasePay(e.target.value)} type="number" value={techBasePay} />
									<p className="text-muted-foreground text-xs">Assuming 40 hours/week, 4 weeks/month</p>
								</div>
								<div className="space-y-2">
								...
								<div className="space-y-2">
									...
									<Input id="ticket" onChange={(e) => setAvgTicket(e.target.value)} type="number" value={avgTicket} />
								</div>
								<div className="space-y-2">
									...
									<Input
										...
										value={bonusPerJob}
									/>
									<p className="text-muted-foreground text-xs">Flat bonus paid for each completed job</p>
								</div>
							</CardContent>
						</Card>
					)}
				...
				<div className="space-y-4">
					<Card className="border-primary/20 bg-gradient-to-br from-primary/10 to-transparent">
						<CardHeader>
							<CardTitle>{calculationType === "technician" ? "Total Compensation" : "Commission Earned"}</CardTitle>
						</CardHeader>
						<CardContent>
							<div className="space-y-4">
								<div>
									...
									<p className="font-bold text-5xl text-success">${commission.toFixed(2)}</p>
								</div>
								{calculationType !== "technician" && (
									<div className="grid grid-cols-2 gap-4">
										<div>
											...
											<p className="font-semibold text-lg">
												{calculationType === "flat" ? `${flatRate}%` : `${((commission / salesNum) * 100).toFixed(2)}%`}
											</p>
										</div>
										<div>
							...
							<CardContent className="space-y-3">
								<div className="space-y-2">
									<div className="flex justify-between text-sm">
										...
										<span className="font-semibold">${(Math.min(salesNum, tier1Num) * rate1).toFixed(2)}</span>
									</div>
									{salesNum > tier1Num && (
										<div className="flex justify-between text-sm">
											<span className="text-muted-foreground">
									...
									{salesNum > tier2Num && (
										<div className="flex justify-between text-sm">
											...
											<span className="font-semibold">${(Math.max(salesNum - tier2Num, 0) * rate3).toFixed(2)}</span>
										</div>
									)}
