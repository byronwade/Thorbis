...
export function useSchedule() {
  ...
  const visibleTimeRange = useMemo(() => {
    return useViewStore.getState().getVisibleTimeRange()
  }, [currentDate, zoom])
        ...
        if (result.success) {
          ...
          const convertedJobs = result.data.jobs.map((job: any) => {
            ...
            if (convertedJob.recurrence?.endDate) {
              convertedJob.recurrence.endDate = new Date(convertedJob.recurrence.endDate)
            }

            return convertedJob
          })

          const convertedTechnicians = result.data.technicians.map((tech: any) => {
            const convertedTech = {
              ...tech,
            ...
            if (convertedTech.schedule?.daysOff) {
              convertedTech.schedule.daysOff = convertedTech.schedule.daysOff.map(
  ...
  const getJobsForTechnician = useCallback(
    (technicianId: string): Job[] => {
      return filteredJobs.filter((job) => job.technicianId === technicianId)
    },
    [filteredJobs]
  )
