...
export function PlaidVerificationTracker({
	...
	}, [companyId, plaidItemId, isPolling, allVerified, status, onVerificationComplete]);
	...
	return (
		<div className="space-y-6">
			...
			<div className="rounded-lg bg-muted/40 p-6 space-y-4">
					...
					<div className="flex-1">
						...
						{lastChecked && (
							<p className="text-xs text-muted-foreground mt-2">
								...
								{isPolling && status !== "failed" && " • Checking every 15 seconds..."}
							</p>
						)}
				...
				{/* Progress Bar */}
				{accounts.length > 0 && status !== "verified" && status !== "failed" && (
					<div className="space-y-2">
						<div className="h-2 bg-muted rounded-full overflow-hidden">
							<div
								className="h-full bg-primary transition-all duration-500"
								style={{ width: `${(verifiedCount / accounts.length) * 100}%` }}
							/>
						</div>
						...
						{accounts.map((account) => (
							<div
								...
								className={cn(
									...
									account.verified ? "bg-green-500/10 ring-1 ring-green-500/20" : "bg-muted/40"
								)}
							>
								<div className="flex items-center justify-between">
									<div className="flex items-center gap-3">
										...
										>
											<Building2 className={cn("h-4 w-4", account.verified ? "text-green-500" : "text-muted-foreground")} />
										</div>
										<div>
			...
			{status !== "failed" && (
				<div className="rounded-lg bg-blue-500/10 p-4 space-y-3">
					<div className="flex items-center gap-2">
						...
						<span className="font-medium">What information does Plaid access?</span>
					</div>
					<div className="space-y-2 text-sm text-muted-foreground">
						<p>
							<strong className="text-foreground">Account details:</strong> Account name, type, and last 4 digits
						</p>
						<p>
							<strong className="text-foreground">Balance:</strong> Current available balance (for verification)
						</p>
						<p>
							<strong className="text-foreground">Routing/Account numbers:</strong> For setting up ACH payments
						</p>
						<p className="text-xs mt-2">
							Plaid uses bank-level encryption and never stores your login credentials. We cannot access your account without your permission.
						</p>
					</div>
				</div>
			)}
				...
				<div className="space-y-3">
					...
					<div className="space-y-3 text-sm text-muted-foreground">
						<div className="flex items-start gap-3">
							...
							<p>
								Plaid is securely connecting to your bank to verify account ownership
							</p>
						</div>
						<div className="flex items-start gap-3">
							<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 flex-shrink-0 mt-0.5">
							...
							<p>
								Confirming account details and retrieving routing/account numbers
							</p>
						</div>
						<div className="flex items-start gap-3">
							<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 flex-shrink-0 mt-0.5">
							...
							<p>
								Setting up secure ACH payment capabilities for deposits and transfers
							</p>
						</div>
					</div>
				</div>
			...
			{status === "verified" && (
				<div className="rounded-lg bg-green-500/10 p-4 space-y-2">
					...
					<p className="text-sm text-muted-foreground">
						Your bank account{accounts.length > 1 ? "s are" : " is"} verified and ready. You can now:
					</p>
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-muted-foreground mt-1">
									{error || "We couldn't verify your bank connection. Please try again."}
								</p>
							</div>
						</div>
					</div>
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<ul className="text-sm text-muted-foreground mt-2 space-y-1 list-disc ml-4">
								...
								<li>Check if you received an email from your bank to confirm the connection</li>
			...
			<div className="rounded-lg border border-muted p-4 space-y-2">
				...
				<p className="text-xs text-muted-foreground">
					Plaid uses the same security protocols as banks. Your login credentials are encrypted and never stored by us or Plaid. You can disconnect your bank at any time from Settings.
				</p>
