	...
	useEffect(() => {
		...
		const checkDNS = async () => {
			...
			try {
				...
				const response = await fetch(`/api/email/verify-dns?domain=${encodeURIComponent(domain)}`);
				const data = await response.json();

				if (data.success && data.records) {
					setRecords((prev) =>
						prev.map((record) => {
							const verified = data.records.find((r: any) => r.id === record.id)?.verified;
		...
		<div className="space-y-6">
					...
					{records.map((record) => (
						<div
							...
							className={cn(
								...
								record.verified ? "bg-green-500/10 ring-1 ring-green-500/20" : "bg-muted/40"
							)}
						>
							...
							<div className="text-xs space-y-1 font-mono">
								<div className="flex flex-wrap items-center gap-1">
									...
									<code className="bg-background px-2 py-1 rounded break-all">{record.name}</code>
								</div>
								<div className="flex flex-wrap items-center gap-1">
									<span className="text-muted-foreground">Value:</span>
									<code className="bg-background px-2 py-1 rounded break-all">{record.value}</code>
								</div>
							</div>
			...
			{!allVerified && (
				<div className="rounded-lg bg-blue-500/10 p-4 space-y-3">
					...
					<div className="space-y-2 text-sm text-muted-foreground">
						<p>
							<strong>5 minutes:</strong> Most records verify this quickly if configured correctly
						</p>
						<p>
							<strong>1-2 hours:</strong> Common propagation time for slower DNS providers
						</p>
						<p>
							<strong>Up to 48 hours:</strong> Maximum time in rare cases (usually much faster)
						</p>
					</div>
					{timeElapsed > 0 && (
				...
				<div className="space-y-4">
					...
					<div className="space-y-3 text-sm text-muted-foreground">
						<div className="flex items-start gap-3">
							...
							<p>
								Log into your domain registrar or DNS provider (where you bought {domain})
							</p>
						</div>
						<div className="flex items-start gap-3">
							<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 flex-shrink-0 mt-0.5">
							...
							<p>
								Find the DNS settings, DNS management, or Zone file section
							</p>
						</div>
						<div className="flex items-start gap-3">
							<div className="flex h-6 w-6 items-center justify-center rounded-full bg-primary/10 flex-shrink-0 mt-0.5">
						...
						<div className="flex items-start gap-3">
							...
							<p>
								Save your changes and wait. We'll automatically detect when records are live.
							</p>
						</div>
					</div>
				</div>
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<ul className="text-sm text-muted-foreground mt-2 space-y-1 list-disc ml-4">
								<li>Double-check that you copied the values exactly (no extra spaces)</li>
								<li>Make sure you selected the correct record type (TXT vs CNAME)</li>
								<li>Some providers need you to use @ instead of the full domain name</li>
								<li>Try refreshing your DNS provider's page and checking if records saved</li>
							</ul>
						</div>
					</div>
				</div>
			...
			{allVerified && (
				<div className="rounded-lg bg-green-500/10 p-4 space-y-2">
					...
					<p className="text-sm text-muted-foreground">
						You can now send emails from {domain}. All notifications and invoices will use this domain.
					</p>
				</div>
			)}
		</div>
