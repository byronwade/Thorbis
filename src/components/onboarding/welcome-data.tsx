...
export async function WelcomeData({ isCreatingNewCompany }: WelcomeDataProps) {
	...
	if (activeCompanyId) {
		...
		if (company) {
			...
			const currentStep = typeof onboardingProgress.currentStep === "number" ? onboardingProgress.currentStep : 1;
			const onboardingComplete = isOnboardingComplete({
				progress: onboardingProgress,
	...
	const { data: userData } = await supabase.from("users").select("email, name").eq("id", user.id).maybeSingle();
