...
export function InvoiceOverdueBanner({ dueDate, balanceAmount, onQuickPay }: InvoiceOverdueBannerProps) {
	...
	return (
		<Alert className={`mb-6 border-2 ${overdueStatus.colors.bg} ${overdueStatus.colors.border} ${getAnimationClass()}`}>
			<div className="flex items-start gap-4">
				...
				<div className="flex-1">
					<AlertTitle className={`mb-2 flex items-center gap-2 font-bold text-lg ${overdueStatus.colors.text}`}>
						<span>PAST DUE</span>
					...
					<AlertDescription className={`space-y-2 ${overdueStatus.colors.text}`}>
						...
						<div className="flex items-center gap-4">
							<div>
								...
								<p className="font-bold text-3xl">{formatCurrency(balanceAmount)}</p>
							</div>
						</div>
					</AlertDescription>
				</div>
