		...
		>
			...
			<div className={cn("flex flex-1 flex-col", variant === "compact" && "py-6")}>
				<CardHeader className="gap-3">
					...
					<CardTitle className="line-clamp-2 text-2xl text-balance">
					...
					{post.tags.length ? (
						<ul className="text-muted-foreground flex flex-wrap gap-2 text-xs">
							{post.tags.slice(0, 4).map((tag) => (
								<li key={tag.id}>
									<Link
										className="border-border hover:border-primary hover:text-primary inline-flex items-center gap-1 rounded-full border px-2 py-1 leading-none transition-colors"
