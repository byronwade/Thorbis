...
export function GanttJobBlock({ job, isSelected, onClick, style, highlightUnassigned = false }: GanttJobBlockProps) {
	...
	const additionalAssignments = job.assignments.filter((assignment) => assignment.role !== "primary");
	const assignmentLabel = job.isUnassigned ? "Unassigned" : primaryAssignment?.displayName || "Assigned";
				...
				<TooltipTrigger asChild>
					<div
						className={cn(
							...
							priorityColor,
							job.isUnassigned && "border-destructive/40 bg-destructive/30 text-destructive-foreground",
					...
					>
						<div className="flex items-center justify-between gap-2">
							<p className="line-clamp-1 font-semibold text-xs leading-tight">{job.title}</p>
							<div
								className={cn(
						...
						<div className="flex items-center gap-1 text-[10px] opacity-90">
							{job.isUnassigned ? <AlertTriangle className="size-3" /> : <Users className="size-3" />}
						...
						{job.assignments.length > 0 && (
							<div className="flex flex-wrap gap-1">
								...
								{job.assignments.length > 3 && (
									<span className="text-[10px] text-white/70">+{job.assignments.length - 3} more</span>
								)}
							</div>
						)}
				...
				<TooltipContent className="w-64 space-y-2 text-left">
					...
					<div className="space-y-1 text-[11px]">
						...
						{job.assignments.length > 0 ? (
							...
							<span className="font-medium text-destructive">No technician assigned</span>
						)}
					</div>
				</TooltipContent>
			</Tooltip>
