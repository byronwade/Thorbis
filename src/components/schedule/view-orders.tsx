					...
					{dayDates.map((d) => {
						...
						return (
							<div className="flex flex-col" key={d.toISOString()}>
								<h3 className="px-1 pb-2 font-semibold text-neutral-600 text-xs">
									{"Today's Orders"}{" "}
									<span className="ml-1 rounded-full bg-neutral-100 px-2 py-0.5 text-[10px]">{orders.length}</span>
								</h3>

								{orders.length === 0 && (
									<div className="rounded-xl border border-dashed bg-white p-6 text-center text-neutral-500 text-xs">
										No orders scheduled
									</div>
								)}
										...
										<div className="space-y-3 p-3">
											{o.groups.map((g, gi) => (
												<div className="rounded-lg border p-2" key={`${o.id}-g${gi}`}>
															...
															>
																<div className="flex items-center gap-2">
																	...
																	<div className="min-w-0 flex-1">
																		...
																		{it.meta && <div className="truncate text-[11px] text-neutral-600">{it.meta}</div>}
																	</div>
																	{it.qty && (
																		<span className="rounded bg-neutral-100 px-1.5 py-0.5 text-[10px]">{it.qty}</span>
																	)}
																</div>
															</div>
														))}
													...
													<div className="mt-2">
														<Button className="w-full justify-center rounded-md text-xs" variant="outline">
															Add Truck
														</Button>
													</div>
												</div>
