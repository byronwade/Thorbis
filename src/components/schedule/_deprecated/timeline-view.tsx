...
function jobToGanttFeature(job: Job, technicianId: string, viewMode: ViewMode, dayOffset = 0): GanttFeature {
...
function TimelineContent({ viewMode, ganttData, onScrollToToday }: {
  viewMode: ViewMode
  ganttData: Array<{ technician: import("./schedule-types").Technician; features: GanttFeature[] }>
              ...
              <div className="flex items-start gap-3 p-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <div className={cn("size-2 shrink-0 rounded-full", technicianStatusColors[technician.status])} />
                  </div>
                ...
                return (
                  <div className="flex h-full w-full flex-col gap-0.5 px-1">
                    <div className="flex items-start justify-between gap-1">
                      ...
                      <div className={cn("size-1.5 shrink-0 rounded-full", priorityBorderColors[job.priority].replace("border-", "bg-"))} />
                    </div>
...
export function TimelineView() {
  ...
  const scrollToDate = (targetDate: Date) => {
    ...
    if (viewMode === "hourly") {
      ...
      const daysToDate = Math.floor((startOfDate.getTime() - timelineStart.getTime()) / (1000 * 60 * 60 * 24))
      ...
      const daysDifference = Math.floor((startOfDate.getTime() - timelineStart.getTime()) / (1000 * 60 * 60 * 24))
      ...
      const monthsDifference = (targetDate.getFullYear() - timelineStart.getFullYear()) * 12 +
      ...
      const scrollPosition = (monthsDifference * parsedColumnWidth) + (monthFraction * parsedColumnWidth)
      ganttElement.scrollLeft = scrollPosition
    }
  }
        ...
        <div className="flex items-center justify-between border-b bg-background p-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex gap-1">
              <Button size="sm" variant="default">Hourly</Button>
  ...
  return (
    <div className="flex h-full w-full flex-col">
      ...
      <GanttProvider range={getGanttRange(viewMode)} zoom={getZoomLevel(viewMode)}>
        <TimelineContent viewMode={viewMode} ganttData={ganttData} onScrollToToday={scrollToToday} />
      </GanttProvider>
    </div>
  )
}
