...
function jobToGanttFeature(job: Job, _technicianId: string, viewMode: ViewMode, dayOffset = 0): GanttFeature {
								...
								return (
									<div className="flex h-full w-full flex-col gap-0.5 px-1">
										<div className="flex items-start justify-between gap-1">
											<p className="line-clamp-1 flex-1 font-semibold text-[10px] leading-tight">{job.title}</p>
											<div
												className={cn(
										...
										<p className="line-clamp-1 text-[9px] text-muted-foreground">{job.customer}</p>
	...
	return (
		<div className="flex h-full w-full flex-col">
			...
			<GanttProvider range={getGanttRange(viewMode)} zoom={getZoomLevel(viewMode)}>
				<TimelineContent ganttData={ganttData} onScrollToToday={scrollToToday} viewMode={viewMode} />
			</GanttProvider>
		</div>
	);
}
