...
const STATUS_CONFIG: Record<TextStatus, { label: string; color: string; icon: typeof CheckCircle2 }> = {
...
const DEMO_TEXTS: AIText[] = [
	{
		...
		content: "Hi, I'd like to schedule an appointment for tomorrow if possible.",
		aiGenerated: false,
		autoReplied: true,
	...
	},
	{
		...
		customerName: "John Smith",
		customerPhone: "+1 (555) 123-4567",
		content: "Hi John! I've checked our schedule and we have availability tomorrow at 10am or 2pm. Which works better for you?",
		aiGenerated: true,
		autoReplied: true,
	...
	},
	{
		...
		content: "Hi Sarah! This is a reminder about your scheduled service appointment tomorrow at 9:00 AM. Reply CONFIRM to confirm or RESCHEDULE to change.",
		aiGenerated: true,
		autoReplied: false,
	...
	},
	{
		...
		customerName: "Sarah Johnson",
		customerPhone: "+1 (555) 234-5678",
		content: "Great! Your appointment is confirmed for tomorrow at 9:00 AM. See you then!",
		aiGenerated: true,
		autoReplied: true,
		...
		<div className="flex flex-col h-full p-4 space-y-4">
					...
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								...
								<CardDescription>Automatically responds to incoming messages</CardDescription>
							</div>
						</div>
						<div className="flex items-center gap-6">
							<div className="flex items-center gap-2">
								<span className="text-sm text-muted-foreground">Auto-Reply</span>
								<Switch checked={autoReplyEnabled} onCheckedChange={setAutoReplyEnabled} disabled={!aiEnabled} />
							</div>
							<div className="flex items-center gap-2">
			...
			<div className="grid grid-cols-4 gap-4">
				<div className="p-4 border rounded-lg bg-card">
					<div className="text-xs text-muted-foreground">Total Messages (Today)</div>
					<div className="text-2xl font-semibold">{stats.total}</div>
				</div>
				<div className="p-4 border rounded-lg bg-card">
					<div className="text-xs text-muted-foreground">AI Generated</div>
					<div className="text-2xl font-semibold text-primary">{stats.aiGenerated}</div>
				</div>
				<div className="p-4 border rounded-lg bg-card">
					<div className="text-xs text-muted-foreground">Auto-Replied</div>
					<div className="text-2xl font-semibold text-green-500">{stats.autoReplied}</div>
				</div>
				<div className="p-4 border rounded-lg bg-card">
				...
				<div className="divide-y overflow-auto h-full">
					{texts.length === 0 ? (
						<div className="flex flex-col items-center justify-center h-40 text-muted-foreground">
							...
							<p className="text-xs">Text messages will appear here when AI processes them</p>
						</div>
					) : (
						texts.map((text) => {
							const status = STATUS_CONFIG[text.status];
							const StatusIcon = status.icon;
							const DirectionIcon = text.direction === "inbound" ? ArrowDown : ArrowUp;

							return (
								<div key={text.id} className="p-4 flex items-start gap-4 hover:bg-muted/50 transition-colors">
									<div className={cn("size-10 rounded-full flex items-center justify-center shrink-0", text.direction === "inbound" ? "bg-purple-500/10" : "bg-blue-500/10")}>
										<DirectionIcon className={cn("size-5", text.direction === "inbound" ? "text-purple-500" : "text-blue-500")} />
									</div>

									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-2">
											<span className="font-medium">{text.customerName}</span>
											<span className="text-sm text-muted-foreground">{text.customerPhone}</span>
											{text.aiGenerated && (
										...
										<p className="text-sm mt-1 text-foreground/80">{text.content}</p>
									...
									<span className="text-xs text-muted-foreground shrink-0">{formatDistanceToNow(new Date(text.createdAt), { addSuffix: true })}</span>
								</div>
							);
						})
					)}
				...
				<CardContent className="py-4">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-muted-foreground">Full AI SMS automation with Telnyx is under development. Demo data shown above.</p>
						</div>
