...
export function AppToolbar() {
  ...
  if (!toolbarConfig) {
    return (
      <header className="sticky top-0 z-40 flex w-full shrink-0 border-b border-border/50 bg-background/90 backdrop-blur-md md:rounded-t-2xl">
      ...
      <div className="flex min-h-14 w-full items-center gap-4 px-4 py-2">
        ...
        <div className="flex shrink-0 flex-col">
          ...
          {toolbarConfig.subtitle && (
            <p className="text-muted-foreground text-sm">{toolbarConfig.subtitle}</p>
          )}
        </div>
