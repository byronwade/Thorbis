...
import { getTechnicianSchedules, type ScheduleAppointment, type TechnicianSchedule } from "@/actions/schedule";
...
export function CSRScheduleView({ className, companyId }: CSRScheduleViewProps) {
		...
		return (
			<div className={cn("flex h-full items-center justify-center", className)}>
				<div className="text-center">
					...
					<Button className="mt-4" onClick={() => window.location.reload()} size="sm" variant="outline">
						Retry
					</Button>
				</div>
			</div>
	...
	const getStatusBadge = (status: ScheduleAppointment["status"]) => {
		switch (status) {
			case "completed":
				return (
					<Badge className="border-success/50 bg-success/10 text-[10px] text-success" variant="outline">
						Completed
					</Badge>
				);
			case "in_progress":
				return (
					<Badge className="border-warning/50 bg-warning/10 text-[10px] text-warning" variant="outline">
						In Progress
					</Badge>
				);
			case "cancelled":
				return (
					<Badge className="border-destructive/50 bg-destructive/10 text-[10px] text-destructive" variant="outline">
						Cancelled
					</Badge>
				);
			default:
	...
	const sections: UnifiedAccordionSection[] = filteredTechnicians.map((tech) => ({
		...
		icon: (
			<Avatar className="h-5 w-5">
				...
				<AvatarFallback className={cn("text-[10px] text-white", tech.color)}>{getInitials(tech.name)}</AvatarFallback>
			</Avatar>
		),
				...
				{tech.appointments.length > 0 ? (
					<div className="space-y-3">
						{tech.appointments.map((appointment) => (
							<div className="rounded-lg border bg-card p-4 transition-colors hover:bg-muted/50" key={appointment.id}>
								{/* Time & Status */}
								...
								<div className="mb-3 space-y-1">
									<div className="flex items-center gap-2">
										...
										<span className="font-medium text-sm">{appointment.customer_name}</span>
									</div>
									<div className="flex items-center gap-2">
									...
									<div className="flex items-center gap-2">
										...
										<span className="font-mono text-muted-foreground text-xs">#{appointment.job_number}</span>
									</div>
								</div>
			...
			<div className="space-y-3 border-b bg-card/50 p-4">
				...
				<div className="flex items-center justify-between gap-2">
					<div className="flex items-center gap-2 rounded-lg border bg-muted/30 px-3 py-1.5">
						<Users className="h-3.5 w-3.5 text-muted-foreground" />
						<span className="text-muted-foreground text-xs">{filteredTechnicians.length} Techs</span>
						<span className="text-muted-foreground">•</span>
						<span className="font-medium text-xs">{totalAppointments} Appointments</span>
					</div>
