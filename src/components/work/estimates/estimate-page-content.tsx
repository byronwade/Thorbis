...
import { Calendar, FileCheck, FileText, Link2Off, Receipt, TrendingUp, User } from "lucide-react";
					...
					<div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
						<div className="flex flex-col gap-4">
							...
							<div className="flex flex-col gap-2">
								...
								<p className="text-muted-foreground text-sm sm:text-base">{formatCurrency(estimate.total_amount)}</p>
							</div>
						</div>
					</div>
					...
					<div className="flex flex-wrap items-center gap-3">
						{estimate.valid_until && (
							<div className="inline-flex items-center gap-2 rounded-full bg-muted px-3 py-1 text-sm">
								...
								<span className="font-medium">Valid until {new Date(estimate.valid_until).toLocaleDateString()}</span>
							</div>
						)}
						{job && (
							<Link
		...
		if (contract || invoice) {
			const workflowStages = [
				...
				},
				{
					...
					status:
						invoice?.status === "paid" ? ("completed" as const) : invoice ? ("current" as const) : ("pending" as const),
							...
							<Table>
								...
								<TableBody>
									{lineItems.map((item: any, index: number) => (
										<TableRow key={index}>
											<TableCell className="font-medium">{item.description || item.name || "Item"}</TableCell>
											<TableCell className="text-right">{item.quantity || 1}</TableCell>
											<TableCell className="text-right">
												{formatCurrency(item.unit_price || item.unitPrice || item.price || 0)}
											</TableCell>
											<TableCell className="text-right font-semibold">
												{formatCurrency((item.quantity || 1) * (item.unit_price || item.unitPrice || item.price || 0))}
											</TableCell>
										</TableRow>
									))}
									<TableRow className="border-t-2 bg-muted/50">
										...
										<TableCell className="text-right font-semibold">{formatCurrency(estimate.subtotal || 0)}</TableCell>
									</TableRow>
									{estimate.tax_amount > 0 && (
										<TableRow>
											...
											<TableCell className="text-right">{formatCurrency(estimate.tax_amount || 0)}</TableCell>
										</TableRow>
									)}
									{estimate.discount_amount > 0 && (
										<TableRow>
											...
											<TableCell className="text-right">-{formatCurrency(estimate.discount_amount || 0)}</TableCell>
										</TableRow>
									)}
									<TableRow className="border-t-2 bg-muted/50">
										<TableCell className="text-right font-bold text-lg" colSpan={3}>
											Total
										</TableCell>
					...
					<UnifiedAccordionContent>
						<div className="space-y-3">
							<div>
								...
								<p className="text-sm">{new Date(estimate.valid_until).toLocaleDateString()}</p>
							</div>
							{estimate.status === "accepted" && estimate.accepted_at && (
								<div>
									<Label>Accepted At</Label>
									<p className="text-sm">{new Date(estimate.accepted_at).toLocaleString()}</p>
								</div>
							)}
							{estimate.status === "declined" && estimate.declined_at && (
								<div>
									<Label>Declined At</Label>
									<p className="text-sm">{new Date(estimate.declined_at).toLocaleString()}</p>
			...
			<Dialog onOpenChange={(open) => !open && setUnlinkJobId(null)} open={unlinkJobId !== null}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							This will remove the job association from this estimate. The estimate will remain in the system but will
							no longer appear on the job's page.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
