...
import type { POStatus, PurchaseOrder } from "@/components/work/purchase-orders-table";
...
const priorityBadge: Record<PurchaseOrder["priority"], { label: string; className: string }> = {
	...
	return (
		<EntityKanban<PurchaseOrder, POStatus>
			calculateColumnMeta={(columnId, items): ColumnMeta => {
				...
				const total = columnItems.reduce((sum, item) => sum + (item.entity as PurchaseOrder).totalAmount, 0);
	...
	return (
		<div className="space-y-3">
			<div className="flex items-start justify-between gap-3">
				<div>
					<p className="font-semibold text-muted-foreground text-xs uppercase tracking-wide">{order.poNumber}</p>
					...
					<div className="mt-2 flex flex-wrap items-center gap-2">
						<Badge
							className={cn(
								...
								(columnId === "approved" || columnId === "received") && "bg-primary/10 text-primary"
							)}
							variant={
								columnId === "cancelled"
									? "destructive"
						...
						<Badge className={cn("text-xs", priority.className)} variant="outline">
							{priority.label}
						</Badge>
						{order.autoGenerated && (
							<Badge className="bg-secondary/20 text-secondary-foreground/80" variant="secondary">
								Auto-generated
							</Badge>
						)}
					</div>
			...
			<div className="flex items-center justify-between pt-2 text-muted-foreground text-xs">
				<span>Total</span>
				<span className="font-semibold text-foreground">{currencyFormatter.format(order.totalAmount / 100)}</span>
			</div>
