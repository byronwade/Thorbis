...
import { AlertTriangle, CircleDollarSign, FileCheck, Flame, Home, MapPin, Users, Zap } from "lucide-react";
...
export function PropertyIntelligenceWidget({ property, enrichment }: PropertyIntelligenceWidgetProps) {
	// ============================================================================
	// No Data State
	// ============================================================================
		...
		return (
			<div className="space-y-3">
				...
				{property ? (
					...
					<div className="text-center text-muted-foreground text-sm">No property data available</div>
				)}

				<Separator />

				{/* Enrichment status */}
				<div className="rounded-lg border border-muted-foreground/25 border-dashed p-3 text-center">
					<p className="text-muted-foreground text-xs">Property intelligence unavailable</p>
					<p className="text-muted-foreground text-xs">Configure API keys to enrich property data</p>
				</div>
			</div>
		);
	}
	...
	return (
		<div className="space-y-4">
			...
			<div className="space-y-2">
				<h4 className="flex items-center gap-1.5 font-semibold text-xs">
						...
						<div className="space-y-1.5">
							{permits.slice(0, 3).map((permit) => (
								<div className="rounded-md border p-2 text-xs" key={permit.permitNumber}>
									...
									<div className="mt-1 text-muted-foreground">{permit.description}</div>
			...
			{/* Risk Factors */}
			{riskFactors.floodZone || riskFactors.earthquakeZone || riskFactors.fireZone || riskFactors.hurricaneZone ? (
				<>
					<Separator />
					<div className="space-y-2">
						<h4 className="flex items-center gap-1.5 font-semibold text-xs">
						...
						<div className="space-y-1.5 text-xs">
							{riskFactors.floodZone ? (
								<div className="flex items-center justify-between">
									...
									<span className={getRiskColor(riskFactors.floodZone)}>{riskFactors.floodZone}</span>
								</div>
							) : null}
							{riskFactors.earthquakeZone ? (
								<div className="flex items-center justify-between">
									<span className="text-muted-foreground">Earthquake Zone</span>
									<span className={getRiskColor(riskFactors.earthquakeZone)}>{riskFactors.earthquakeZone}</span>
								</div>
							) : null}
							...
							{riskFactors.hurricaneZone ? (
								<div className="flex items-center justify-between">
									...
									<span className={getRiskColor(riskFactors.hurricaneZone)}>{riskFactors.hurricaneZone}</span>
								</div>
							) : null}
						</div>
					</div>
