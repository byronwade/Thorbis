...
export function PaymentTrackerWidget({ job }: PaymentTrackerWidgetProps) {
	...
	const paymentMilestones = [
		...
		},
		{
			...
			status: paidAmount >= totalAmount * 0.7 ? "paid" : paidAmount >= totalAmount * 0.3 ? "due" : "upcoming",
		},
		{
			id: "3",
			name: "Final Payment",
			...
			status: paidAmount >= totalAmount ? "paid" : paidAmount >= totalAmount * 0.7 ? "due" : "upcoming",
		},
	];
		...
		<div className="space-y-4">
			...
			<div>
				<div className="mb-2 flex items-center justify-between">
					...
					<span className="font-bold text-base">{Math.round(percentagePaid)}%</span>
				</div>
			...
			<div className="space-y-2 rounded-lg bg-muted/30 p-3 text-sm">
				...
				<div className="flex justify-between">
					...
					<span className="font-bold text-warning">{formatCurrency(remainingAmount)}</span>
				</div>
			</div>
								...
								<div className="flex-1 space-y-1">
									<div className="flex items-center justify-between gap-2">
										...
										<Badge
											className="text-xs"
											variant={
												milestone.status === "paid" ? "default" : milestone.status === "due" ? "destructive" : "outline"
											}
										>
											{milestone.status}
										</Badge>
