...
export function LinkedDataAlerts({ job, invoices = [], estimates = [] }: LinkedDataAlertsProps) {
	...
	const hasUnpaidDeposit =
		job.financial?.deposit_amount && job.financial.deposit_amount > 0 && !job.deposit_paid_at;
			...
			<HoverCard key="past-due" openDelay={200}>
				...
				<HoverCardContent align="start" className="w-96" side="bottom">
					<div className="space-y-3">
						<div>
							<h4 className="text-sm font-semibold">Past Due Invoices</h4>
						...
						<div className="flex items-center justify-between">
							<span className="text-sm font-semibold">Total Past Due</span>
	...
	if (outstandingEstimates.length > 0) {
		alerts.push(
			<HoverCard key="estimates" openDelay={200}>
				<HoverCardTrigger asChild>
					<button className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 transition-colors hover:border-amber-300 hover:bg-amber-100 dark:border-amber-900/30 dark:bg-amber-900/20 dark:text-amber-400">
				...
				<HoverCardContent align="start" className="w-96" side="bottom">
					<div className="space-y-3">
						<div>
							<h4 className="text-sm font-semibold">Outstanding Estimates</h4>
	...
	if (hasUnpaidDeposit) {
		alerts.push(
			<button
				className="inline-flex items-center gap-2 rounded-full border border-red-200 bg-red-50 px-4 py-2 text-sm font-medium text-red-700 transition-colors hover:border-red-300 hover:bg-red-100 dark:border-red-900/30 dark:bg-red-900/20 dark:text-red-400"
				key="deposit"
			>
	...
	if (isCompletionOverdue) {
		const daysPastDue = Math.floor(
			(today.getTime() - new Date(job.scheduled_end!).getTime()) / (1000 * 60 * 60 * 24)
		);
		alerts.push(
			<button
				className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 transition-colors hover:border-amber-300 hover:bg-amber-100 dark:border-amber-900/30 dark:bg-amber-900/20 dark:text-amber-400"
				key="overdue"
			>
