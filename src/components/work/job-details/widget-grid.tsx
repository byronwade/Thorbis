...
export function WidgetGrid({
	...
	const orderedWidgets = useMemo(
		() =>
			items.map((id) => widgets.find((w) => w.id === id)).filter((w): w is NonNullable<typeof w> => w !== undefined),
		[items, widgets]
	);
	...
	const activeWidget = useMemo(() => orderedWidgets.find((w) => w.id === activeId), [activeId, orderedWidgets]);
				...
				<div className="relative" style={gridStyle}>
					{orderedWidgets.map((widget) => {
						...
						return (
							...
							>
								<WidgetContainer isDragging={activeId === widget.id} isEditMode={isEditMode} widget={widget}>
									<WidgetRenderer
