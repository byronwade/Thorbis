		...
		<div className="space-y-6">
					...
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
						{categoryPhotos.map((photo) => (
							<div className="group relative overflow-hidden rounded-lg border" key={photo.id}>
								...
								<div className="p-3">
									{photo.title && <p className="mb-1 font-medium text-sm">{photo.title}</p>}
									{photo.description && <p className="mb-2 text-muted-foreground text-xs">{photo.description}</p>}
			...
			<div className="rounded-md bg-muted/50 p-4">
				...
				<p className="text-muted-foreground text-xs">
					{photos.length} photo{photos.length !== 1 ? "s" : ""} across {Object.keys(groupedPhotos).length} categor
