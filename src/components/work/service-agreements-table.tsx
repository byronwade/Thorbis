...
export function ServiceAgreementsTable({
	...
	const columns: ColumnDef<ServiceAgreement>[] = [
		...
		},
		{
			...
			render: (agreement) => (
				<div className="min-w-0">
					<div className="truncate font-medium text-sm">{agreement.customer}</div>
		...
		},
		{
			...
			hideOnMobile: true,
			render: (agreement) => (
				<span className="text-sm">{agreement.startDate}</span>
			),
		},
		{
			key: "endDate",
			...
			hideOnMobile: true,
			render: (agreement) => (
				<span className="text-sm">{agreement.endDate}</span>
			),
		},
		{
			key: "value",
