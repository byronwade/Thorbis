...
import { Archive, Briefcase, Edit, Eye, MoreHorizontal, Trash2 } from "lucide-react";
...
function getStatusBadge(status: string) {
	...
	> = {
		...
		scheduled: {
			className: "border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 dark:border-blue-900 dark:bg-blue-950/50 dark:text-blue-400",
			label: "Scheduled"
		},
		in_progress: {
...
function getPriorityBadge(priority: string) {
	const variants: Record<string, { className: string; label: string }> = {
		low: {
			className: "border-blue-200/50 bg-blue-50/50 text-blue-700 dark:border-blue-900/50 dark:bg-blue-950/30 dark:text-blue-400",
			label: "Low",
		},
		medium: {
			className: "border-yellow-200/50 bg-yellow-50/50 text-yellow-700 dark:border-yellow-900/50 dark:bg-yellow-950/30 dark:text-yellow-400",
			label: "Medium",
		},
		high: {
			className: "border-orange-200/50 bg-orange-50/50 text-orange-700 dark:border-orange-900/50 dark:bg-orange-950/30 dark:text-orange-400",
			label: "High",
		},
		urgent: {
			className: "border-red-200/50 bg-red-50/50 text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-400",
			label: "Urgent",
		},
	};
...
export function JobsTable({
	...
	const columns: ColumnDef<Job>[] = [
		...
		},
		{
			...
			hideOnMobile: true,
			render: (job) => (
				<span className="text-muted-foreground text-sm tabular-nums">{formatDate(job.scheduledStart)}</span>
			),
		},
		{
			key: "totalAmount",
	...
	const searchFilter = (job: Job, query: string) => {
		...
		const aiTags = job.aiTags
			? (typeof job.aiTags === 'string' ? JSON.parse(job.aiTags) : job.aiTags)
			: [];
		const aiCategories = job.aiCategories
			? (typeof job.aiCategories === 'string' ? JSON.parse(job.aiCategories) : job.aiCategories)
			: [];
		const aiEquipment = job.aiEquipment
			? (typeof job.aiEquipment === 'string' ? JSON.parse(job.aiEquipment) : job.aiEquipment)
			: [];
