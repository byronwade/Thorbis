	...
	return (
		<Popover onOpenChange={setOpen} open={open}>
			<PopoverTrigger asChild>
				<Button
					aria-expanded={open}
					className={cn("w-full justify-between", !selectedVendor && "text-muted-foreground", className)}
					disabled={disabled}
					role="combobox"
					variant="outline"
				>
					{selectedVendor ? (
						<span className="flex items-center gap-2">
							...
							<span className="text-muted-foreground text-xs">({selectedVendor.vendor_number})</span>
						</span>
					) : (
