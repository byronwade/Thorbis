			...
			<div className="flex items-center gap-3">
				...
				<div className="flex items-center gap-1 rounded-lg border bg-muted/30 p-1">
					{QUICK_FILTERS.map((filter) => (
						<Button
							key={filter.value}
							variant={selectedPreset === filter.value && !isCustomRange ? "secondary" : "ghost"}
							size="sm"
							className={cn(
								"h-7 px-3 text-xs font-medium",
								selectedPreset === filter.value && !isCustomRange && "bg-background shadow-sm"
							)}
					...
					<PopoverContent className="w-auto p-0" align="start">
						<div className="flex">
							...
							<div className="w-[130px] border-r p-2 space-y-0.5">
								<p className="px-2 py-1.5 text-[11px] font-medium text-muted-foreground uppercase tracking-wide">Quick Select</p>
								{DATE_PRESETS.map((preset) => (
									<Button
										key={preset.value}
										variant={selectedPreset === preset.value && !isCustomRange ? "secondary" : "ghost"}
										size="sm"
