		...
		return (
			<div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
				...
				<p className="text-muted-foreground text-center max-w-md">
					Complete some jobs with assigned technicians to see team performance metrics.
				</p>
			</div>
		);
	}
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Performance rankings and metrics</p>
				</div>
					...
					<CardContent>
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-muted-foreground">{data.topPerformer.highlight}</p>
							</div>
					...
					<Table>
						...
						<TableBody>
							{data.technicians.map((tech) => (
								<TableRow key={tech.id} className={tech.rank <= 3 ? "bg-yellow-500/5" : ""}>
									<TableCell>
										<RankBadge rank={tech.rank} />
									</TableCell>
									<TableCell className="font-medium">{tech.name}</TableCell>
									<TableCell className="text-right">{tech.jobsCompleted}</TableCell>
									<TableCell className="text-right">{formatCurrency(tech.revenue)}</TableCell>
									<TableCell className="text-right">{formatDuration(tech.avgJobDuration)}</TableCell>
...
function TechnicianCard({
	...
	return (
		<Card className={highlight ? "border-yellow-500/30 bg-gradient-to-br from-yellow-500/5 to-orange-500/5" : ""}>
			...
			<CardContent>
				<div className="grid grid-cols-2 gap-4 text-sm">
					...
					<div>
						...
						<p className="font-semibold">{formatPercent(tech.firstTimeFixRate)}</p>
					</div>
					<div>
