...
import { getFinancialReport, type FinancialReportData } from "@/lib/queries/analytics";
		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">P&L, AR aging, and cash flow analysis</p>
				</div>
			...
			<Card>
				...
				<CardContent>
					{data.monthlyPL.length === 0 ? (
						<p className="text-muted-foreground py-8 text-center">No data available for this period</p>
					) : (
						<Table>
							<TableHeader>
								<TableRow>
							...
							<TableBody>
								{data.monthlyPL.slice(-12).map((month) => (
									<TableRow key={month.month}>
										...
										<TableCell className="text-right">
											<Badge variant={month.margin >= 30 ? "default" : month.margin >= 15 ? "secondary" : "destructive"}>
			...
			<Card>
				...
				<CardContent>
					{data.cashFlow.length === 0 ? (
						<p className="text-muted-foreground py-8 text-center">No cash flow data available</p>
					) : (
						<Table>
							<TableHeader>
								<TableRow>
...
function AgingBucket({
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-muted-foreground text-xs">{formatPercent(percentage)} of total</p>
		</div>
	);
}
