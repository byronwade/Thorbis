...
import { BarChart3, Clock, FileText, Calendar, TrendingUp, ArrowUpRight } from "lucide-react";
...
export async function ReportsStats() {
	...
	const totalRevenue = (recentPayments || []).reduce((sum, p) => sum + (p.amount || 0), 0) / 100;
	...
	return (
		<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
			{stats.map((stat) => (
				<div key={stat.label} className="bg-card rounded-lg border p-4 transition-all hover:shadow-sm">
