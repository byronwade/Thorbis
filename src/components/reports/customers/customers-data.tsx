		...
		<div className="space-y-6">
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground">Retention, segmentation, and lifetime value analysis</p>
				</div>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<SummaryCard
					...
					trend={data.summary.retentionRate >= 90 ? "positive" : data.summary.retentionRate >= 80 ? "neutral" : "negative"}
				/>
				<SummaryCard
				...
				<Card>
					...
					<CardContent>
						{data.bySegment.length === 0 ? (
							<p className="text-muted-foreground py-8 text-center">No segment data available</p>
						) : (
							<Table>
								<TableHeader>
									<TableRow>
								...
								<TableBody>
									{data.bySegment.map((segment) => (
										<TableRow key={segment.segment}>
											...
											<TableCell className="text-right">{formatCurrency(segment.avgTicket)}</TableCell>
										</TableRow>
									))}
								</TableBody>
							</Table>
								...
								<TableBody>
									{data.acquisitionTrend.slice(-12).map((month) => (
										<TableRow key={month.month}>
											...
											<TableCell className="text-right text-green-600">+{month.newCustomers}</TableCell>
											<TableCell className="text-right text-red-600">-{month.churned}</TableCell>
											<TableCell className="text-right">
												<span className={month.net >= 0 ? "text-green-600 font-medium" : "text-red-600 font-medium"}>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Your most valuable customers this year</CardDescription>
				</CardHeader>
				<CardContent>
					{data.topCustomers.length === 0 ? (
						<p className="text-muted-foreground py-8 text-center">No customer data available</p>
					) : (
						<Table>
							<TableHeader>
								<TableRow>
							...
							<TableBody>
								{data.topCustomers.map((customer, index) => (
									<TableRow key={customer.id}>
										...
										<TableCell className="text-right">{formatCurrency(customer.lifetimeValue)}</TableCell>
...
function RiskCard({
	...
	return (
		<div className="space-y-3 p-4 rounded-lg border bg-card/50">
			...
			<p className="text-sm font-semibold">{formatPercent(percentage)} of customers</p>
		</div>
	);
}
