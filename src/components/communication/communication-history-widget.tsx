...
export function CommunicationHistoryWidget({
	...
	const filteredCommunications = typeFilter === "all"
		? communications
			...
			<CardHeader>
				<div className="flex items-center justify-between">
					...
					<Button
						...
						className="gap-1"
					>
						<Link href={`/dashboard/communication${customerId ? `?customerId=${customerId}` : jobId ? `?jobId=${jobId}` : ""}`}>
							View All
				...
				{showFilters && (
					<div className="flex gap-1 mt-3">
						...
						>
							...
							All
							{typeCounts.all > 0 && (
								<Badge variant={typeFilter === "all" ? "secondary" : "outline"} className="ml-0.5 h-4 px-1 text-[10px]">
									{typeCounts.all}
								</Badge>
							)}
						</Button>
						...
						{typeCounts.email > 0 && (
							...
							>
								...
								Email
								<Badge variant={typeFilter === "email" ? "secondary" : "outline"} className="ml-0.5 h-4 px-1 text-[10px]">
									{typeCounts.email}
								</Badge>
							</Button>
						)}
						...
						{typeCounts.sms > 0 && (
							...
							>
								...
								SMS
								<Badge variant={typeFilter === "sms" ? "secondary" : "outline"} className="ml-0.5 h-4 px-1 text-[10px]">
									{typeCounts.sms}
								</Badge>
							</Button>
						)}
						...
						{typeCounts.call > 0 && (
							...
							>
								...
								Calls
								<Badge variant={typeFilter === "call" ? "secondary" : "outline"} className="ml-0.5 h-4 px-1 text-[10px]">
									{typeCounts.call}
								</Badge>
							</Button>
						)}
						...
						{typeCounts.voicemail > 0 && (
							...
							>
								...
								Voicemail
								<Badge variant={typeFilter === "voicemail" ? "secondary" : "outline"} className="ml-0.5 h-4 px-1 text-[10px]">
								...
								return (
									...
									>
										<div className="flex items-start gap-3">
											{/* Type icon */}
											<div className={cn("rounded-full p-2 mt-0.5", typeConfig.bg)}>
											...
											<div className="flex-1 min-w-0 space-y-1">
												<div className="flex items-center justify-between gap-2">
													<p className="font-medium text-sm truncate">
														{communication.subject || (communication.type === "sms" ? "Text Message" : "No Subject")}
													</p>
