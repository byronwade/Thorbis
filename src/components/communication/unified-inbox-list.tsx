...
export function UnifiedInboxList({
	...
	// Type filter buttons
	const typeFilters: { type: CommunicationType; label: string; icon: typeof Mail }[] = [
					...
					{typeFilters.map(({ type, label, icon: Icon }) => {
						...
						return (
							...
							>
								...
								{label}
								{count > 0 && (
									<Badge variant={selectedType === type ? "secondary" : "outline"} className="ml-1">
										{count}
									</Badge>
								)}
							</Button>
			...
			<ScrollArea className="flex-1">
				{filteredCommunications.length === 0 ? (
					<div className="flex h-64 items-center justify-center text-center p-4">
						<div>
							<p className="text-sm font-medium text-muted-foreground">No communications found</p>
							<p className="text-xs text-muted-foreground mt-1">
								{searchQuery ? "Try adjusting your search" : "New messages will appear here"}
							</p>
						</div>
					</div>
				) : (
...
function CommunicationListItem({
	...
	const displayAddress = communication.direction === "inbound"
	...
	// Format timestamp
	const timestamp = formatDistanceToNow(new Date(communication.createdAt), { addSuffix: true });
		...
		>
			<div className="flex items-start gap-3">
				...
				<div className="relative">
					...
					<div className={cn("absolute -bottom-1 -right-1 rounded-full p-1", bg)}>
				...
				<div className="flex-1 min-w-0">
					...
					<div className="flex items-center gap-2 mb-1">
						...
						{isUnread && <CircleDot className="h-3 w-3 text-blue-500 flex-shrink-0 ml-auto" />}
					</div>
						...
						{autoLinked && linkConfidence !== undefined && (
							<TooltipProvider>
								<Tooltip>
									...
									<TooltipContent>
										Auto-linked with {Math.round(linkConfidence * 100)}% confidence
										{communication.linkMethod && ` (${communication.linkMethod})`}
									</TooltipContent>
								</Tooltip>
							</TooltipProvider>
						)}
