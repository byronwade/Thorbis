		...
		<section className="bg-black py-32">
			<div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
				...
				<div className="mb-16 text-center">
					...
					<h2 className="mb-4 text-balance font-semibold text-4xl text-white md:text-6xl">Everything You Need in One Platform</h2>
					<p className="mx-auto max-w-2xl text-foreground text-lg md:text-xl">Powerful features designed to streamline every aspect of your field service business</p>
				</div>
						...
						{categories.map((category, idx) => (
							<button
								...
								className={`group flex shrink-0 items-center gap-3 rounded-2xl border px-6 py-4 transition-all duration-300 ${
									idx === activeCategory ? "scale-105 border-primary bg-gradient-to-br from-primary/10 to-transparent shadow-lg shadow-primary/20" : "border-border bg-black/50 hover:border-primary/50 hover:bg-primary/5"
								}`}
							>
								<div className={`flex size-10 shrink-0 items-center justify-center rounded-xl ${idx === activeCategory ? "bg-primary/20" : "bg-border transition-colors group-hover:bg-primary/10"}`}>
									<category.icon className={`size-5 ${idx === activeCategory ? "text-primary" : "text-foreground transition-colors group-hover:text-primary"}`} />
								</div>
								<div className="text-left">
									<div className={`font-semibold text-sm ${idx === activeCategory ? "text-white" : "text-foreground transition-colors group-hover:text-white"}`}>{category.name}</div>
									<div className="hidden text-foreground text-xs md:block">{category.description}</div>
								</div>
							</button>
						))}
					</div>
				...
				<div className="grid gap-8 lg:grid-cols-12">
							...
							<button
								...
								className={`group w-full rounded-2xl border p-6 text-left transition-all duration-300 ${
									idx === activeFeature
										...
										: "border-border bg-black/30 hover:border-primary/30 hover:bg-primary/5 hover:scale-102"
								}`}
							>
								<div className="flex items-start gap-4">
									<div className={`flex size-12 shrink-0 items-center justify-center rounded-xl transition-all duration-300 ${idx === activeFeature ? "scale-110 bg-primary/20" : "bg-border group-hover:scale-110 group-hover:bg-primary/10"}`}>
										<feature.icon className={`size-6 ${idx === activeFeature ? "text-primary" : "text-foreground transition-colors group-hover:text-primary"}`} />
									</div>
									<div className="flex-1">
										<div className="mb-2 flex items-center gap-2">
											<h3 className={`font-semibold text-lg ${idx === activeFeature ? "text-white" : "text-foreground transition-colors group-hover:text-white"}`}>{feature.name}</h3>
											{feature.badge && (
												<Badge className="gap-1 bg-primary/20 text-primary text-xs" variant="outline">
													{feature.badge}
												</Badge>
											)}
										</div>
										<p className={`text-sm ${idx === activeFeature ? "text-foreground" : "text-foreground/70 transition-colors group-hover:text-foreground"}`}>{feature.description}</p>
									</div>
									<ChevronRight className={`size-5 shrink-0 transition-all duration-300 ${idx === activeFeature ? "translate-x-1 text-primary" : "text-border group-hover:translate-x-1 group-hover:text-primary"}`} />
								</div>
							</button>
						))}
					</div>
					...
					<div className="lg:col-span-7">
						<div className={`sticky top-24 overflow-hidden rounded-3xl border border-primary/20 bg-gradient-to-br ${activeData.color} p-8 lg:p-12`}>
							{/* Icon & Title */}
							<div className="mb-8 flex items-start justify-between">
								<div>
									...
									<div className="mb-2 flex items-center gap-3">
										<h3 className="font-bold text-3xl text-white">{activeFeatureData.name}</h3>
										{activeFeatureData.badge && (
											<Badge className="gap-1.5 bg-primary/20 text-primary" variant="outline">
												<Sparkles className="size-3" />
												{activeFeatureData.badge}
											</Badge>
										)}
									</div>
									<p className="text-foreground text-lg">{activeFeatureData.description}</p>
								</div>
							</div>
							...
							{activeFeatureData.details && (
								<div className="mb-8 space-y-3">
									<h4 className="mb-4 font-semibold text-white text-xl">Key Features</h4>
									{activeFeatureData.details.map((detail, idx) => (
										<div key={idx} className="flex items-start gap-3 rounded-xl border border-primary/20 bg-black/40 p-4 backdrop-blur-sm transition-all duration-300 hover:border-primary/40 hover:bg-black/60">
				...
				<div className="mt-16 text-center">
					<p className="mb-6 text-foreground text-lg">Ready to see all {categories.reduce((acc, cat) => acc + cat.features.length, 0)} features in action?</p>
					<Button className="gap-2" size="lg">
						Start Free Trial
						<ArrowRight className="size-4" />
					</Button>
