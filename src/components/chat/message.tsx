...
export function ChatMessage({ message, isLoading = false }: MessageProps) {
	...
	return (
		<div className="group/message relative mx-auto flex w-full max-w-3xl items-start gap-3 px-4 py-4 md:gap-4 md:px-8 md:py-6">
			{/* Avatar */}
			<div className={cn("flex size-8 shrink-0 select-none items-center justify-center rounded-full", isUser ? "bg-blue-600 text-white" : "bg-zinc-100 dark:bg-zinc-800")}>
				...
				{toolInvocations.map((toolInvocation: any) => {
					...
					if (state === "result") {
						if (toolName === "getWeather") {
							return <Weather key={toolCallId} data={result} />;
						}

						if (toolName === "createDocument") {
							return (
								<div key={toolCallId} className="not-prose rounded-xl border bg-zinc-50 p-4 dark:bg-zinc-900">
