...
export function UsageTrendsChart({ companyId }: { companyId: string }) {
	...
	useEffect(() => {
		async function fetchTrends() {
			...
			const { data: communications } = await supabase.from("communications").select("*").eq("company_id", companyId).gte("created_at", startDate.toISOString()).lte("created_at", endDate.toISOString());

			// Fetch voicemails data
			const { data: voicemails } = await supabase.from("voicemails").select("*").eq("company_id", companyId).gte("received_at", startDate.toISOString()).lte("received_at", endDate.toISOString());
					...
					<TabsContent value="volume">
						<ResponsiveContainer width="100%" height={400}>
							<LineChart data={data}>
								...
								<Line type="monotone" dataKey="voicemails" stroke="#8b5cf6" strokeWidth={2} name="Voicemails" />
							</LineChart>
						</ResponsiveContainer>
					</TabsContent>
