...
export class WaterQualityService {
  private cache: Map<string, { data: WaterQuality; timestamp: number }> = new Map();
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 404 || res.status === 406) {
          ...
          console.warn(`[Water Quality Portal] Service unavailable: ${res.status}`);
        }
        return null;
      }
      ...
      const measurements = data
        .filter((row: any) => row.ResultMeasureValue && !isNaN(Number(row.ResultMeasureValue)))
        .map((row: any) => ({
  ...
  private generateRecommendations(
    ...
    if (classification === "soft") {
      ...
      estimatedSavings = 20000; // $200/year
      ...
      estimatedSavings = 50000; // $500/year
    } else {
      reason = "Your water is very hard. A water softener is highly recommended to prevent scale buildup and extend equipment life.";
      estimatedSavings = 80000; // $800/year
    }
