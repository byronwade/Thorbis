...
export async function getResourceItemBySlug(
	...
	let query = supabase.from("resource_items").select(RESOURCE_ITEM_SELECT).eq("slug", slug).limit(1);
...
export async function getResourceItemsByType(type: ResourceType, limit = 6): Promise<ResourceItem[]> {
