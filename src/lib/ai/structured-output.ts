...
const claudeHaiku = anthropic("claude-3-5-haiku-latest");
...
export const customerInsightsSchema = z.object({
	...
	opportunities: z.array(z.string()).describe("Upsell or service opportunities"),
	lastInteraction: z.string().optional().describe("Last significant interaction"),
	nextSteps: z.array(z.string()).describe("Suggested next steps"),
});

// Job analysis
export const jobAnalysisSchema = z.object({
	complexity: z.enum(["low", "medium", "high"]).describe("Job complexity level"),
...
export async function generateCustomerInsights(customerData: {
	...
	const { object } = await generateObject({
		model: claudeHaiku, // Use faster model for insights
...
Jobs:
${scheduleData.jobs
	.map((j) => `- ${j.id}: ${j.time}, ${j.duration}min, Tech: ${j.technicianId}, Location: ${j.location || "Unknown"}`)
	.join("\n")}
