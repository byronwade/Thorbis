...
const LAYOUT_RULES: LayoutRule[] = [
	...
	{
		pattern: /^\/dashboard\/tv-leaderboard/,
		config: {
			maxWidth: "full",
			padding: "none",
			gap: "none",
	...
	{
		pattern: /^\/dashboard\/work\/(invoices|estimates|materials|equipment|maintenance-plans|service-agreements|purchase-orders)/,
		config: {
			maxWidth: "full",
			padding: "none",
			gap: "none",
...
export function getLayoutConfig(pathname: string): LayoutConfig {
	...
	for (const rule of sortedRules) {
		const matches = typeof rule.pattern === "string" ? pathname === rule.pattern : rule.pattern.test(pathname);
...
export function getPaddingClass(padding: LayoutConfig["padding"], paddingX?: LayoutConfig["paddingX"], paddingY?: LayoutConfig["paddingY"]): string {
