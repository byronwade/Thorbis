...
export function verifyWebhookSignature(params: {
	...
	try {
		...
		const payloadBuffer = Buffer.isBuffer(params.payload) ? params.payload : Buffer.from(params.payload);
		const signedPayload = Buffer.concat([Buffer.from(params.timestamp), Buffer.from("|"), payloadBuffer]);
