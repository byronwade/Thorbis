...
export function generateMetadata(config: SEOConfig): Metadata {
	...
	const fullTitle = includeBrand || section ? buildTitle({ page: title, section, includeBrand }) : title;
	const url = canonical || buildCanonicalUrl({ path });
	const fullImage = buildShareImageUrl(image ? { path: image } : undefined);
	const combinedKeywords = Array.from(new Set([...(keywords ?? []), ...SEO_KEYWORDS])).filter(Boolean);
	...
	const alternateLocales = alternatesLanguages
		? Object.keys(alternatesLanguages).filter((lang) => lang !== locale && lang !== SEO_LOCALES.default)
		: undefined;
