...
export function generateMockTechnicians(count = 20): Technician[] {
  const firstNames = [
    "John",
    "Jane",
    "Mike",
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      ...
      skills: Array.from({ length: randomInt(2, 4) }, () =>
        randomChoice(skills)
      ),
  ...
  technicians.forEach((technician) => {
    for (let i = 0; i < jobsPerTechnician; i++) {
      ...
      const jobEndTime = new Date(
        jobStartTime.getTime() + durationMinutes * 60 * 1000
      );
      ...
      const job: Job = {
        ...
        createdAt: new Date(
          jobStartTime.getTime() - randomInt(1, 7) * 24 * 60 * 60 * 1000
        ),
        updatedAt: new Date(),
      };
