 ...
 */
export async function uploadDocument(formData: FormData): Promise<ActionResult<UploadDocumentResult>> {
	try {
		const { file, companyId, contextType, contextId, folder, description, tagsRaw } = extractUploadFormData(formData);
...
async function validateJobContext(
	...
	const { data: job, error: jobError } = await supabase.from("jobs").select("company_id").eq("id", contextId).single();
...
async function handleJobDocumentUpload(params: JobUploadParams): Promise<ActionResult<UploadDocumentResult>> {
...
async function handleNonJobDocumentUpload(params: NonJobUploadParams): Promise<ActionResult<UploadDocumentResult>> {
 ...
 */
export async function uploadDocuments(formData: FormData): Promise<ActionResult<UploadDocumentResult[]>> {
	try {
 ...
 */
export async function moveDocument(attachmentId: string, newFolderPath: string): Promise<ActionResult> {
	try {
		const supabase = await createClient();
		if (!supabase) {
			return { success: false, error: "Server configuration error" };
...
export async function createFolder(params: CreateFolderParams): Promise<ActionResult<{ folderId: string }>> {
	try {
		...
		let path = "/";
		if (parentId) {
			const { data: parent } = await supabase.from("document_folders").select("path").eq("id", parentId).single();
