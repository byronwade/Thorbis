...
export async function createSchedule(
	...
	try {
		...
		const { data: schedule, error } = await supabase.from("schedules").insert(validated).select("id").single();
...
export async function getSchedule(scheduleId: string): Promise<{ success: boolean; error?: string; schedule?: any }> {
	try {
		const supabase = await createClient();
...
export async function updateSchedule(
	...
	try {
		...
		const { error } = await supabase.from("schedules").update(validated).eq("id", scheduleId).is("deleted_at", null);
...
export async function markReminderSent(scheduleId: string): Promise<{ success: boolean; error?: string }> {
	try {
		const supabase = await createClient();
		...
		const { error } = await supabase.from("schedules").update({ reminder_sent: true }).eq("id", scheduleId);
