	...
	return withErrorHandling(async () => {
		...
		if (descendants && descendants.length > 0) {
			...
			for (const descendant of descendants) {
				...
				const newDescendantPath = newPath
					? `${newPath}${descendantPathSuffix}`
					: descendantPathSuffix.substring(1); // Remove leading dot
...
export async function updatePriceBookItem(
	...
	return withErrorHandling(async () => {
		...
		const priceChanged =
			...
			data.markupPercent !== item.markup_percent;
...
export async function bulkDeleteItems(itemIds: string[]): Promise<ActionResult<number>> {
	return withErrorHandling(async () => {
		...
		const { data: items } = await supabase.from("price_book_items");
