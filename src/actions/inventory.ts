...
export async function createInventory(formData: FormData): Promise<ActionResult<string>> {
	return withErrorHandling(async () => {
		...
		if (createError || !inventory) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("create inventory"), ERROR_CODES.DB_QUERY_ERROR);
		}
 ...
 */
export async function updateInventory(inventoryId: string, formData: FormData): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		const supabase = await createClient();
		if (!supabase) {
			throw new ActionError("Database connection failed", ERROR_CODES.DB_CONNECTION_ERROR);
		...
		if (updateError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("update inventory"), ERROR_CODES.DB_QUERY_ERROR);
		}
...
export async function deleteInventory(inventoryId: string): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		if (deleteError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("delete inventory"), ERROR_CODES.DB_QUERY_ERROR);
		}
...
export async function adjustStock(formData: FormData): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		const { error: updateError } = await supabase.from("inventory").update(updateData).eq("id", adjustment.inventoryId);

		if (updateError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("adjust stock"), ERROR_CODES.DB_QUERY_ERROR);
		}
...
function parseInventoryFormData(formData: FormData) {
	return inventorySchema.parse({
		...
		maximumQuantity: formData.get("maximumQuantity") ? Number(formData.get("maximumQuantity")) : undefined,
...
async function getPriceBookItemForCompanyOrThrow(
	...
	if (priceBookItem.company_id !== companyId) {
		throw new ActionError("Price book item not found", ERROR_CODES.AUTH_FORBIDDEN, HTTP_STATUS_FORBIDDEN);
	}

	return priceBookItem;
}

async function ensureNoExistingInventoryForItem(supabase: SupabaseClientType, priceBookItemId: string) {
...
function buildInventoryInsertPayload(companyId: string, data: ReturnType<typeof parseInventoryFormData>) {
...
function calculateAdjustedQuantities(
	...
	if (newQuantityOnHand < 0) {
		throw new ActionError("Stock adjustment would result in negative inventory", ERROR_CODES.BUSINESS_RULE_VIOLATION);
	}
...
export async function reserveStock(formData: FormData): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		if (updateError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("reserve stock"), ERROR_CODES.DB_QUERY_ERROR);
		}
 ...
 */
export async function releaseReservedStock(inventoryId: string, quantity: number): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		const supabase = await createClient();
		if (!supabase) {
			throw new ActionError("Database connection failed", ERROR_CODES.DB_CONNECTION_ERROR);
		...
		if (updateError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("release reserved stock"), ERROR_CODES.DB_QUERY_ERROR);
		}
...
export async function useReservedStock(
	...
	return withErrorHandling(async () => {
		...
		const { error: updateError } = await supabase.from("inventory").update(updateData).eq("id", inventoryId);

		if (updateError) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("use reserved stock"), ERROR_CODES.DB_QUERY_ERROR);
		}
...
export async function archiveInventoryItem(inventoryId: string): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		if (!inventory.deleted_at) {
			...
			if (archiveError) {
				throw new ActionError(ERROR_MESSAGES.operationFailed("archive inventory item"), ERROR_CODES.DB_QUERY_ERROR);
			}
		}
...
export async function getLowStockItems(): Promise<ActionResult<unknown[]>> {
	return withErrorHandling(async () => {
      ...
      `
		...
		if (error) {
			throw new ActionError(ERROR_MESSAGES.operationFailed("fetch low stock items"), ERROR_CODES.DB_QUERY_ERROR);
		}
 ...
 */
export async function getItemsNeedingStockCheck(daysSinceLastCheck = 30): Promise<ActionResult<unknown[]>> {
	return withErrorHandling(async () => {
		const supabase = await createClient();
		if (!supabase) {
			throw new ActionError("Database connection failed", ERROR_CODES.DB_CONNECTION_ERROR);
