...
function generateRevenueCSV(data: Awaited<ReturnType<typeof getRevenueReport>>): string {
	const lines: string[] = [];

	// Summary section
	lines.push("REVENUE REPORT SUMMARY");
	lines.push("Metric,Value");
	lines.push(`Total Revenue,${escapeCSV(formatCurrency(data.summary.totalRevenue))}`);
	lines.push(`Previous Period Revenue,${escapeCSV(formatCurrency(data.summary.previousPeriodRevenue))}`);
	lines.push(`Growth %,${escapeCSV(formatPercent(data.summary.growthPercent))}`);
	lines.push(`Average Ticket,${escapeCSV(formatCurrency(data.summary.averageTicket))}`);
	...
	for (const customer of data.topCustomers) {
		lines.push(`${escapeCSV(customer.name)},${escapeCSV(formatCurrency(customer.revenue))},${customer.jobCount}`);
	}
...
function generateJobsCSV(data: Awaited<ReturnType<typeof getJobPerformanceReport>>): string {
	const lines: string[] = [];
	...
	lines.push(`Completion Rate,${escapeCSV(formatPercent(data.summary.completionRate))}`);
	lines.push(`First Time Fix Rate,${escapeCSV(formatPercent(data.summary.firstTimeFixRate))}`);
	lines.push(`Callback Rate,${escapeCSV(formatPercent(data.summary.callbackRate))}`);
	lines.push(`Average Duration (mins),${data.summary.averageDuration.toFixed(0)}`);
	lines.push("");
	...
	for (const status of data.byStatus) {
		lines.push(`${escapeCSV(status.status)},${status.count},${escapeCSV(formatPercent(status.percentage))}`);
	}
	lines.push("");

	// By technician
	lines.push("JOBS BY TECHNICIAN");
	lines.push("Technician,Jobs Completed,Avg Duration (mins),First Time Fix Rate");
	...
	for (const week of data.trendsWeekly) {
		lines.push(`${escapeCSV(week.week)},${week.completed},${week.scheduled},${week.cancelled}`);
	}
...
function generateFinancialCSV(data: Awaited<ReturnType<typeof getFinancialReport>>): string {
	const lines: string[] = [];
	...
	lines.push(`Gross Profit,${escapeCSV(formatCurrency(data.summary.grossProfit))}`);
	lines.push(`Gross Margin,${escapeCSV(formatPercent(data.summary.grossMargin))}`);
	lines.push(`Outstanding AR,${escapeCSV(formatCurrency(data.summary.outstandingAR))}`);
	...
	lines.push(`Current,${data.arAging.current.count},${escapeCSV(formatCurrency(data.arAging.current.amount))}`);
	lines.push(`1-30 Days,${data.arAging.days1_30.count},${escapeCSV(formatCurrency(data.arAging.days1_30.amount))}`);
	lines.push(`31-60 Days,${data.arAging.days31_60.count},${escapeCSV(formatCurrency(data.arAging.days31_60.amount))}`);
	lines.push(`61-90 Days,${data.arAging.days61_90.count},${escapeCSV(formatCurrency(data.arAging.days61_90.amount))}`);
	lines.push(`Over 90 Days,${data.arAging.over90.count},${escapeCSV(formatCurrency(data.arAging.over90.amount))}`);
	lines.push("");
...
function generateTeamCSV(data: Awaited<ReturnType<typeof getTeamLeaderboard>>): string {
	const lines: string[] = [];
	...
	lines.push(`Avg Revenue per Tech,${escapeCSV(formatCurrency(data.teamAverages.revenuePerTech))}`);
	lines.push(`Team Avg Rating,${data.teamAverages.avgRating.toFixed(1)}`);
	lines.push(`Team First Time Fix Rate,${escapeCSV(formatPercent(data.teamAverages.firstTimeFixRate))}`);
	lines.push("");
	...
	lines.push("Rank,Name,Jobs Completed,Revenue,Avg Job Duration (mins),First Time Fix Rate,Rating");
...
function generateCustomersCSV(data: Awaited<ReturnType<typeof getCustomerAnalytics>>): string {
	const lines: string[] = [];
	...
	lines.push(`Retention Rate,${escapeCSV(formatPercent(data.summary.retentionRate))}`);
	lines.push(`Avg Lifetime Value,${escapeCSV(formatCurrency(data.summary.avgLifetimeValue))}`);
	lines.push("");
	...
	for (const month of data.acquisitionTrend) {
		lines.push(`${escapeCSV(month.month)},${month.newCustomers},${month.churned},${month.net}`);
	}
