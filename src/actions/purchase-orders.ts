...
function calculateTotals(lineItems: any[]): {
	...
	const subtotal = lineItems.reduce((sum, item) => sum + (item.total || item.unit_price * item.quantity), 0);
...
export async function createPurchaseOrder(formData: FormData): Promise<ActionResult<string>> {
	return withErrorHandling(async () => {
		...
		const poNumber = formData.get("po_number")?.toString() || (await generatePONumber(supabase, activeCompanyId));
 ...
 */
export async function updatePurchaseOrderLineItems(poId: string, lineItems: any[]): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		const supabase = await createClient();
		if (!supabase) {
			throw new ActionError("Database connection failed", ERROR_CODES.DB_CONNECTION_ERROR);
		...
		if (error) {
			throw new ActionError("Failed to update purchase order line items", ERROR_CODES.DB_QUERY_ERROR);
		}
 ...
 */
export async function archivePurchaseOrder(poId: string): Promise<{ success: boolean; error?: string }> {
	try {
		const supabase = await createClient();
		if (!supabase) {
...
export async function unlinkPurchaseOrderFromJob(poId: string): Promise<ActionResult<void>> {
	return withErrorHandling(async () => {
		...
		const { error: unlinkError } = await supabase.from("purchase_orders").update({ job_id: null }).eq("id", poId);
