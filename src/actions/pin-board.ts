...
async function assertAdminOrOwner(supabase: NonNullable<Awaited<ReturnType<typeof createClient>>>, companyId: string) {
...
export async function createPinBoardCategory(data: z.infer<typeof categorySchema>) {
...
export async function updatePinBoardCategory(id: string, data: Partial<z.infer<typeof categorySchema>>) {
  const supabase = await createClient();
  const companyId = await getActiveCompanyId();
...
export async function updatePinBoardPost(id: string, data: Partial<z.infer<typeof postSchema>>) {
  const supabase = await createClient();
  const companyId = await getActiveCompanyId();
...
export async function seedDefaultCategories() {
  ...
  try {
    ...
    const defaultCategories = [
      { name: "SOPs & Procedures", slug: "sops", icon: "file-text", color: "blue", sort_order: 0 },
      { name: "Contractors & Partners", slug: "contractors", icon: "users", color: "green", sort_order: 1 },
      { name: "Sales Materials", slug: "sales", icon: "briefcase", color: "purple", sort_order: 2 },
      { name: "Company Announcements", slug: "announcements", icon: "megaphone", color: "orange", sort_order: 3 },
      { name: "Training & Resources", slug: "training", icon: "graduation-cap", color: "cyan", sort_order: 4 },
    ];
