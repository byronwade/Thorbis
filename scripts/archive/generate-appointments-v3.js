...
function generateAppointmentsForDay(date, teamMember) {
	const appointments = [];
	const appointmentsPerDay = randomInt(1, 3); // 1-3 appointments per day (increased minimum)
...
async function generateAppointments() {
	...
	const { count: currentCount } = await supabase
		...
		.eq("type", "appointment")
		.eq("company_id", "2b88a305-0ecd-4bff-9898-b166cc7937c4")
		.is("deleted_at", null);

	console.log(`Current appointments: ${currentCount || 0}`);

	const targetTotal = 10000;
	const needed = targetTotal - (currentCount || 0);

	if (needed <= 0) {
		console.log("✅ Already have 10,000+ appointments!");
		return;
	}

	console.log(`Need to generate: ${needed} more appointments`);
	...
	// Keep generating until we have enough appointments
	while (allAppointments.length < needed && dayOffset < 730) {
		// Up to 2 years
		...
		if (dayOfWeek !== 0 && dayOfWeek !== 6) {
			...
			for (const teamMember of teamMembers) {
				if (allAppointments.length >= needed) break;

				const dayAppointments = generateAppointmentsForDay(date, teamMember);
	...
	// Trim to exact needed amount
	allAppointments = allAppointments.slice(0, needed);
	...
	for (let i = 0; i < allAppointments.length; i += batchSize) {
		...
		const progress = ((appointmentsCreated / needed) * 100).toFixed(1);
		process.stdout.write(
			`\rProgress: ${appointmentsCreated}/${needed} (${progress}%)`,
		);
	}
