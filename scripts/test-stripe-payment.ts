...
async function testPaymentFlow() {
  ...
  try {
    ...
    prices.data.forEach((price) => {
      const amount = price.unit_amount ? `$${(price.unit_amount / 100).toFixed(2)}` : 'Usage-based';
    ...
    if (!basePriceId) {
      console.error('   ❌ STRIPE_PRICE_ID_BASE_PLAN not configured in .env.local');
      return;
    }
    ...
    console.log(`  • Delete test customer: stripe customers delete ${customer.id}`);
    console.log('');
