...
async function cleanupIncompleteCompanies() {
	try {
		...
		memberships.forEach((m: any) => {
			console.log(`  - ${m.companies.name} (${m.companies.id}): ${m.companies.stripe_subscription_status}`);
		});
		...
		for (const membership of incompleteCompanies) {
			...
			const { error: archiveError } = await supabase
				...
				.update({
					...
					permanent_delete_scheduled_at: new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString(), // 90 days from now
				})
				.eq("id", companyId);
